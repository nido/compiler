...
struct operator_from_tree_t {
 ...
 GS_PRAGMA_STMT,                OPERATOR_UNKNOWN,
#endif
	       ...
	       UINT32 field_id)
{
  ...
  else if (code == GS_MODIFY_EXPR) {
    gs_t assign = gs_tree_operand(exp, 0); 
    gs_t val = gs_tree_operand(exp, 1); 

    WN *wn1 = WGEN_Expand_Expr (val);
    (void) WGEN_Lhs_Of_Modify_Expr(code, gs_tree_operand (exp, 0), NULL, FALSE,
                                  0, 0, 0, FALSE, wn1, 0, FALSE, FALSE);

    return WGEN_Array_Expr(assign, ty_idx, component_ty_idx, component_offset, field_id);
  }
		       ...
		       bool is_imagpart)
{
  ...
  default:
    Fail_FmtAssertion ("WGEN_Lhs_Of_Modify_Expr: unhandled tree node in LHS of MODIFY_EXPR (%s (%d))", gs_code_name (code), (int)code);
  }
...
#else
	  WN_set_ty (load_wn, MTYPE_To_TY (WN_desc(load_wn)));
#endif
      }
      break;
	...
	if (gs_tree_this_volatile(exp))
	  ...
	  if (gs_decl_built_in (func)) {
		...
		if (arglist == 0
		  ...
		  arg1 = gs_tree_value (arglist);
		  arg2 = gs_tree_value (gs_tree_chain (arglist));
		  gs_long_long_t len1 = c_strlen (arg1);
#ifdef TARG_ST
		  if (Enable_Expand_Builtin && len1 >= 0) {
#else
		  if (len1) {
#endif
		    gs_long_long_t len2 = c_strlen (arg2);
		    if (len2 >= 0) {
...
#endif
              arg_wn = WGEN_Expand_Expr (gs_tree_value (gs_tree_operand (exp, 1)));
	      // [SC] Note a better abstraction here:
	      // wn = WN_CreateExp1 (OPR_FLOOR, ret_mtype,
	      //		  TY_mtype(Get_TY (gs_long_double_type_node())), arg_wn);
#ifdef TARG_IA64
...
#endif
#ifdef TARG_ST100
	    case GSBI_BUILT_IN_MPSSE: 
	      iopc = INTRN_MPSSE;
	      intrinsic_op = TRUE;
	      break;

	    case GSBI_BUILT_IN_MASSE: 
	      iopc = INTRN_MASSE;
	      intrinsic_op = TRUE;
	      break;

	    case GSBI_BUILT_IN_DIVW: 
	      iopc = INTRN_DIVW;
	      intrinsic_op = TRUE;
	      break;

	    case GSBI_BUILT_IN_DIVUW: 
	      iopc = INTRN_DIVUW;
	      intrinsic_op = TRUE;
	      break;
#endif
...
#endif
#ifdef TARG_ST
#define INTRN_WGEN_EXPR
#include "gfec_2_gsbi.h"
#undef INTRN_WGEN_EXPR
