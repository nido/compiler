...
{
  // Arthur: I do not know what this really means. There are no
  //         dedicated return Pregs on ST100, so I try this ...
#ifdef TARG_IA64
...
BOOL WN_Verifier::STID_check_st_class(WN *wn)
{
  ...
  if (opr == OPR_STID) {
    ...
    {
      // TB: To avoid too much dev warm whith -fprofile-arcs...
      if (!Profile_Arcs_Enabled || ST_class(st)!=CLASS_NAME) {
...
WN_Verifier::Field_id_valid (WN* wn)
{
	...
	if (TY_kind(*ty) != KIND_STRUCT) {
	  if (WN_field_id(wn) != 0) {
	    DevWarn("non-zero field id for memory op on scalar");
	  }
...
/* (cbr) DDTSst24451. add support for label diffs initializers */
      case INITVKIND_LABDIFF:
        {
          LABEL_IDX lab0 = INITV_labd0(val_idx);
          LABEL_IDX lab1 = INITV_labd1(val_idx);
          if (lab_map->Find(lab0) || lab_map->Find(lab1))
            renamed_labels_found = TRUE;
        }
#endif
...
/* (cbr) DDTSst24451. add support for label diffs initializers */
      case INITVKIND_LABDIFF:
        {
          LABEL_IDX lab0 = INITV_labd0(new_initv);
          LABEL_IDX lab1 = INITV_labd1(new_initv);

          LABEL_IDX new_lab = lab_map->Find(lab0);
          Set_INITV_labd0(new_initv, new_lab); 
          new_lab = lab_map->Find(lab1);
          Set_INITV_labd1(new_initv, new_lab); 
        }
        old_initv = INITV_next(old_initv);
        prev = new_initv;
        break;
