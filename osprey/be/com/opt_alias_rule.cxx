...
//    FALSE -- not aliased
//
BE_EXPORTED BOOL ALIAS_RULE::Aliased_Ofst_Rule(const POINTS_TO *mem1, const POINTS_TO *mem2) const
{
...
//    FALSE -- not aliased
//
BE_EXPORTED BOOL ALIAS_RULE::Aliased_Memop_By_Analysis(const POINTS_TO *p1, const POINTS_TO *p2) const
{
...
//    FALSE -- not aliased
//
BE_EXPORTED BOOL ALIAS_RULE::Aliased_Memop_By_Declaration(const POINTS_TO *p1,
					      const POINTS_TO *p2,
					      TY_IDX ty1, TY_IDX ty2) const
{
  if (p1->Expr_kind() == EXPR_IS_INVALID ||
...
//    FALSE -- not aliased
//
BE_EXPORTED BOOL ALIAS_RULE::Aliased_Memop(const POINTS_TO *p1, const POINTS_TO *p2,
			       TY_IDX ty1, TY_IDX ty2) const
{
...
BE_EXPORTED BOOL ALIAS_RULE::Aliased_Memop(const POINTS_TO *p1, const POINTS_TO *p2) const
{
  if (!Aliased_Memop_By_Analysis(p1, p2))
    return FALSE;
...
//  
BE_EXPORTED READ_WRITE ALIAS_RULE::Aliased_with_Call(ST *st, INT32 flags, const POINTS_TO *mem) const
{
  if (mem->Dedicated()) 
    return WRITE;
...
BE_EXPORTED READ_WRITE ALIAS_RULE::Aliased_with_Intrinsic_Call(const WN *call_wn, const POINTS_TO *mem) const
{
...
BE_EXPORTED READ_WRITE ALIAS_RULE::Aliased_with_Asm(const WN *wn, const POINTS_TO *mem) const
{
  if (mem->Dedicated())
    return READ_AND_WRITE;
...
//
BE_EXPORTED BOOL ALIAS_RULE::Aliased_with_Global(const POINTS_TO *mem) const
{
...
//
BE_EXPORTED BOOL ALIAS_RULE::Aliased_with_Indirect(const POINTS_TO *mem) const
{
