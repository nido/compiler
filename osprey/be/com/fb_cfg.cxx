...
#ifdef TARG_ST
//TB: merge code from open64 0.15
/*
...
#ifdef TARG_ST
// 	[TB]: Tranform critical edge check into a warning: to be
// 	further analyzed to understand why after annotation, cdg
// 	contains critical edges
	if (!(node.one_edge_preds || pred.one_edge_succs))
	  DevWarn( "FEEDBACK::Verify found node[%d] and pred[%d] with one_edge_* both false", nx, nx_pred );
#else
...
// 	further analyzed to understand why after annotation, cdg
// 	contains critical edges
	if (!(node.one_edge_succs || succ.one_edge_preds))
	  DevWarn( "FEEDBACK::Verify found node[%d] and pred[%d] with one_edge_* both false", nx, nx_succ );
#else
