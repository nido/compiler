...
BE_EXPORTED void Create_Restricted_Map(MEM_POOL *pu_pool)
{
...
BE_EXPORTED void Delete_Restricted_Map(void)
{
  if (WOPT_Enable_Restricted_Map && Restricted_map != NULL) {
...
BE_EXPORTED void ALIAS_MANAGER::Gen_alias_id_list(WN *wn, POINTS_TO_LIST *ptl)
{
  if (ptl == NULL) {
    Set_id(wn,0);
    return;
...
BE_EXPORTED ALIAS_MANAGER *Create_Alias_Manager(MEM_POOL *pu_pool)
{
  return CXX_NEW(ALIAS_MANAGER(), pu_pool);
}

//  Delete the alias manager
BE_EXPORTED void Delete_Alias_Manager(ALIAS_MANAGER *am, MEM_POOL *pu_pool)
{
...
#ifdef TARG_ST
  // [CG] Why ANSI type rules can't be applied also to STORE <--> STORE ?
#endif
...
ALIAS_RESULT Aliased_with_region(const ALIAS_MANAGER *am, const WN *wn, const WN *region_or_call, READ_WRITE how)
{
  ...
  if (opr == OPR_ASM_STMT) {
#ifdef TARG_ST
    // [CG 2004/11/18] This is a misusage of the Aliased_with_Asm method,
    // the method expects the asm wn as first argument and a point_to
    // as second argument.
...
//
void Dump_alias_mgr(const ALIAS_MANAGER *am, const WN *tree, FILE *fp)
{
  am->Print(tree, fp);
}
...
#ifdef TARG_ST
  BOOL may_refer_to_alloca_mem_b;

  if (alias_id != 0) {
    POINTS_TO *pt = Pt(alias_id);
    may_refer_to_alloca_mem_b = !pt->Not_alloca_mem();
  } else {
    may_refer_to_alloca_mem_b = TRUE;
  }
#else
  POINTS_TO *pt = Pt(alias_id);
#endif
...
#ifdef TARG_ST
	    may_refer_to_alloca_mem_b ? "may" : "does not");
#else
	    pt->Not_alloca_mem() ? "does not" : "may");
#endif
  }
#ifdef TARG_ST
  // if alias_id == 0, we have no alias info, so convervatively return
  // TRUE.
  return may_refer_to_alloca_mem_b;
#else
  return !pt->Not_alloca_mem();
#endif
}

BOOL May_refer_to_alloca_mem(const ALIAS_MANAGER *am, const WN *wn)
{
...
#ifdef TARG_ST
  // This function is now defined here and not in wopt.
BE_EXPORTED void
