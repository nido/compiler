 ...
 */
BE_EXPORTED extern void St_Block_Union(ST *blk1, ST *blk2);
 ...
 * for addressing. */
BE_EXPORTED extern void  Base_Symbol_And_Offset_For_Addressing (
 ...
 */
BE_EXPORTED extern INT32 Adjusted_Alignment ( ST *st);
 ...
 */
BE_EXPORTED extern INT32 ST_alignment ( ST *st);

/* create a slink symbol in current symtab */
BE_EXPORTED extern void Create_Slink_Symbol (void);

/* find slink symbol in specified symtab */
BE_EXPORTED extern ST* Find_Slink_Symbol (SYMTAB_IDX stab);
/* return slink symbol that matches the ST_symtab_id(st) */
BE_EXPORTED extern ST *Find_Slink_For_ST(ST *st);

/*
 * return slink symbol at level ST_scope_id(function) - ST_scope_id(sym)
 */
BE_EXPORTED extern ST *Find_Slink_For_Scope(ST *function, ST *sym);

/* Determine if the ST is an uplevel reference */
BE_EXPORTED extern BOOL ST_is_uplevelTemp(const ST *);
