...
#ifdef TARG_ST // [CL] fix for asm()
      // preg for asm() is negative
      // We should probably ensure unique numbers,
      // but it currently seems useless as optimizations
      // are disabled around asm() statements and
      // thus the STID following and ASM_STMT always
      // remains next to it -> no risk to overwrite
      // a result before having saved it
      if (offst >= 0)
#endif
      if (!Preg_Is_Dedicated(offst))
...
IPO_SYMTAB::Copy_Local_Tables(BOOL label_only)
{
  ...
  if (label_only == FALSE) {
	...
	// [CL] merged fix from Open64 0.16
...
{
    ST *copy_st;
#ifdef TARG_ST // [CL] grabbed from version 0.15 and fixed
