...
// [HK]
//
// Provide trunc(), which doesn't exist in all the GNU library. This is a
// quick and dirty hack, and should be handled elsehow.
//
static inline double trunc__(double d)
{
  if (d < 0.0) {
    return 1.0 + floor(d);
  }
...
static WN *em_exp_float(WN *block, WN *x, WN *pow, TYPE_ID type)
{
  if (Is_Constant(pow))
  {
    ...
    */
    ...
    {
      WN  *ipow = WN_Intconst(MTYPE_I4, (INT64) trunc__(n));
...
#else
// Arthur: temporary -- I am too lazy to enter the number.
static QUAD_TYPE M_LOG10Q = {0,0,0,0};
#endif
