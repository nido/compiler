...
Create_And_Set_ST_Base(ST *blk1, ST *blk2, STACK_DIR dir)
{
...
#ifdef TARG_ST
  // [CG]: Currently this this is unsupported.
  // Indeed setting the base symbol for an array make further 
  // wopt optimizations to reference the base instead of the array itself.
  // As cgemit does not emit base CLASS_BLOCK symbols undefined
  // symbol error arise.
  if (Is_Global_Symbol(blk1_base))
    DevWarn("Create_And_Set_ST_Base(): Merging of global symbols is unsafe");
#endif
  ...
  Is_True(Has_No_Base_Block(blk2),
	  ("Block_Merge: ST_base of blk2 is already initialized"));

#if 0
  fprintf(TFile, "blk1: %p %s (base: %p %s)\n",  blk1, ST_name(blk1), blk1_base, ST_name(blk1_base));
  fprintf(TFile, "blk2: %s (base: %s)\n",  ST_name(blk2), ST_name(Base_Symbol(blk2)));
#endif
  if (ST_class(blk1_base) != CLASS_BLOCK) {
	...
	ST_size(blk1_base));
      ...
      Enter_ST(base);
#if 0
      fprintf(TFile, "now blk1_base: %s (base: %s)\n",  ST_name(blk1_base), ST_name(Base_Symbol(blk1_base)));
#endif
  }
...
#if 0
  fprintf(TFile, "now blk2: %s (base: %s)\n",  ST_name(blk2), ST_name(Base_Symbol(blk2)));
#endif
  return base;
}
...
#if 0
  fprintf(TFile, "ST_Block_Union of : (%s %x, base %s %x) and (%s %x, base %s %x)\n", 
	  ST_name(blk1), blk1, ST_name(ST_base(blk1)), ST_base(blk1), 
	  ST_name(blk2), blk2, ST_name(ST_base(blk2)), ST_base(blk2));
#endif
