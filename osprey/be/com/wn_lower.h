...
BE_EXPORTED extern PARITY WN_parity(WN *tree);

/* return true if parity shows independence */
BE_EXPORTED extern BOOL WN_parity_independent(WN *wn1, WN *wn2);

/*
 * lowering specific initialization
 */
BE_EXPORTED extern void Lower_Init(void);
BE_EXPORTED extern void Lowering_Finalize(void);

BE_EXPORTED extern FLD_HANDLE FLD_And_Offset_From_Field_Id (TY_IDX  struct_ty_idx, 
					        UINT    field_id, 
					        UINT&   cur_field_id,
					        UINT64& offset);

BE_EXPORTED extern WN *WN_Lower(WN *tree, LOWER_ACTIONS actions, struct ALIAS_MANAGER *alias, char *msg);

/*
 * lower an scf node but not things underneath it 
 */
BE_EXPORTED extern WN *lower_scf_non_recursive(WN *block, WN *tree, LOWER_ACTIONS actions);

/* 
 * lower a block node and everything underneath it  
 */
BE_EXPORTED extern WN *lower_block(WN *tree, LOWER_ACTIONS actions);

/*
 * character string for action
 */
BE_EXPORTED extern const char *LOWER_ACTIONS_name(LOWER_ACTIONS action);
 ...
 */
BE_EXPORTED extern UINT32 compute_offset_alignment(INT32 offset, UINT32 align);

/*
 * return alignment consistant with TY, ST and offset
 */
BE_EXPORTED extern TY *compute_alignment_type(WN *tree, TY *type, INT64 offset);

/*
 * check trace flags and generate appropriate dumps
 */
BE_EXPORTED extern void WN_Lower_Checkdump(char *msg, WN *tree, LOWER_ACTIONS actions);

/*
 * lower M or lower WHIRL to conform to unsigned 64-bit instruction only ISA
 */
BE_EXPORTED extern void U64_lower_wn(WN *, BOOL);
