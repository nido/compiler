...
class BE_EXPORTED_CLASS ALIAS_RULE {
...
public:
  ...
  BE_EXPORTED_METHOD ALIAS_RULE(ALIAS_CONTEXT ac):_context(ac) {}

  BE_EXPORTED_METHOD BOOL Aliased_Strongly_Typed_Rule(TY_IDX , TY_IDX ) const;

  //  Check if a rule is applicable
  BE_EXPORTED_METHOD BOOL Rule_enabled(ALIAS_CONTEXT rule) const   {  return _context & rule; }

  //  Add all call-by-reference language here!!!
  BE_EXPORTED_METHOD BOOL Call_by_reference(void) const   { return Rule_enabled(F_CALL_RULE); }

  // Get/Set alias context.
  BE_EXPORTED_METHOD void Set_context(ALIAS_CONTEXT c)    { _context = c; }
  BE_EXPORTED_METHOD ALIAS_CONTEXT Get_context(void)      { return _context; }

  //  Two memory operations are referencing the same location.
  BE_EXPORTED_METHOD BOOL Same_location(const WN *, const WN *, const POINTS_TO *, const POINTS_TO *);

  //  Offset,size of two memop overlapped.
  BE_EXPORTED_METHOD BOOL Aliased_Ofst_Rule(const POINTS_TO *, const POINTS_TO *) const;
  ...
  //
  BE_EXPORTED_METHOD BOOL Aliased_Memop(const POINTS_TO *, const POINTS_TO *) const;
  BE_EXPORTED_METHOD BOOL Aliased_Memop(const POINTS_TO *, const POINTS_TO *, TY_IDX , TY_IDX ) const;
  BE_EXPORTED_METHOD BOOL Aliased_Memop_By_Analysis(const POINTS_TO *, const POINTS_TO *) const;
  BE_EXPORTED_METHOD BOOL Aliased_Memop_By_Declaration(const POINTS_TO *, const POINTS_TO *, TY_IDX , TY_IDX ) const;
  BE_EXPORTED_METHOD BOOL Aliased_Memop_By_Offset(const POINTS_TO *, const POINTS_TO *) const;

  BE_EXPORTED_METHOD BOOL Aliased_with_Global(const POINTS_TO *) const;
  BE_EXPORTED_METHOD BOOL Aliased_with_Indirect(const POINTS_TO *) const;

  //   The second set returns whether a scalar is aliased by a CALL
  //
  BE_EXPORTED_METHOD READ_WRITE Aliased_with_Call(ST *, INT32, const POINTS_TO *) const;
#ifdef TARG_ST
  //   Returns whether a scalar is aliased with an INTRINSIC_CALL
  //
  BE_EXPORTED_METHOD READ_WRITE Aliased_with_Intrinsic_Call(const WN *, const POINTS_TO *) const;
#endif
  // READ_WRITE Aliased_with_Region(REGION *, const POINTS_TO *);

  BE_EXPORTED_METHOD READ_WRITE Aliased_with_Asm(const WN *, const POINTS_TO *) const;

#ifdef TARG_ST
  // [CG] These functions are now defined in wopt, as they are local to
  // wopt
#else
  ...
  //
  BE_EXPORTED_METHOD const BS *Alias_Set_Indirect(const OPT_STAB *) const;
  BE_EXPORTED_METHOD const BS *Alias_Set_Call_By_Value(const OPT_STAB *) const;
  BE_EXPORTED_METHOD const BS *Alias_Set_Call_By_Ref(const OPT_STAB *) const;
  BE_EXPORTED_METHOD const BS *Alias_Set_Return(const OPT_STAB *) const;
  BE_EXPORTED_METHOD const BS *Alias_Set_Asm(const OPT_STAB *) const;
#endif
};
