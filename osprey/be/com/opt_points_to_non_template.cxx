...
#ifdef TARG_ST
  // [CG 2004/12/13]
  // The comment above is wrong. There may be a cycle in the use-def
  // chain while the pointer is not a linear induction variable.
  // For instance:
  // st1v0 <- base
  // LOOP:
  //  st1v2 <- PHI(st1v0, st1v1)
  //  use st1v2
  //  st1v1 <- other_base
  // END_LOOP:
  // In this case the st1v2 does not points to (base+unknown).
  // We really can't determine the points to.
  // Thus this optimistic assumption does not work with the
  // recursive FSA algorithm.
  // It works though with the fix point FPFSA algorithm
  // enabled with WOPT_Enable_FPFSA.
#endif
...
#ifdef TARG_ST
      /* (cbr) for static c++ initializers. assume const_var can be written */
      (ST_class(st) == CLASS_VAR && ST_is_const_var (st) &&
       !PU_is_static_initializer (Get_Current_PU()))) {
#else
      (ST_class(st) == CLASS_VAR && ST_is_const_var (st))) {
#endif
...
#ifdef TARG_ST
      // [SC] Be pessimistic about variables referenced in nested PUs,
      // because we can reach the nested PU via a function pointer.
      if (!ST_addr_saved(st) && !ST_has_nested_ref(st))
#else
      if (!ST_addr_saved(st))
#endif
	Set_not_addr_saved();
