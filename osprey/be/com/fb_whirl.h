...
class BE_EXPORTED_CLASS FEEDBACK {
private:
...
public:
  ...
  BE_EXPORTED_METHOD FEEDBACK( WN *wn, MEM_POOL *m,
	    INT32 invoke_size = 1,
	    INT32 branch_size = 1,
	    INT32 loop_size = 1,
  ...
  BE_EXPORTED_METHOD void  FB_set_in_out_same( WN *wn );

  BE_EXPORTED_METHOD void  Print         ( FILE *fp, const WN *wn ) const;
  BE_EXPORTED_METHOD void  Print_with_wn ( FILE *fp, WN *wn ) const;

  BE_EXPORTED_METHOD const FB_Info_Invoke&  Query_invoke  ( const WN *wn ) const;
  BE_EXPORTED_METHOD const FB_Info_Branch&  Query_branch  ( const WN *wn ) const;
  ...
  BE_EXPORTED_METHOD FB_FREQ Query_total_out ( const WN *wn ) const;

  BE_EXPORTED_METHOD void Annot_invoke  ( WN *wn, const FB_Info_Invoke & fb_info );
  BE_EXPORTED_METHOD void Annot_branch  ( WN *wn, const FB_Info_Branch & fb_info );
  ...
  BE_EXPORTED_METHOD void Delete(WN *wn) {
    IPA_WN_MAP32_Set ( _maptab, WN_MAP_FEEDBACK, wn, 0 );
  }

  BE_EXPORTED_METHOD FB_VERIFY_STATUS Verify( const char *caller         = NULL,
			   bool        abort_if_error = TRUE ) const;

  BE_EXPORTED_METHOD FB_VERIFY_STATUS Verify_and_guess( const char *caller         = NULL,
				     bool        abort_if_error = TRUE ) const;
  ...
  BE_EXPORTED_METHOD void FB_lower_branch  ( WN *wn_br,   WN *wn_branch );
  BE_EXPORTED_METHOD void FB_lower_circuit ( WN *wn_cand, WN *wn_left_br, WN *wn_right_br );
  BE_EXPORTED_METHOD void FB_factor_circuit( WN *wn_left, WN *wn_right,
			  WN *wn_outer, WN *wn_inner );

  BE_EXPORTED_METHOD void FB_lower_loop     ( WN *wn_loop, WN *wn_top_br, WN *wn_back_br );
  BE_EXPORTED_METHOD void FB_lower_loop_alt ( WN *wn_loop, WN *wn_top_br );
  BE_EXPORTED_METHOD void FB_lower_while_do_to_do_while ( WN *wn_loop, WN *wn_top_br );
  ...
  BE_EXPORTED_METHOD void FB_convert_goto_to_if(   WN *wn_branch, WN *wn_if );
  BE_EXPORTED_METHOD void FB_convert_goto_to_loop( WN *wn_branch, WN *wn_loop );

  BE_EXPORTED_METHOD void FB_simplify_branch_to_goto( WN *wn_branch );

  // Cloning (clone freq = original freq * scale; original reduced by same)
  BE_EXPORTED_METHOD void FB_set_zero_node(    WN *wn );
  BE_EXPORTED_METHOD void FB_set_zero(         WN *wn );
  BE_EXPORTED_METHOD void FB_set_unknown_node( WN *wn );
  BE_EXPORTED_METHOD void FB_set_unknown(      WN *wn );
  BE_EXPORTED_METHOD void FB_scale_node(       WN *wn, FB_FREQ freq_scale );
  BE_EXPORTED_METHOD void FB_scale(            WN *wn, FB_FREQ freq_scale );
  BE_EXPORTED_METHOD void FB_duplicate_node(   WN *wn_origl, WN *wn_clone );
  BE_EXPORTED_METHOD void FB_duplicate(        WN *wn_origl, WN *wn_clone );
  BE_EXPORTED_METHOD void FB_recombine_node(   WN *wn_origl, WN *wn_clone );
  BE_EXPORTED_METHOD void FB_recombine(        WN *wn_origl, WN *wn_clone );
  BE_EXPORTED_METHOD void FB_clone_node(       WN *wn_origl, WN *wn_clone, FB_FREQ freq_scale );
  BE_EXPORTED_METHOD void FB_clone(            WN *wn_origl, WN *wn_clone, FB_FREQ freq_scale );
...
public:
  BE_EXPORTED_METHOD void FB_clone_loop_test( WN *wn_origl, WN *wn_clone, WN *wn_loop );

  BE_EXPORTED_METHOD void Display_FB_CFG_From_Whirl(const char *caller = NULL);
