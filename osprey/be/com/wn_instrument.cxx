...
#ifdef TARG_ST
  // [CG] Set the no_instrument_function attribute
...
#else
  WN *instr;
  // TB: Fix a bug for switch without any case inside. 
  if (num_case_values != 0)
    instr = Gen_Call_ref35( SWITCH_INIT_NAME, PU_Handle(),
				total_switches, switch_num_targets,
				total_case_values, switch_case_values );
  else {
    instr = Gen_Call_ref35( SWITCH_INIT_NAME, PU_Handle(),
			    total_switches, switch_num_targets,
				total_case_values, WN_Intconst( MTYPE_U4, 0 ) );
  }
#endif
...
WN_Instrument( WN *wn, PROFILE_PHASE phase )
{
  ...
  if (ST_asm_function_st(*WN_st(wn))) return;
#endif
