...
#ifdef TARG_ST
static TY_IDX
field_type (TY_IDX struct_type, INT32 field_id)
{
  UINT cur_field_id = 0;
  FLD_HANDLE fld = FLD_get_to_field (struct_type, field_id,
				     cur_field_id);
...
// ============================================================
// [CG]: Returns the object type for a var or ivar.
// For ivar returns the pointed object type.
// Equivalent to WN_object_ty (wn_util.cxx)
// ============================================================
TY_IDX
CODEREP::object_ty(void) const
{ 
  Is_True(Kind() == CK_IVAR || Kind() == CK_VAR,
	  ("CODEREP::object_ty, illegal kind"));
  /* First get pointed type. */
  TY_IDX pointed;
  if (Kind() == CK_VAR) {
    pointed = Lod_ty();
    if (!Bit_field_valid() &&
	Field_id() != 0 && TY_kind(pointed) == KIND_STRUCT)
      return field_type(pointed, Field_id());
  } else {
    OPERATOR opr = Opr();
    if (OPERATOR_has_1ty(opr)) {
	  ...
	  opr == OPR_MLOAD ||
	  opr == OPR_ISTBITS) {
	pointed = TY_pointed (Ilod_ty());
	DevAssert(pointed, ("TY_pointed of %s type is NULL", OPERATOR_name(opr)));
      } else {
	pointed = Lod_ty();
      }
    } else if (OPERATOR_has_2ty(opr)) {
      // We must dereference the second type.
      pointed = TY_pointed (Ilod_base_ty());
      DevAssert(pointed, ("TY_pointed of type 2 of operator %s is NULL", OPERATOR_name(opr)));
    }
    if ((opr != OPR_ILDBITS && opr != OPR_ISTBITS) && 
	I_field_id() != 0 && TY_kind(pointed) == KIND_STRUCT)
      return field_type(pointed, I_field_id());
  }
  return pointed;
...
static BOOL Sign_extended(MTYPE dtyp, MTYPE dsctyp)
{
  ...
  if (MTYPE_is_class_integer(dtyp) && MTYPE_is_class_integer(dsctyp) &&
...
CODEREP::Print(INT indent, FILE *fp) const
{
  ...
  if (Kind() == CK_OP) {
    ...
    // Arthur: Print mu first:
    // Check if the MU has been deleted.
    if (Ivar_mu_node() && Ivar_mu_node()->OPND())  {
      for (i = 0; i < indent; i++) fprintf(fp, " ");
      fprintf(fp, " mu (sym%d<cr%d>)\n", Ivar_mu_node()->OPND()->Aux_id(), 
...
CODEREP::Fixup_type(MTYPE opr_type, CODEMAP *htable)
{
  ...
  case NEED_CVTL:
    ...
    cr->Init_expr(opc, this);
#ifdef TARG_ST
    // FdF 20041202: Size of CVTL is not opr_type !
    cr->Set_offset(MTYPE_size_min(opr_type));
#else
...
#if 0
  fprintf(TFile, "CODEMAP::Canon_add_sub WN:\n");
...
#if 0
  fprintf(TFile, "CODEMAP::Canon_add_sub retv:\n");
  retv->Print(0,TFile);
#endif
...
#ifdef TARG_ST
  // Arthur: only can do this if not Only_32_Bit_Ops
  if (WOPT_Enable_Cvt_Folding && !Only_32_Bit_Ops &&
#else
		 ...
		 UINT field_id, BOOL is_store)
{
...
#if 0
  Is_Trace(Tracing(), (TFile, "CODEMAP::Add_def dtyp = %s, dsctyp = %s\n",
	                         MTYPE_name(dtyp), MTYPE_name(dsctyp)));
#endif
...
#ifdef TARG_ST
  /* CVTL-RELATED start (correctness) */
  // Arthur: this should also take I5/U5 types into account
  //         I tried to generalize the code for the IA-64
  if ((MTYPE_is_class_integer(dtyp) && MTYPE_is_class_integer(dsctyp)) &&
      (MTYPE_size_min(dtyp) > MTYPE_size_min(MTYPE_I4)) &&
      (MTYPE_size_min(dsctyp) == MTYPE_size_min(MTYPE_I4))) {
    if ( ! is_store ) 
  ...
  /* CVTL-RELATED finish */
  /*  
  Is_Trace(Tracing(), (TFile, "CODEMAP::Add_def retv\n"));
  Is_Trace_cmd(Tracing(), retv->Print(0, TFile));
  */
  return retv;
}
...
#if 0
        // Arthur: I was not happy with what it used to do
        INT need_cvt =
        Need_load_type_conversion (to_dtyp, Dtyp(), Dsctyp(),
                                   is_sign_extd, to_sign_extd, &opc);
#endif
        INT need_cvt =
...
CODEMAP::Cur_def(WN *wn, OPT_STAB *opt_stab)
{
  ...
  if ( retv == NULL ) {
...
#if 0
    fprintf(TFile, "  cached current def :\n");
    retv->Print(0,TFile);
#endif
...
#ifdef TARG_ST
    //
    // Account for the I5/U5 types
    // Arthur: It is possible that the retv->Dtyp() is smaller than
    //         dtyp = WN_rtype(wn). It happens for example when an
    //         int variable on ST100 is passed to a function as a
    //         long, so 'wn' is {I,U}5{I,U}4LDID whereas the 'retv'
    //         will be {I,U}4{I,U}4LDID (bug in autcor.5.c). 
    //
    //         The CODEMAP::Add_expr() will add the CVT:
    //              I4I4LDID
    //            I5I4CVT
    //          I5PARM
    //
    if (dsctyp != MTYPE_BS &&
	     MTYPE_is_class_integer(dtyp) && 
	     MTYPE_is_class_integer(dsctyp) &&
	     MTYPE_size_min(dsctyp) > MTYPE_size_min(dtyp)) {
	...
	MTYPE_size_min(dsctyp) > MTYPE_size_min(dtyp)) {
      ...
      retv = retv->Var_type_conversion(this, 
                                dtyp, dsctyp, ty, WN_field_id(wn));
...
#if 0
      // Arthur:
      //         dsctyp <= dtyp && dsctyp != MTYPE_BS
      //
      //         U64-bit lowering "canonicalizes" LOADs into
      //         something like U8CVTL 16 (U8U8LDID) before the CODEMAP
      //         is built.
      //         This is important for being able to hash different
      //         variables (represented by LDIDs, ILOADs, and what
      //         else) efficiently and do the PRE on LOAD/STOREs.
      //         For example, I want to recognize that a LOAD of I2
      //         is a PRE with respect to a LOAD of I4 subject to
      //         sign-extending the upper 16 bits. The two loads
      //         should be considered same from the standpoint of
      //         LPRE and a CVTL should be inserted.
      //
      //         There is no U64_lowering for the ST100, but I may still
      //         need to "canonicalize" those LOADs in case if I
      //         enable the PRE between integer and subinteger types.
      //         see opt_etable.cxx, Is_the_same_as() comment.
      //
      //         What better place to do it ?? -- do it here while 
      //         building the CODEMAP ?
      //
      //         Var_type_conversion() could perhaps do it ??
      //         However, Var_type_conversion() is used by many routines
      //         and I am not sure how a change to it will affect the 
      //         rest of the compiler. Cur_def() is a pretty localized
      //         routine used, it seems, only for building the hashed
      //         CODEMAP.
      //
      //         Change I4I{1,2}LDID into a I4CVTL(I4I2LDID), etc.
      //         NOTE: the dsctyp of the LOAD can not be reset -- we
      //               will loose an important information.
      //
      /*
      else if (retv->Kind() != CK_OP &&
	       MTYPE_is_class_integer(retv->Dtyp()) && 
	       MTYPE_is_class_integer(retv->Dsctyp()) &&
	       (MTYPE_size_min(retv->Dtyp()) == MTYPE_size_min(MTYPE_I4)) &&
	       (MTYPE_size_min(retv->Dsctyp()) < MTYPE_size_min(MTYPE_I4))) {
	CODEREP *new_cr = Alloc_stack_cr(0);
	OPCODE opc = OPCODE_make_op(OPR_CVTL, retv->Dtyp(), MTYPE_V);
	new_cr->Init_expr(opc, retv);
	new_cr->Set_offset(MTYPE_size_min(retv->Dsctyp()));
	retv = Hash_Op(new_cr);
      }
      */
#endif
		  ...
		  COPYPROP *copyprop)
{
  ...
  if (WN_operator(wn) == OPR_SHL) {
    fprintf(TFile, "CODEMAP::Add_expr \n");
    fdump_tree_no_st(TFile, wn);
  }
#endif
  ...
  if (OPCODE_is_leaf(op)) {
...
#if 0
      fprintf(TFile, "CODEMAP::Add_expr Cur_def tree\n");
      retv->Print(0, TFile);
#endif
	...
	stmt->Set_volatile_stmt();
      ...
      if (WOPT_Enable_Input_Prop && !copyprop->Disabled()) {
        ...
        if (newtree) {
#if 0
	  fprintf(TFile, "CODEMAP::Add_expr copyprop tree\n");
	  newtree->Print(0, TFile);
#endif
	  if (retv->Kind() == CK_VAR) 
	    retv = newtree;
	  else {
...
#if 0
	  fprintf(TFile, "CODEMAP::Add_expr retv\n");
	  retv->Print(0, TFile);
#endif
...
STMTREP::Enter_rhs(CODEMAP *htable, OPT_STAB *opt_stab, COPYPROP *copyprop, EXC *exc)
{
  ...
  case OPR_MSTORE:
#if 0
    fprintf(TFile, "  ***** STMTREP::Enter_rhs Wn(): \n");
    fdump_tree_no_st(TFile, Wn());
#endif
    Set_rhs(htable->Add_expr(WN_kid0(Wn()), opt_stab, this, &proped, copyprop));
#if 0
    fprintf(TFile, "  ***** STMTREP::Enter_rhs Rhs(): \n");
    Rhs()->Print(0,TFile);
#endif
    if (WOPT_Enable_Input_Prop && proped) {
      FOLD ftmp;
  ...
  case OPR_MSTORE:
	...
	base_ccr.Set_scale(0);
      ...
      Set_lhs(htable->Add_idef(opc, opt_stab->Get_occ(Wn()), 
			       ...
			       MTYPE_M,
//			       MTYPE_M,
// Arthur: MLOAD's desc can only be MTYPE_V !
			       MTYPE_V,
			       WN_ty(Wn()),
			       WN_field_id(Wn()),
			       base_ccr.Scale(),
...
STMTREP::Print(FILE *fp) const
{
  ...
  if (Has_chi()) {
	...
	fprintf(fp, "sym%d<cr%d> <- chi(", 
                    cnode->Aux_id(), cnode->RESULT()->Coderep_id());
	fprintf(fp, "sym%d<cr%d>) ", 
                      cnode->Aux_id(), cnode->OPND()->Coderep_id());
      }
      else {
	fprintf(fp, "sym%dv%d <- chi(", cnode->Aux_id(), cnode->Result());
	fprintf(fp, "sym%dv%d) not-live", cnode->Aux_id(), cnode->Opnd());
      }
      fprintf(fp, "\n");
#else
      cnode->Print(fp);
