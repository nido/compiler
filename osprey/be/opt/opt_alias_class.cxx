...
ALIAS_CLASS_REP::Process_pending(ALIAS_CLASSIFICATION &ac)
{
  ...
  if (ac.Tracing() && WOPT_Enable_Verbose) {
					...
					AC_PTR_OBJ_PAIR rhs)
{
  ...
  if (rhs.Obj_class() != NULL) {
    ...
    if (rhs.Ref_class() != NULL) {
      ...
      if (WOPT_Enable_Verbose && Tracing()) {
	fprintf(TFile, " -> added to pending.\n");
      }
    } else {
      if (WOPT_Enable_Verbose && Tracing()) {
	fprintf(TFile, " -> skipped as rhs is unclassifiable.\n");
      }
    }
  }
}
...
ALIAS_CLASSIFICATION::Handle_assignment(WN *const stmt)
{
  ...
  if (rhs_class.Ref_class() != NULL) {
    ...
    if (WOPT_Enable_Verbose && Tracing()) {
      fprintf(TFile, "RHS is non pointer constant. Merge skipped.\n");
    }
  }
...
#ifdef TARG_ST
  // [CG] Handle ASM statement
  else if (opc == OPC_ASM_STMT) {
    if (WOPT_Enable_Verbose && Tracing()) {
      fprintf(TFile, "cwnk: Handling asm:\n");
      Dump_wn_tree(TFile, wn);
    }
    return Handle_asm(wn);
  }
#endif
