...
)
{
  ...
  if (MTYPE_signed(t1))
    ...
    case 32: return ((t1 == MTYPE_A4) ? MTYPE_A4 : MTYPE_U4);
    case 40: return MTYPE_U5;
    case 64: return ((t1 == MTYPE_A8) ? MTYPE_A8 : MTYPE_U8);
    default: Is_True(FALSE, ("WN_mtype_from_class_size: unrecognized bit size for mtype"));
    }
  return MTYPE_V;
}
...
#ifdef TARG_ST
  // Arthur: take the POINTER CLASS into account
  // I can not use MTYPE_is_pointer because it means a different thing.
  // in case of a pointer, strip off integer-ness and unsigned-ness
  // TODO: POINTER CLASS shouldn't really be UNSIGNED INTEGER
  if ((mclass & MTYPE_CLASS_POINTER) != 0)
    return MTYPE_CLASS_POINTER;
#endif
...
Ldid_from_mtype( MTYPE mtype )
{
  switch ( mtype ) {
    ...
    case MTYPE_I5:	return OPC_I5I5LDID;
    ...
    case MTYPE_U5:	return OPC_U5U5LDID;
    case MTYPE_U8:	return OPC_U8U8LDID;
    case MTYPE_A4:	return OPC_A4A4LDID;
    case MTYPE_A8:	return OPC_A8A8LDID;
...
Mtype_from_mtype_class_and_size( INT mtype_class, INT bytes )
{
  ...
  // unsigned integer?
  else if ( (mtype_class & MTYPE_CLASS_UNSIGNED) || 
...
Ldid_from_mtype_class_and_size( INT mtype_class, INT bytes )
{
  ...
  if (mtype_class & MTYPE_CLASS_POINTER) {
    switch ( bytes ) {
      case 4: return OPC_A4A4LDID;
      case 8: return OPC_A8A8LDID;
    }
  }
  // unsigned integer?
  else if ( (mtype_class & MTYPE_CLASS_UNSIGNED) || 
...
Stid_from_mtype_class_and_size( INT mtype_class, INT bytes )
{
  ...
  if (mtype_class & MTYPE_CLASS_POINTER) {
    switch ( bytes ) {
      case 4: return OPC_A4STID;
      case 8: return OPC_A8STID;
    }
  }
  // unsigned integer?
  else if ( (mtype_class & MTYPE_CLASS_UNSIGNED) || 
...
#ifdef TARG_ST
    // FdF 20040927: Keep the association of several memory
    // references with a single prefetch instruction.
    WN_MAP_Set(WN_MAP_PREFETCH, wn, pf);
#endif
  }
}
...
{
  Is_True(MTYPE_is_class_integer(result_ty),
          ("Is_hi_sign_extended: handles integral type only"));

#ifdef TARG_ST
  // Arthur: seems like this function returns whether the upper
  //         32 bits of a 64 bit register would be sign-extended
  //         by a LDID/ILOAD. And IA-64 seems to fill it with
  //         0s when loading <= 32 bits of value ? So, they always
  //         say sign-extended for such cases.
  //         Loads on other targets may not behave the same way.
  //         In particular, on ST220 there are no 64-bit loads ...
  //         Try considering over 32-bit loads not sign-extended ?
  //         Under 32 bits depends
  //         on whether the result_ty is signed or not (is
  //         this true ?).
  //
  // TODO: have a flag, something like Load_Sign_Extended, or
  //       whatever to parametrize this. For now just ifdef
  //       will do it
  //
#if 0
  if (MTYPE_size_min(result_ty) == MTYPE_size_min(MTYPE_I4) ||
