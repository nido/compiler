...
CODEREP::Convert_type(CODEMAP *htable, CODEREP *expr, BOOL icopy_phase)
{
...
#if 0//TARG_ST
  fprintf(TFile,"CODEREP::Convert_type \n");
  this->Print(4,TFile);
  fprintf(TFile,"    to\n");
  expr->Print(4,TFile);
#endif
  ...
  case NEED_CVT:
#ifndef TARG_ST
      // [CG]: Unsafe to convert an [UI]4[UI]8CVT into a CVTL
      // Indeed parent of the expression are of type [UI]4 not [UI]8
      // and we don't want implicit conversions of operands.
			    ...
			    BOOL inside_cse, BOOL in_array)
{
...
#if 0//TARG_ST
  fprintf(TFile,"Copy_propagate_cr: \n");
  x->Print(4,TFile);
#endif
  ...
  case CK_VAR: 
    {
      ...
      if (id_cr) {
#ifdef OPEN64_0_16
	// [CG]: Fixes case where the type is not 
	// determined as in H WHIRL for M types.
  ...
  case CK_IVAR: {
    ...
    inside_cse = inside_cse || x->Usecnt() > 1;
#if 0//TARG_ST
      fprintf(TFile,"...call Copy_propagate_cr\n");
#endif
    ...
    if (expr || expr2) { // need rehash
      ...
      if (expr) {
#if 0//TARG_ST
      fprintf(TFile,"CODEREP::x \n");
      x->Print(4,TFile);
      fprintf(TFile,"CODEREP::cr \n");
      cr->Print(4,TFile);
      fprintf(TFile,"CODEREP::expr \n");
      expr->Print(4,TFile);
#endif
