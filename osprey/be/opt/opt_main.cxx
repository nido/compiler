...
#include "W_regex.h"                      // For regcomp and regexec


extern "C" void
Perform_Procedure_Summary_Phase (WN* w, struct DU_MANAGER *du_mgr,
				 /*struct ALIAS_MANAGER*/void *alias_mgr,
				 EMITTER *emitter);
	      ...
	      ALIAS_MANAGER *alias_mgr)
{
  ...
  if (WOPT_Enable_DCE) {
    ...
    // trace with: Get_Trace(TP_GLOBOPT, DCE_DUMP_FLAG)
    if (Get_Trace(TP_WOPT2, CR_DUMP_FLAG)) {
      fprintf(TFile,"%s\t\t After Do_dead_code_elim\n%s\n", DBar, DBar);
      comp_unit->Cfg()->Print(TFile);
    }
  }
#ifdef TARG_ST
  ...
  if (WOPT_Enable_Extra_Rename_Pass > 0) {
    if (Get_Trace(TP_WOPT2, CR_DUMP_FLAG)) {
      fprintf(TFile,"%s\t\t After Rename_CODEMAP passes \n%s\n", 
	      DBar, DBar);
      comp_unit->Cfg()->Print(TFile);
    }
  }
  ...
  if ( phase == MAINOPT_PHASE ) {
    ...
    if (Get_Trace(TP_WOPT3, CR_DUMP_FLAG)) {
      fprintf(TFile,"%s\t\t Before MAINOPT_PHASE\n%s\n", DBar, DBar);
      comp_unit->Opt_stab()->Print(TFile);
      comp_unit->Htable()->Print(TFile);
      comp_unit->Cfg()->Print(TFile);
    } 
    else if (Get_Trace(TP_WOPT2, CR_DUMP_FLAG)) {
      fprintf(TFile,"%s\t\t Before MAINOPT_PHASE\n%s\n", DBar, DBar);
      comp_unit->Opt_stab()->Print(TFile);
      comp_unit->Cfg()->Print(TFile);
    }
    else if (Get_Trace(TP_GLOBOPT, CR_DUMP_FLAG)) {
      fprintf(TFile,"%s\t\t Before MAINOPT_PHASE\n%s\n", DBar, DBar);
      comp_unit->Cfg()->Print(TFile);
    }

#if defined(TARG_IA64) || defined(TARG_ST100) || defined(TARG_ST200) 
    if (WHIRL_Mtype_B_On)
	...
	comp_unit->Do_vnfre(FALSE/*before_epre*/);
      ...
      if (Get_Trace(TP_GLOBOPT, 0x8000)) {
	fprintf(TFile,"%s\t\t After MainOpt: Exp PRE\n%s\n", DBar, DBar);
	comp_unit->Cfg()->Print(TFile);
      }
	...
	comp_unit->Do_local_rvi();
        ...
        if (Get_Trace(TP_WOPT2, CR_DUMP_FLAG)) {
	  fprintf(TFile,"%s\t\t After Do_local_rvi\n\n%s", DBar, DBar);
	  comp_unit->Opt_stab()->Print(TFile);
	  comp_unit->Cfg()->Print(TFile);
        }
	...
	comp_unit->Do_load_pre(WOPT_Enable_Const_PRE /*do_consts*/,
      ...
      if (Get_Trace(TP_GLOBOPT, 0x8000)) {
	fprintf(TFile,"%s\t\t After MainOpt: Load PRE\n%s\n", DBar, DBar);
	comp_unit->Cfg()->Print(TFile);
      }
	...
	comp_unit->Htable()->Verify_var_phi_hash();
      ...
      if (Get_Trace(TP_GLOBOPT, 0x8000)) {
	fprintf(TFile,"%s\t\t After MainOpt: Store PRE\n%s\n", DBar, DBar);
	comp_unit->Cfg()->Print(TFile);
      }
      ...
      if (Get_Trace(TP_GLOBOPT, 0x8000)) {
	fprintf(TFile,"%s\t\t After MainOpt: Bitwise DCE\n%s\n", DBar, DBar);
	comp_unit->Cfg()->Print(TFile);
      }
