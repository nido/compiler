...
inline void topological_sort(Graph& in, Vertex_id root, Container& out)
{
  ...
  typedef cluster_vector<edge, __GNUEXT::select1st<edge> > succ_graph;
  succ_graph g;
  copy(in, g);
  if (root < g.size())
...
typedef cluster_vector<edge, __GNUEXT::select1st<edge> > successor_graph;
typedef cluster_vector<edge, __GNUEXT::select2nd<edge> > predecessor_graph;
