...
#define Alloc_stack_cr(cnt) ((CODEREP*)alloca(sizeof(CODEREP)+(cnt)*sizeof(CODEREP *)))
  ...
  void Init_const(MTYPE wt, INT64 v)
    {
      Init(CK_CONST); 
      // Arthur: I want to use A4/A8.
      // I have changed the Set_dtyp_const_val() so just
      // call it.
      Set_dtyp_const_val(wt, v);
    }
  ...
  void      Set_dtyp(MTYPE dt)        { Is_True(Kind() != CK_CONST,
					...
					_dtyp = dt; }
  // I had to change this to be able to handle the A4/A8 MTYPEs.
  // Tried not to change behavoiur for the IA64 target.
  void	    Set_dtyp_const_val(MTYPE dt, INT64 v) { 
    Is_True(Kind() == CK_CONST, ("CODEREP::Set_dtyp_const_val, illegal kind"));

    // This should clean up higher bits in <=32 bit values and
    // map U4 -> I4 and U8 -> I8 as originally intended for IA64.
    if (MTYPE_size_min(dt) <= 32)
      v = ((v << 32) >> 32);

    if (MTYPE_is_unsigned(dt))
      dt = MTYPE_complement(dt);
