...
Unify_Rty(OPERATOR opr, MTYPE rty)
{
...
#endif
   {
      ...
      case OPR_MPY:
#ifdef TARG_ST
	// [CG]: It seems that this is a bug.
	// We should differenciate
	// unsigned/signed div/rem
	// An example is : (unsigned)x / (unsigned)-1
	// different from: (signed)x / (signed)-1
#else
      case OPR_DIV:
      case OPR_REM:
#endif
...
VN_BINARY_EXPR::simplify(VN *v)
{
   ...
   else if (is_integral)
   {
	 ...
	 if (Is_Literal_Expr(opnd2))
	 {
	   ...
	   if (
#ifdef TARG_ST
	       // [CG] Seems to be a bug.
	       // We must test signedness to differenciate between
	       // signed and unsigned REM/MOD
	       (MTYPE_is_signed(rty) && intconst == -1) ||
#endif 
	       intconst == 1)
	     simplified = Create_Scalar_Literal_From_Int(rty, 0);
	 }
