...
void PF_LG::Gen_Pref_Node (PF_SORTED_REFS* srefs, mINT16 start, mINT16 stop,
  ...
  while (bitvec) {
...
#ifdef TARG_ST
	  if (_myugs->Get_Stride_One_Size() > Cache.LineSize(1))
	    PF_SET_STRIDE_1L (flag, 1);
	  else { 
	    PF_SET_STRIDE(flag);
	    if(Cache.LineSize(1)/_myugs->Get_Stride_One_Size() != prefetch_vec[depth]) {
	      int new_stride = Cache.LineSize(1)/_myugs->Get_Stride_One_Size();
	      DevWarn("Stride for automatic prefetch was set to %d, changed to %d.", prefetch_vec[depth], new_stride);
	...
	INT CacheLineSize;
        if ((level == level_1) || (level == level_1and2))
	  CacheLineSize = Cache.LineSize(1);
	else
	  CacheLineSize = Cache.LineSize(2);

        increment = Stride_Forward() * LNO_Prefetch_Ahead * CacheLineSize;
        ...
        if ((((level == level_1) || (level == level_1and2)) &&
            ...
            increment = stride_size * LNO_Prefetch_Iters_Ahead;
        }

#ifdef TARG_ST
	DO_LOOP_INFO* dli = loopnode->Get_LoopInfo ();
	UINT64 num_iters = Get_Good_Num_Iters(dli);
	// FdF ddts 16752: Do not generate a PREFETCH when the number
	// of iterations of the loop is less or equal to half the
	// stride of the prefetch
	if (num_iters*2 <= PF_GET_STRIDE_1L(flag))
	  return;
	// FdF: Otherwise, do not prefetch ahead of more that half the
	// number of iterations of the loop.
	INT max_increment = (stride_size * num_iters / 2 / CacheLineSize) * CacheLineSize;
	if (absof(increment) > absof(max_increment))
...
#define MAX_LEN 1024
    ...
    for (j=start; j<stop && !drop_prefetch; j++) {
      ...
      if (tmp == NULL) {
        ...
        SET_AUTO(tmp);
        newnode = TRUE;
	VB_PRINT(printf("Gen_pref_node (ref %p, pref %p)\n", parent_ref, pfnode));
      }
      ...
      case level_1:
        ...
        PF_PTR_distance_1L(tmp) = distance;
      ...
      case level_2:
        ...
        PF_PTR_distance_2L(tmp) = distance;
...
#ifdef TARG_ST
PF_UGS *PF_BASE_ARRAY::Get_UGS (WN *ref_addr) {
  PF_UGS *pf_ugs;
  int i, j;
  for (i=0; i<NumUGSs (); i++) {
    pf_ugs = Get_UGS(i);
    for (j = 0; j < pf_ugs->NumREFs(); j++) {
      WN *ugs_ref = pf_ugs->Get_Ref(j);
      if (ugs_ref == ref_addr)
	return pf_ugs;
    }
  }
