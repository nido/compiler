		 ...
		 double*        doverhead_best) // return scalar
{
  ...
  *stripdepth = depth+1;
  ...
  if (Debug_Cache_Model) {
    fprintf(TFile, "%s \t CACHE MODEL (REQUIRED INNER LOOP=%d, ", DBar, required_inner);
    fprintf(TFile, "LOOP OVERHEAD=%d) \n%s", Loop_Overhead, DBar); 
  ...
  for (i = 0; i <= depth; i++) {
    ...
    if (exact_iters != -1 && est != exact_iters) {
      DevWarn("Loop at depth %d -- Est_Num_Iterations=%lld, expected %lld",
                                 i, dli->Est_Num_Iterations, exact_iters);
      est = exact_iters;
    }
