...
#ifdef TARG_ST
static void Unfold_Prefetch(WN *wn, WN *array);
#endif
  ...
  if (opcode == OPC_BLOCK) {
    ...
    *inner_depth=0;
	...
	// FdF 20060420: Fix for ddts 25220.
      ...
      // PREFETCH.
      else if (WN_opcode(kid) == OPC_PREFETCH) {
	extern BOOL PU_has_manual_prefetch;
	PU_has_manual_prefetch = TRUE;
	WN_pf_set_confidence(kid, 3);	  
      }
#endif
      WN *prev_kid = WN_prev(kid);
...
#ifdef TARG_ST
  } else if (OPCODE_is_load(opcode) || OPCODE_is_store(opcode) || oper == OPR_PREFETCH) {
#else
...
#ifdef TARG_ST
      if (promote_pointers && (oper == OPR_PREFETCH || dlistack->Elements())) {
#else
...
#ifdef TARG_ST
	else if ((oper == OPR_PREFETCH) && WN_operator(WN_kid0(wn)) != OPR_ARRAY) {
          Promote_Pointer(wn,0,0);
	}
#endif
      }
...
#ifdef TARG_ST
	  (WN_pragma(wn) == WN_PRAGMA_LOOPDEP) ||
#endif
...
#ifdef TARG_ST
	  // FdF 10/05/2004: Keep it also on non-inner loops, it will
	  // be used by the CG.
	  dli->Is_Ivdep = IVDEP_DEFAULT;
...
#endif
	} else if (WN_pragma(wn) == WN_PRAGMA_LOOPDEP) {
#ifndef TARG_ST
          if (!dli->Is_Inner) {
	    ErrMsgSrcpos(EC_LNO_Generic,WN_Get_Linenum(wn),
		"LOOPDEP on non-inner loop ignored");
	  } else
#endif
	  {

	    switch (WN_pragma_arg1(wn)) {
	    case LOOPDEP_VECTOR: dli->Is_Ivdep = IVDEP_VECTOR; break;
	    case LOOPDEP_PARALLEL: dli->Is_Ivdep = IVDEP_PARALLEL; break;
	    case LOOPDEP_LIBERAL: dli->Is_Ivdep = IVDEP_LIBERAL; break;
	    default: FmtAssert(FALSE, ("Wrong argument to pragma LOOPDEP"));
	    }
	    LWN_Delete_From_Block(LWN_Get_Parent(wn),wn);
	  }
...
static void Promote_Pointer(WN *wn, INT kid_num, INT load_size)
{
...
#ifdef TARG_ST
    } else if (OPCODE_is_prefetch(opcode)) {
      if ((addr_oper == OPR_LDID) || (addr_oper == OPR_LDA)) {
	switch (OPCODE_desc(WN_opcode(addr))) {
        case MTYPE_I1 : case MTYPE_U1: load_size=1; break;
        case MTYPE_I2 : case MTYPE_U2: load_size =2; break;
        case MTYPE_I4 : case MTYPE_U4: case MTYPE_F4: load_size=4; break;
        case MTYPE_I8 : case MTYPE_U8: case MTYPE_F8: case MTYPE_C4:
...
#ifdef TARG_ST
  if (OPCODE_is_load(opcode) || OPCODE_is_store(opcode) || opcode == OPC_PREFETCH) {
#else
  if (OPCODE_is_load(opcode) || OPCODE_is_store(opcode)) {
#endif
   if ((addr_oper == OPR_LDID) || (addr_oper == OPR_LDA)) {
  ...
  // check that we're multiplying by a multiple of the size of the element
#ifdef TARG_ST
  if (opcode != OPC_PREFETCH)
#endif
