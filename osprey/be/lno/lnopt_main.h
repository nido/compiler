...
//
//              LOOPMODE pragma seen for this loop. Default is
//              (1,0). Values give property on the modulus and residue
...
#ifdef TARG_ST
  enum DLI_IVDEP { IVDEP_NONE=0, IVDEP_DEFAULT, IVDEP_VECTOR, IVDEP_PARALLEL, IVDEP_LIBERAL};
#endif
...
inline BOOL Do_Loop_Is_Ivdep(WN *wn)
{
  DO_LOOP_INFO *dli = Get_Do_Loop_Info(wn);
#ifdef TARG_ST
  // FdF 10/05/2004: Check dli->Is_inner because pragmas are now kept
  // also on non-inner loops.
  return(dli->Is_Inner && dli->Is_Ivdep != IVDEP_NONE);
#else
  return(dli->Is_Ivdep);
#endif
}
...
#ifdef TARG_ST
inline BOOL Do_Loop_Is_Ivdep_default(WN *wn)
{
  DO_LOOP_INFO *dli = Get_Do_Loop_Info(wn);
  return(dli->Is_Ivdep == IVDEP_DEFAULT);
}

inline BOOL Do_Loop_Is_Ivdep_parallel(WN *wn)
{
  DO_LOOP_INFO *dli = Get_Do_Loop_Info(wn);
  return(dli->Is_Ivdep == IVDEP_PARALLEL);
}

inline BOOL Do_Loop_Is_Ivdep_vector(WN *wn)
{
  DO_LOOP_INFO *dli = Get_Do_Loop_Info(wn);
  return(dli->Is_Ivdep == IVDEP_VECTOR);
}

inline BOOL Do_Loop_Is_Ivdep_liberal(WN *wn)
{
  DO_LOOP_INFO *dli = Get_Do_Loop_Info(wn);
  return(dli->Is_Ivdep == IVDEP_LIBERAL);
}
#endif
