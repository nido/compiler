...
#ifdef TARG_ST
  if (iters < 0) {
    DO_LOOP_INFO* dli = Get_Do_Loop_Info(outerloop);
    mINT16 new_modulus = 1, new_residue = 0;
    if (dli->Pragma_Modulus % u == 0) {
      iters = dli->Pragma_Residue;

    // And update the Modulus and Residue value to take into account
    // this unrolling.

      new_modulus = dli->Pragma_Modulus / u;
      new_residue = dli->Pragma_Residue / u;
    }

    dli->Pragma_Modulus = new_modulus;
    dli->Pragma_Residue = new_residue;
  }
#endif
