				...
				BOOL Hoist_Bounds)
{
  ...
  MEM_POOL *pool=dli->Pool();
  {
...
#ifdef TARG_ST
    /* FdF: Be careful when step or one or the other bound is a
       negative value and "IV is unsigned".
       (Fixes for (unsigned i=-5;i<2;i++)) */
    if ((step->Const_Offset < 0) && MTYPE_is_unsigned(WN_rtype(s))) {
      step->Too_Messy = TRUE;
      goto return_point;
    }
#endif
...
#ifdef TARG_ST
    /*  FdF: Fixes Unsigned IV with negative value. */
    if ((WN_operator(WN_kid(l,0)) == OPR_INTCONST) && 
	(WN_const_val(WN_kid(l,0)) < 0) &&
	MTYPE_is_unsigned(WN_desc(l))) {
    ...
    /* FdF: Unsigned IV with negative value is apparently correctly
       handled for upper bound. */
