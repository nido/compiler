	...
	break;
     ...
     case WN_PRAGMA_UNROLL:
      ...
      if (next_non_prag_stid == NULL ||
          WN_opcode(next_non_prag_stid) != OPC_DO_LOOP) {
#ifdef TARG_ST
	// FdF 10/05/2004: If there is no Do_Loop_Info to attach the
	// pragma, keep it in the code. CG will be able in most cases
	// to attach it to the corresponding loop.
	remove = FALSE;
#else
	if (!FF_Pragma_Seen_Before(wn))
	  ErrMsgSrcpos(EC_LNO_Bad_Pragma_String, WN_Get_Linenum(wn),
...
#ifdef TARG_ST
	Get_Do_Loop_Info(next_non_prag_stid)->Required_Unroll = 1;
#else
     ...
     case WN_PRAGMA_LOOPMOD:
      ...
      if (next_non_prag_stid == NULL ||
          WN_opcode(next_non_prag_stid) != OPC_DO_LOOP) {
	// FdF 10/05/2004: Keep the pragma in the WHIRL in this case if there is no Do_Loop_Info to attach it.
	remove = FALSE;
	/*
	  if (!FF_Pragma_Seen_Before(wn))
	  ErrMsgSrcpos(EC_LNO_Bad_Pragma_String, WN_Get_Linenum(wn),
