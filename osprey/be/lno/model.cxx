                  ...
                  HASH_TABLE<WN*,BIT_VECTOR*>* invar_table)
{
  ...
  if (debug_model) {
    fprintf(TFile,"%s\t\t\t Modeling an SNL\n%s", DBar, DBar);
  ...
  if (_OP_resource_count) {
    if (debug_model) {
      fprintf(TFile, "OP_issue: %f\n", _OP_issue);
      fprintf(TFile, "OP_Resource_Count: ");
      TI_RES_COUNT_Print(TFile, _OP_resource_count);
      fprintf(TFile, "\n");
      fprintf(TFile, "Optimal number of cycles for each unroll degree: \n");
    }
    ...
    for (i = 0; i < Max_Unroll_Prod; i++) {
      ...
      if (debug_model) {
	fprintf(TFile, "\t _loop_rcycles_unroll_by[%d] = %f\n", i,
		_loop_rcycles_unroll_by[i]);
      }
...
)
{
...
#ifdef TARG_ST
  _num_ptr_regs_inner = Target_PTRs + 1;
  _num_ptr_refs_inner = 0;
  _num_bool_regs_inner = Target_BRs + 1;
  _num_bool_refs_inner = 0;
#endif
...
#ifdef TARG_ST
      && _base_ptr_regs + _scalar_ptr_regs <= Target_PTRs
      && _base_bool_regs + _scalar_bool_regs <= Target_BRs
#endif
      ) {
  ...
  Cache_Model(arl_for_cache, num_loops-1, inner,
              ...
              _num_cycles_inner, 
#ifdef TARG_ST
              _num_fp_refs_inner+_num_int_refs_inner+_num_ptr_refs_inner+_num_bool_refs_inner,
#else
              _num_fp_refs_inner+_num_int_refs_inner,
#endif
	      _new_order_inner,
  ...
  if (_num_cycles == -1.0 || 
      ...
      (_num_cycles_inner == _num_cycles &&
       // Arthur: This seems to be a bug here ??
#ifdef TARG_ST
       (_num_int_regs_inner+_num_fp_regs_inner+
                          _num_ptr_regs_inner+_num_bool_regs_inner < 
                   _num_int_regs+_num_fp_regs+_num_ptr_regs+_num_bool_regs ||
#else
...
#ifdef TARG_ST
    _num_ptr_regs = _num_ptr_regs_inner;
    _num_ptr_refs = _num_ptr_refs_inner;
    _num_bool_regs = _num_bool_regs_inner;
    _num_bool_refs = _num_bool_refs_inner;
#endif
                       ...
                       ARRAY_REF* arl)
{
  if (l >= num_loops) { // base case of the recursion
    ...
    while (can_allocate &&
      ...
      if (debug_model) {
	fprintf(TFile, "\n");
	fprintf(TFile, "--- Unrolling loop %d (inner %d) %d times\n", l, inner, u);
...
#define MINVAR_MAGIC_COEFF (0.20 * _loop_rcycles_unroll_by[Max_Unroll_Prod-1])
  ...
  *can_reg_allocate = TRUE;
  ...
  if (debug_model) {
    fprintf(TFile, "MEM_rcycles = %.2f / %.2f: %.2f\n", MEM_issue, _num_mem_units, MEM_rcycles);
    fprintf(TFile, "MEM_rcycles_minus_spills: %.2f\n", MEM_rcycles_minus_spills);
  ...
  if (debug_model) {
    fprintf(TFile, "issue_limit: %.2f\n", issue_limit);
    fprintf(TFile, "issue_limit_minus_spills: %.2f\n", issue_limit_minus_spills);
    fprintf(TFile, "ideal_resource_cycles: %.2f\n", ideal_resource_cycles);
    fprintf(TFile, "resource_cycles: %.2f\n", resource_cycles);
    fprintf(TFile, "resource_cycles_minus_spills: %.2f\n", resource_cycles_minus_spills);
  }
                           ...
                           HASH_TABLE<WN*,BIT_VECTOR*>* invar_table)
{
  ...
  if (oper == OPR_CVT   || 
    ...
    // Arthur: I believe that considering PTR, BOOL, etc. MTYPEs
    //         I should consider the MTYPE in the target-specific 
    //         part ... I'll need to pass the MTYPE to all these
    //         routines eventually ...
#ifdef TARG_ST
    else {
#else
...
#ifdef TARG_ST
	    // on ST100 I need to see the mtype to compute resources.
	    // TODO: this would also work for the IA64.
	    *num_instr += LNOTARGET_Int_Mult_Res(resource_count, rtype);
#else
        ...
        case OPR_ADD: 
#ifdef TARG_ST
          *num_instr += LNOTARGET_Int_Add_Res(resource_count, rtype);
#else
        ...
        case OPR_SUB: 
#ifdef TARG_ST
          *num_instr += LNOTARGET_Int_Sub_Res(resource_count, rtype);
#else
        ...
        case OPR_CIOR: 
          *num_instr += LNOTARGET_Int_Cior_Res(resource_count, rtype);
          break;
        case OPR_SHL: 
          *num_instr += LNOTARGET_Int_Shl_Res(resource_count, rtype); 
          break;
        case OPR_ASHR: 
          *num_instr += LNOTARGET_Int_Ashr_Res(resource_count, rtype); 
          break;
#else
        case OPR_LAND: 
...
#ifdef TARG_ST
        case OPR_EQ:  
          *num_instr += LNOTARGET_Int_Eq_Res(resource_count, rtype); 
          break;
        case OPR_NE:  
          *num_instr += LNOTARGET_Int_Ne_Res(resource_count, rtype); 
          break;
        case OPR_GT:  
          *num_instr += LNOTARGET_Int_Gt_Res(resource_count, rtype); 
          break;
        case OPR_GE: 
          *num_instr += LNOTARGET_Int_Ge_Res(resource_count, rtype); 
          break;
        case OPR_LT: 
          *num_instr += LNOTARGET_Int_Lt_Res(resource_count, rtype); 
          break;
        case OPR_LE: 
          *num_instr += LNOTARGET_Int_Le_Res(resource_count, rtype); 
          break;
#else
        case OPR_EQ:  
...
) : _stack(pool)
{
  ...
  _num_bad_ptr = orig->_num_bad_ptr;
  _num_bad_bool = orig->_num_bad_bool;
#else
  _num_bad_fp = orig->_num_bad_fp;
  _num_bad_int = orig->_num_bad_int;
#endif

  // Copy the stack of array references:
...
{
#ifdef TARG_ST
  fprintf(fp,"The number of bad references is: \n");
  fprintf(fp,"\t %d fp,\n", Num_Fp_Bad());
  fprintf(fp,"\t %d int,\n", Num_Int_Bad());
  fprintf(fp,"\t %d ptr,\n", Num_Ptr_Bad());
  fprintf(fp,"\t %d bool\n", Num_Bool_Bad());
...
ARRAY_REF::Unroll(INT loop_no, INT num_copies)
{
  ...
  _num_bad_ptr *= num_copies;
  _num_bad_bool *= num_copies;
  for (INT i=0; i<Elements(); i++) {
...
)
{
  ...
  if (debug_model) {
    fprintf(TFile, "build_rec: ");
    Dump_WN (wn, TFile, 0, 0, 0);
  }
  ...
  if (OPCODE_is_load(opcode) && opr != OPR_LDID) {
    if (WN_operator(WN_kid0(wn)) == OPR_ARRAY) {
      ...
      if (MTYPE_is_float(WN_desc(wn)))
        _num_bad_fp++;
      else if (MTYPE_is_class_pointer(WN_desc(wn)))
	_num_bad_ptr++;
      else if (MTYPE_is_class_boolean(WN_desc(wn)))
	_num_bad_bool++;
      else
        _num_bad_int++;
#else
...
)
{
  ...
  if (debug_model) {
    fprintf(TFile, "build_array: ");
    Dump_WN (wn_array, TFile, 0, 0, 0);
  }
  ...
  if (!array || array->Too_Messy ||
    ...
    if (MTYPE_is_float(type))
      _num_bad_fp++;
    else if (MTYPE_is_class_pointer(type))
      _num_bad_ptr++;
    else if (MTYPE_is_class_boolean(type))
      _num_bad_bool++;
    else
      _num_bad_int++;
#else
...
#endif
    if (debug_model) {
      fprintf(TFile,"  array reference not good \n");
    }
...
#endif
      if (debug_model) {
	fprintf(TFile,"  array dim %d expression not good \n", i);
      }
...
#ifdef TARG_ST
  // Arthur: must handle multidimensional arrays while gcc does
  //         not support them !
  if (array->Num_Vec() != WN_num_dim(wn_array)) {
...
#endif
    if (debug_model) {
      fprintf(TFile,"  array base not good \n");
    }
  ...
  if (debug_model) {
    fprintf(TFile,"  appending array reference: \n");
    arn->Print(TFile);
  }
...
/* ====================================================================
 *   LAT_DIRECTED_GRAPH16::Add_Vertices_Op_Edges
 *
 *   Walk the code
 ...
 *   Return -1 on failure
 *
 *   Arthur: I am trying to consider integer accesses also.
 * ====================================================================
 */
INT 
LAT_DIRECTED_GRAPH16::Add_Vertices_Op_Edges (
...
/* ====================================================================
 *   LAT_DIRECTED_GRAPH16::Add_Vertices_Op_Edges_Rec
 *
...
)
{
  ...
  if (oper == OPR_CVT   || 
...
#ifdef TARG_ST
    // Arthur: I guess, the integer OPs are not considered as
    //         they are used in address calculation, don't really
    //         know. I'll just try to enable the DU latency
    //         calculations for the ST100
    //         The MTYPE is passed to the latency calculating
    //         routine.
    //  It seems that only some types of WNs are analysed for
    //  latency determination in the loop. These correspond to
    //  the long latency FP operations on MIPS and IA64 targets.
    //  Apparently, on those targets the inetger operations are
    //  not considered long latency enough to use them in latency
    //  calculations. All in all, the usefull WNs seem to be:
    //     OPR_ADD
    //     OPR_SUB
    //     OPR_MPY
    //     OPR_MAX
    ...
    //     etc.
    //
    //  On the ST100, the long latency DU operations will be 
    //  copya and copyd (CVT I guess in WHIRL), and also perhaps
    //  some of the things that expand in a sequence of OPs
    //  later, eg. mult. So I'll try to do it this way ...
    else {
      // multiply-adds
      if (Target_ISA_Has_Madd() && 
          (oper == OPR_ADD || oper == OPR_SUB) && 
        ...
        op_latency = LNOTARGET_Max_Lat(rtype);
      } 
      else if (oper == OPR_MIN) {
        op_latency = LNOTARGET_Min_Lat(rtype);
      } 
      else if (oper == OPR_SQRT) {
        op_latency = LNOTARGET_Sqrt_Lat(rtype);
      } 
      else if (oper == OPR_DIV) {
        op_latency = LNOTARGET_Div_Lat(rtype);
      } 
      else if (oper == OPR_RECIP) {
        op_latency = LNOTARGET_Recip_Lat(rtype);
      } 
      else if (oper == OPR_RSQRT) {
        op_latency = LNOTARGET_Rsqrt_Lat(rtype);
      } 
      else if ((top = WHIRL_To_TOP(wn)) != TOP_UNDEFINED) {
	op_latency = LNOTARGET_Top_Latency(top);
      }
      else {
	// This function is based on the WN_opcode rather that on
	// TOP because for some OPCODEs there may be several TOPs
	// needed, eg. I8 type on the ST100. But I still would like
	// to try the transformation.
	return LNOTARGET_Opcode_Lat(WN_opcode(wn));
      }
    }
#endif
...
/* ====================================================================
 *   LAT_DIRECTED_GRAPH16::LNO_Latency_Madd
 *
 *   deal with a madd
 * ====================================================================
 */
...
{
  TYPE_ID rtype = WN_rtype(wn);
  INT add_op_latency = LNOTARGET_Madd_Add_Lat(rtype);
  INT mult_op_latency = LNOTARGET_Madd_Mult_Lat(rtype);
...
/* ====================================================================
 *   LAT_DIRECTED_GRAPH16::LNO_Latency_Intrinsic
 *
...
)
{
  ...
  return LNOTARGET_Exp_Lat((INTRINSIC) WN_intrinsic(wn), num_multiplies);
}
...
/* ====================================================================
 *   LOOP_MODEL::Unique_Unstored_Ptr_Scalar_Refs
 * ====================================================================
 */
INT 
LOOP_MODEL::Unique_Unstored_Ptr_Scalar_Refs (
...
)
{
  ...
  SYMBOL_TREE *symbol_tree = CXX_NEW(SYMBOL_TREE(
	/*is_floating_point*/FALSE,&LNO_local_pool), &LNO_local_pool);
  INT outer=0;
  _num_ptr_scalar_refs = 0;
  while (!_can_be_inner[outer]) outer++;
  symbol_tree->Initialize_Innermost_Loop_Var_Symbol(wn);
  symbol_tree->Enter_Scalar_Refs(REF_POINTER,wn,ar,
             pi,_can_be_inner,_num_loops,outer,&_num_int_scalar_refs);
  INT result = symbol_tree->Num_Int_Unstored();
  CXX_DELETE(symbol_tree,&LNO_local_pool);
  MEM_POOL_Pop(&LNO_local_pool);
  return result;
...
/* ====================================================================
 *   LOOP_MODEL::Unique_Unstored_Bool_Scalar_Refs
 * ====================================================================
 */
INT 
LOOP_MODEL::Unique_Unstored_Bool_Scalar_Refs (
...
)
{
  ...
  while (!_can_be_inner[outer]) outer++;
  symbol_tree->Initialize_Innermost_Loop_Var_Symbol(wn);
  symbol_tree->Enter_Scalar_Refs(REF_BOOLEAN,wn,ar,pi,
                _can_be_inner,_num_loops,outer,&_num_int_scalar_refs);
  INT result = symbol_tree->Num_Int_Unstored();
  CXX_DELETE(symbol_tree,&LNO_local_pool);
  MEM_POOL_Pop(&LNO_local_pool);
  return result;
...
/* ====================================================================
 *   SYMBOL_TREE::Integer_Ref_Needs_Reg
 * ====================================================================
 */
BOOL 
SYMBOL_TREE::Integer_Ref_Needs_Reg (
...
INT SYMBOL_TREE_NODE::Num_Int_Unstored() const
{
  INT result=0;
#ifdef TARG_ST
  if (!_is_store && MTYPE_is_class_integer(_symbol.Type))
#else
  if (!_is_store && !MTYPE_float(_symbol.Type))
#endif
	result+=_weight;
...
/* ====================================================================
 *   SYMBOL_TREE_NODE::Num_Ptr_Unstored
 *
 *   Return the sum of each unstored PTR node times its weight 
 * ====================================================================
 */
INT 
SYMBOL_TREE_NODE::Num_Ptr_Unstored() const
{
  INT result=0;
  if (!_is_store && MTYPE_is_class_pointer(_symbol.Type))
    result+=_weight;
  if (_left) result += _left->Num_Ptr_Unstored();
  if (_right) result += _right->Num_Ptr_Unstored();
  return result;
}

/* ====================================================================
 *   SYMBOL_TREE_NODE::Num_Bool_Unstored
 *
 *   Return the sum of each unstored BOOL node times its weight 
 * ====================================================================
 */
INT 
SYMBOL_TREE_NODE::Num_Bool_Unstored() const
{
  INT result=0;
  if (!_is_store && MTYPE_is_class_boolean(_symbol.Type))
    result+=_weight;
  if (_left) result += _left->Num_Bool_Unstored();
  if (_right) result += _right->Num_Bool_Unstored();
  return result;
}
                         ...
                         INT* tlb_out)
{
  ...
  debug_model = Get_Trace(TP_LNOPT, TT_LNO_MODEL);
  if (debug_model) {
    fprintf(TFile,"%s\t\t\t Register Model For\n%s", DBar, DBar);
    extern void fdump_tree(FILE *, WN *);
    fdump_tree (TFile, inner);
    fprintf(TFile,"%s", DBar);
  }
  */
