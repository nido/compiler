...
#if defined(TARG_ST) && defined(__linux__)
//
// Arthur: this drags in a bunch of stuff, in particular tn.h, etc.
//         I prefer declare a couple of routines extern rather
//
...
// **************** symbols defined in lai.so ****************
/*
#ifdef __linux__

extern void (*LAI_Process_Command_Line_p) (INT, char **, INT, char **);
#define LAI_Process_Command_Line (*LAI_Process_Command_Line_p)

extern void (*LAI_Init_p) ();
#define LAI_Init (*LAI_Init_p)

extern void (*LAI_Fini_p) ();
#define LAI_Fini (*LAI_Fini_p)

extern void (*LAI_PU_Initialize_p) (WN*);
#define LAI_PU_Initialize (*LAI_PU_Initialize_p)

extern void (*LAI_PU_Finalize_p) ();
#define LAI_PU_Finalize (*LAI_PU_Finalize_p)

extern WN* (*LAI_Emit_Code_p) (WN*, ALIAS_MANAGER*, DST_IDX, BOOL);
#define LAI_Emit_Code (*LAI_Emit_Code_p)

#else

#pragma weak LAI_Process_Command_Line
#pragma weak LAI_Init
#pragma weak LAI_Fini
#pragma weak LAI_PU_Finalize
#pragma weak LAI_PU_Initialize
#pragma weak LAI_Emit_Code
...
BE_EXPORTED extern void (*wopt_main_p) (INT argc, char **argv, INT, char **);
#define wopt_main (*wopt_main_p)

BE_EXPORTED extern void (*Wopt_Init_p) ();
#define Wopt_Init (*Wopt_Init_p)

BE_EXPORTED extern void (*Wopt_Fini_p) ();
#define Wopt_Fini (*Wopt_Fini_p)

BE_EXPORTED extern WN* (*Perform_Preopt_Optimization_p) (WN *, WN *);
#define Perform_Preopt_Optimization (*Perform_Preopt_Optimization_p)

BE_EXPORTED extern WN* (*Perform_Global_Optimization_p) (WN *, WN *, ALIAS_MANAGER *);
#define Perform_Global_Optimization (*Perform_Global_Optimization_p)

BE_EXPORTED extern WN* (*Pre_Optimizer_p) (INT32, WN*, DU_MANAGER*, ALIAS_MANAGER*);
#define Pre_Optimizer (*Pre_Optimizer_p)

BE_EXPORTED extern DU_MANAGER* (*Create_Du_Manager_p) (MEM_POOL *);
#define Create_Du_Manager (*Create_Du_Manager_p)

BE_EXPORTED extern void (*Delete_Du_Manager_p) (DU_MANAGER *, MEM_POOL *);
#define Delete_Du_Manager (*Delete_Du_Manager_p)

BE_EXPORTED extern BOOL (*Verify_alias_p) (ALIAS_MANAGER *, WN *);
...
BE_EXPORTED extern void (*lno_main_p) (INT, char**, INT, char**);
#define lno_main (*lno_main_p)

BE_EXPORTED extern void (*Lno_Init_p) ();
#define Lno_Init (*Lno_Init_p)

BE_EXPORTED extern void (*Lno_Fini_p) ();
#define Lno_Fini (*Lno_Fini_p)

BE_EXPORTED extern WN* (*Perform_Loop_Nest_Optimization_p) (PU_Info*, WN*, WN*, BOOL);
...
BE_EXPORTED extern void (*Ipl_Extra_Output_p) (Output_File *);
#define Ipl_Extra_Output (*Ipl_Extra_Output_p)

BE_EXPORTED extern void (*Ipl_Init_p) ();
#define Ipl_Init (*Ipl_Init_p)

BE_EXPORTED extern void (*Ipl_Fini_p) ();
#define Ipl_Fini (*Ipl_Fini_p)

BE_EXPORTED extern void (*ipl_main_p) (INT, char **);
#define ipl_main (*ipl_main_p)

BE_EXPORTED extern void (*Perform_Procedure_Summary_Phase_p) (WN*, DU_MANAGER*,
						  ALIAS_MANAGER*, void*);
...
BE_EXPORTED extern void (*Prp_Process_Command_Line_p) (INT, char **, INT, char **);
#define Prp_Process_Command_Line (*Prp_Process_Command_Line_p)
BE_EXPORTED extern BOOL (*Prp_Needs_Whirl2c_p) ();
#define Prp_Needs_Whirl2c (*Prp_Needs_Whirl2c_p)
BE_EXPORTED extern BOOL (*Prp_Needs_Whirl2f_p) ();
#define Prp_Needs_Whirl2f (*Prp_Needs_Whirl2f_p)
BE_EXPORTED extern void (*Prp_Init_p) ();
#define Prp_Init (*Prp_Init_p)
BE_EXPORTED extern void (*Prp_Instrument_And_EmitSrc_p) (WN *pu);
#define Prp_Instrument_And_EmitSrc (*Prp_Instrument_And_EmitSrc_p)
BE_EXPORTED extern void (*Prp_Fini_p) ();
#define Prp_Fini (*Prp_Fini_p)


BE_EXPORTED extern void       (* Anl_Process_Command_Line_p) (INT, char **, INT, char **);
#define Anl_Process_Command_Line (*Anl_Process_Command_Line_p)
BE_EXPORTED extern BOOL        (*Anl_Needs_Whirl2c_p) ();
#define Anl_Needs_Whirl2c (*Anl_Needs_Whirl2c_p)
BE_EXPORTED extern BOOL        (*Anl_Needs_Whirl2f_p) ();
#define Anl_Needs_Whirl2f (*Anl_Needs_Whirl2f_p)
BE_EXPORTED extern void        (*Anl_Init_p) ();
#define Anl_Init (*Anl_Init_p)
BE_EXPORTED extern WN_MAP	    (*Anl_Init_Map_p) (MEM_POOL *id_map_pool); 
#define Anl_Init_Map (*Anl_Init_Map_p)
BE_EXPORTED extern void        (*Anl_Static_Analysis_p) (WN *pu, WN_MAP id_map);
#define Anl_Static_Analysis (*Anl_Static_Analysis_p)
BE_EXPORTED extern void        (*Anl_Fini_p) ();
#define Anl_Fini (*Anl_Fini_p)
BE_EXPORTED extern void        (*Anl_Cleanup_p) ();
#define Anl_Cleanup (*Anl_Cleanup_p)
...
// [HK]
#if __GNUC__ >= 3
extern void Prompf_Emit_Whirl_to_Source(PU_Info* , WN* ) __attribute__ ((weak));
#else
extern void Prompf_Emit_Whirl_to_Source(PU_Info* current_pu, WN* func_nd);
#endif // __GNUC__ >= 3
#else
BE_EXPORTED extern void (*Prompf_Emit_Whirl_to_Source_p) (PU_Info*, WN*);
#define Prompf_Emit_Whirl_to_Source (*Prompf_Emit_Whirl_to_Source_p)
#endif
...
BE_EXPORTED extern BOOL   Prompf_anl_loaded; /* Defined in cleanup.c */
BE_EXPORTED extern BOOL   Purple_loaded;     /* Defined in cleanup.c */
BE_EXPORTED extern BOOL   Whirl2f_loaded;    /* Defined in cleanup.c */
BE_EXPORTED extern BOOL   Whirl2c_loaded;    /* Defined in cleanup.c */
...
/* dso handlers */
static void* cg_handler = NULL;
static void* targinfo_handler = NULL;
static void* wopt_handler = NULL;
static void* ipl_handler = NULL;
static void* lno_handler = NULL;
static void* prompf_anl_handler = NULL;
static void* purple_handler = NULL;
static void* whirl2c_handler = NULL;
static void* whirl2f_handler = NULL;

static void
close_components ()
{
    close_so(cg_handler);
    close_so(targinfo_handler);
    close_so(wopt_handler);
    close_so(ipl_handler);
    close_so(lno_handler);
    close_so(prompf_anl_handler);
    close_so(purple_handler);
    close_so(whirl2c_handler);
    close_so(whirl2f_handler);
}
...
load_components (INT argc, char **argv)
{
    ...
    if (Run_cg) {
      ...
      cg_handler = load_so ("cg"SO_EXT, CG_Path, Show_Progress);
    ...
    if (Run_wopt || Run_preopt || Run_lno || Run_autopar) {
      ...
      wopt_handler = load_so ("wopt"SO_EXT, WOPT_Path, Show_Progress);
    ...
    if (Run_ipl) {
      ...
      ipl_handler = load_so ("ipl"SO_EXT, Ipl_Path, Show_Progress);
    ...
    if (Run_lno || Run_autopar) {
      ...
      lno_handler = load_so ("lno"SO_EXT, LNO_Path, Show_Progress);
	  ...
	  load_so("ipl"SO_EXT, Ipl_Path, Show_Progress);
      }
  }

    if (Run_prompf || Run_w2fc_early) {
      Get_Phase_Args (PHASE_PROMPF, &phase_argc, &phase_argv);
      prompf_anl_handler = load_so("prompf_anl"SO_EXT, Prompf_Anl_Path, Show_Progress);
    ...
    if (Run_purple) {
      ...
      purple_handler = load_so("purple"SO_EXT, Purple_Path, Show_Progress);
      Purple_loaded = TRUE;
	...
	(Run_purple && Prp_Needs_Whirl2c()))
    {
      ...
      whirl2c_handler = load_so("whirl2c"SO_EXT, W2C_Path, Show_Progress);
      Whirl2c_loaded = TRUE;
      if (Run_prompf)
	...
	(Run_purple && Prp_Needs_Whirl2f()))
    {
      ...
      whirl2f_handler = load_so("whirl2f"SO_EXT, W2F_Path, Show_Progress);
      Whirl2f_loaded = TRUE;
      if (Run_prompf)
...
#ifdef TARG_ST
    // TB: coverage init
    if (Profile_Arcs_Enabled || Test_Coverage_Enabled || Branch_Probabilities)
      {
	wn_coverage_init();
      }
#endif
...
#ifdef TARG_ST
    // [CG] We disable mostly all if the PU is an asm function
    // used to wrap-up file scope asm statement.
...
static BOOL has_asm (WN* wn)
{
  ...
  if (opr==OPR_ASM_STMT) return TRUE;

  if (opr==OPR_BLOCK)
    for (WN* stmt=WN_first(wn); stmt; stmt=WN_next(stmt)) {
      if (has_asm (stmt)) return TRUE;
    }
  else if (opr != OPR_IO) {
    for (INT i=0; i<WN_kid_count(wn); i++) {
      if (has_asm(WN_kid(wn,i))) return TRUE;
    }
  }
...
#ifdef TARG_ST
      /* TB: Add coverage here. */
      if (!Is_Set_PU_Info_flags(current_pu, PU_IS_PROFILER) && 
	  (Profile_Arcs_Enabled || Test_Coverage_Enabled || Branch_Probabilities)) {
	// in -O0 and -O1: lower control flow because not already done
...
#ifdef TARG_ST
	//
	// Arthur: we need to do the last thing -- split all the
	//         longlongs and doubles
	//
	if (Only_32_Bit_Ops) {
          LOWER_ACTIONS actions =
            ...
            LOWER_PARITY_MAPS |
            LOWER_ALIAS_MAPS;
	  WN_Lower(rwn, actions, alias_mgr, "Lowering HILO");
	}
#endif
        WB_LWR_Terminate();
	if (Get_Trace(TP_REGION,TT_REGION_ALL)) {
...
extern "C" {
  BE_EXPORTED extern void Process_Fill_Align_Pragmas (WN* func_wn);
  BE_EXPORTED extern void Rewrite_Pragmas_On_Structs (WN* block_wn, WN* wn);
}
...
#ifdef TARG_ST
    // [CG]: Lowering of promoted params
    // Must be done before RT_LOWER and LOWER_ENTRY_EXIT in LOWER_TO_CG
    // because it may generate type conversions (handled in RT_LOWER) and
...
#ifdef TARG_ST
    if (!found) {
      fprintf(stderr, "Warning: found no feedback file named %s in dir: %s\n", prefix, path);
    }
    } else {
      fprintf(stderr, "Warning: unable to open dir: %s\n", path);
    }
#endif
  }
...
// from the main executable rather than one from the shared library).
extern "C" {
    void trace_wn ( WN* wn ) {
        dump_tree(wn);
    }
...
#if defined(__CYGWIN__) || defined(__MINGW32__)
  #define setlinebuf(fp) (setvbuf(fp, (char*)NULL, _IOLBF, (size_t)0))
#endif
  setlinebuf (stdout);
  setlinebuf (stderr);
  Handle_Signals ();
...
#ifdef TARG_ST
  // TB: coverage init
  // Coverage thunk is generated here and added to the pu_tree list
  // When running ipl, nothing to do in this phase.
  // All instrumenetation will be done by apa_link (and be).
  if (!Run_ipl && Profile_Arcs_Enabled) {
    wn_coverage_Generate_Func_Start_Profiler_PU(&pu_tree);
  }
#endif
...
#ifndef linux
  // there seem to be a bug in linux that makes the deallocation of
  // static variables from a shared library fail after dlclose...
  close_components();
#endif

#ifndef __CYGWIN__
  // Deallocation also goes badly on some Solaris 5.7 patch levels < 19
  // So here we are with a fix identical to Cygwin's
  // Instead of going through exit(RC_OKAY) we avoid destructor mess
  // This could be well due to bad nesting of MEM_POOL_PUSH/MEM_POOL_POP
  // To be checked overall
  fflush(0);
  _exit ( RC_OKAY );
#else
  // workarround a bug in the deallocation of STL objects under Cygwin
