...
/*   unsigned nb_instr = 0; */
  ...
  for (ix = 0, blk = fn->blocks; ix != fn->num_blocks; ix++, blk++)
	...
	{
/* 	  if (!arc->fake && !arc->on_tree) */
/* 	    fprintf(stdout,"edge %d->%d has counter number %d value %d\n",arc->src->index,  */
/* 		    arc->dst->index, nb_instr++, *count_ptr); */
/* 	  else */
/* 	    fprintf(stdout,"edge %d->%d has no counter\n",arc->src->index,  */
/* 		    arc->dst->index); */
	  if (!arc->fake)
	    non_fake_succ++;
