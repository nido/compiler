...
#ifdef TARG_ST
      /* (cbr) local variables will be created when expanding function */
      && (current_binding_level == global_binding_level || TREE_STATIC (decl))
#endif
      && DECL_INITIAL (decl) != 0
      && DECL_INITIAL (decl) != error_mark_node)
    {
      ...
      if (TYPE_SIZE (t) && !integer_zerop (TYPE_SIZE(t)))
#endif
...
#ifdef SGI_MONGOOSE
	/* (cbr) */
	WFE_Assemble_Constructor (IDENTIFIER_POINTER (DECL_ASSEMBLER_NAME (fndecl)));
#else
  ...
  if (DECL_STATIC_DESTRUCTOR (fndecl))
    {
      if (targetm.have_ctors_dtors)
#ifdef SGI_MONGOOSE
	WFE_Assemble_Destructor (IDENTIFIER_POINTER (DECL_ASSEMBLER_NAME (fndecl)));
#else
