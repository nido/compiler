...
typedef char * string;
string get_driver_directory (const char *program);
string get_root_directory(string);
#endif
...
#ifdef TARG_ST
#define GCC_CROSS_COMPILE
#endif

#ifdef GCC_CROSS_COMPILE
...
"gfec -E %{traditional|ftraditional|traditional-cpp:-traditional-cpp}";
#else
static const char *trad_capable_cpp =
...
#ifdef TARG_ST
  /* (cbr) */
  tooldir_base_prefix = get_root_directory(get_driver_directory(argv[0]));
  tooldir_prefix = concat (tooldir_base_prefix, dir_separator_str, NULL);
#else
...
#ifdef TARG_ST
  add_prefix (&exec_prefixes,
	      concat (tooldir_prefix, "lib/cmplrs", dir_separator_str, NULL),
...
extern char *SYS_full_path(const char *program);

/* ==================================================================
 *   get_driver_directory
 *
 *   Depending on the host system, figure out the directory in which
 *   compiler driver resides.
 * ==================================================================
 */
string get_driver_directory (
  const char *program
)
{
  char *path = NULL;
  path = SYS_full_path(program);
  return path;
}
#endif
...
/*
 * Search for last occurence of 's1' in 's2', returns index
 * of the start of 's1' or zero if not found.
 * like strstr bug finds out the last occurence of the string
 */
static char *lstrstr(const char *s1, const char *s2){
    char *  strstr_ = strstr(s1, s2) ;
    char * lstrstr_ = strstr_ ;
    int skip = strlen(s2)  ;
    if (strstr_) 
	do {
	    lstrstr_ = strstr_ ;
	    strstr_ += skip ;
	} while (strstr_ = strstr(strstr_, s2)) ;
    return lstrstr_ ;
}

/*
 * get_root_directory
 * Based on information provided by get_driver_directory
 * Gives the root directory of the installation
 */
string get_root_directory(string driver_directory)
{
#if __GNUC__ >= 3
  string root_path = xmalloc(PATH_MAX);
#else
  string root_path = malloc(PATH_MAX);
#endif // __GNUC__ >= 3

#if defined(__CYGWIN__)
    char posix_driver_directory[_POSIX_PATH_MAX] ;
    cygwin_conv_to_full_posix_path(driver_directory, posix_driver_directory) ;
    if(root_path) {
	char *suffix ;
	// [CM] 20040209 MBTst16085 and 15633 : driver can live in bin or lib/cmplrs
	if ((suffix = lstrstr(posix_driver_directory, BINPATH)) || (suffix = lstrstr(posix_driver_directory, PHASEPATH))) {
	    strncpy(root_path, posix_driver_directory, suffix - posix_driver_directory) ;
	    *(root_path + (suffix  - posix_driver_directory)) = '\0' ;
	} else *root_path = '\0' ;
    }
#else
    if(root_path && driver_directory) {
	char *suffix ;
	// [CM] 20040209 MBTst16085 and 15633 : driver can live in bin or lib/cmplrs
	if ((suffix = lstrstr(driver_directory, BINPATH)) || (suffix = lstrstr(driver_directory, PHASEPATH))) {
	    strncpy(root_path, driver_directory, suffix - driver_directory) ;
	    *(root_path + (suffix  - driver_directory)) = '\0' ;
	} else *root_path = '\0' ;
    }
#endif
    return root_path ;
