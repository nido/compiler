...
#ifndef TARG_ST
      /* (cbr) don't output debugging infos here */
...
compile_file ()
{
...
#ifdef TARG_ST
  /* (cbr) make sure names are initialized before Orig_Src_File_Name */
  {
  ...
  WFE_Init (save_argc, save_argv);	/* sgi initialization */
  WFE_File_Init (save_argc, save_argv);	/* inits per source file */
#endif  
...
#ifndef TARG_ST
  /* (cbr) those were stripped from -TENV, -TARG,... that we want to keep */
  save_argc = argc;
  save_argv = argv;
#endif
     ...
     char **argv;
{
  ...
  /* (cbr) -TENV and -TARG options must not be passed to the front end */
 {
...
#ifdef TARG_ST
   /* (cbr) normally done in  parse_options_and_default_flag */
     save_argc = argc;
     save_argv = argv;
#endif
  ...
  /* Parse the options and do minimal processing; basically just
   ...
   parse_options_and_default_flags (gnu_argc, gnu_argv);
 }
#else
  /* Parse the options and do minimal processing; basically just
     enough to default flags appropriately.  */
