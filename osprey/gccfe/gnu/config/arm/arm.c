...
machine_mode_t output_memory_reference_mode;
...
arm_gen_compare_reg (code, x, y)
     ...
     rtx x, y;
{
  machine_mode_t mode = SELECT_CC_MODE (code, x, y);
     ...
     unsigned int to;
{
  ...
  if(from == ARG_POINTER_REGNUM) {
    if(to == THUMB_HARD_FRAME_POINTER_REGNUM) {
      return 0;
    }
    ...
    else if(to == ARM_HARD_FRAME_POINTER_REGNUM) {
...
get_arm_condition_code (comparison)
     rtx comparison;
{
  machine_mode_t mode = GET_MODE (XEXP (comparison, 0));
  int code;
