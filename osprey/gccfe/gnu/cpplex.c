...
#ifdef TARG_ST
  /* [CG]: Avoid uninitialized read at buffer->cur[0] 
     (avoids valgrind errors). */
...
#ifdef TARG_ST
      /* (cbr) see ddts pro-release-1-7-0-B/2 */
     ...
     cpp_reader *pfile;
{
 ...
 skipped_white:
#ifdef TARG_ST
  /* [CG]: Avoid uninitialized read at buffer->cur
     (avoids valgrind errors). */
  if (buffer->cur >= buffer->rlimit) { c = 0; buffer->cur++; }
  else
#endif
  c = *buffer->cur++;
