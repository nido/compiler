...
#ifdef TARG_ST
  gs_t n = t;
  while (n != NULL && !gs_em(n)) {
    gs_t next = NULL;
    int j;
    // The spin tree can be very deep, so to avoid very large
    // stack usage, choose to iterate on one child and recurse
    // on the others.
    // The best child to iterate on depends on the node type:
    // for a CONS node it is the cdr, for a normal tree node
    // it is the GS_TREE_CHAIN.
    int arity = gs_code_arity(gs_code(n));
    int iterate_opd = (gs_code(n) == CONS) ? 1 : GS_TREE_CHAIN;
    _gs_em(n, true);
    for (j = 0; j < arity; j++) {
      if (j == iterate_opd) {
	next = gs_operand(n, j);
      } else {
	flag_reachable_nodes(gs_operand(n, j));
      }
    }
    n = next;
  }
...
#ifdef TARG_ST
#if 0
  // [CM] We imitate DeWarn output here : some scripts (CControl gdb test suite) rely on this format
  // To filter-out harmless output in their context.
  if (leaked_nodes)
    fprintf ( stderr, "!!! DevWarn during Spin Translation: leaked nodes: %d\n", leaked_nodes);
#endif
#endif
#endif
