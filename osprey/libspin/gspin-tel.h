...
#if ! defined TARG_ST || defined _LP64
  return (gs_long_long_t) gs_ull (gs_tree_int_cst_low (t));
#else
  gs_unsigned_long_long_t h = gs_ull (gs_tree_int_cst_high (t));
  gs_unsigned_long_long_t l = gs_ull (gs_tree_int_cst_low (t));
  l = l << 32 >> 32;	// zero-out high 32 bits
  h = h << 32;
  return (gs_long_long_t) (h | l);
#endif
...
#ifdef TARG_ST
static inline gs_t gs_va_list_type_node(void) {
  gs_t t = gs_index(gs_global_trees_list(gs_program), GS_TI_VA_LIST_TYPE);
  GS_ASSERT(t != NULL, "gs_va_list_type_node: got NULL node");
  return t;
}

static inline gs_t gs_long_double_type_node(void) {
  gs_t t = gs_index(gs_global_trees_list(gs_program), GS_TI_LONG_DOUBLE_TYPE);
  GS_ASSERT(t != NULL, "gs_long_double_type_node: got NULL node");
  return t;
}
