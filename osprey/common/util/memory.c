...
)
{
  ...
  UINT32      hn   = Hash(line,(char *)file);
  MEM_STAT   *ms   = Hash_Get(hn,pool,line,(char *)file);
...
Allocate_Block (MEM_POOL *pool)
{
...
#if 0
  fprintf(stderr, " Allocate_Block for %s - requested : %d \n",
          MEM_POOL_name(pool),
  ...
  if (block == NULL) {
#if 0
    fprintf(stderr,"malloc failed ... requested size : %d \n",
            BLOCK_SIZE + PAD_TO_ALIGN(sizeof(MEM_BLOCK)));
    MEM_Trace();
#endif
