...
private:
  ...
  mINT32 Hash(const KEY_TYPE key, const UINT32 tbl_size) const
    {
#ifdef TARG_ST
      // [CG] Warning this hash key computation is endianness
      // sensitive. TODO: change it?
#endif
      // First part: convert key into a 32-bit key
      UINT32 key_uint32;
#ifdef TARG_ST
      // [CG] Avoids strict aliasing problem.
//       const unsigned char *cp = (const unsigned char *) &key;
      ALIASING_INT_PTR<UINT32> p = &key;
      if (sizeof(key) == sizeof(UINT32)) {
	  key_uint32 = *p;
// 	  key_uint32 = cp[0] + (cp[1] << 8) + (cp[2] << 16) + (cp[3] << 24);
      }
      else if (sizeof(key) < sizeof(UINT32)) {
	// Changed the hash key computation for types smaller
	// then UINT32 to avoid uninitialized reads.
