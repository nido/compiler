...
sig_itoa(INT i)
{
  ...
  sprintf(s,"signal %d", i);
  return s;
}
#define SYS_SIGLIST(x) (sig_itoa(x))
#else
#ifdef sun
#define sys_siglist _sys_siglist
#endif
#define SYS_SIGLIST(x) sys_siglist[x]
 ...
 * fatal error message.
 * [CG]: Why stdout? Redefined to write to stderr instead.
 * ====================================================================
 */
...
//   extern char *sys_errlist[];
...
#endif
    case SIGSEGV:
#if defined(ENXIO) && defined(ENOSPC)
    ...
    fprintf (stderr, "Signal: %s", SYS_SIGLIST(sig) );
...
static void
catch_signal_context (int sig, siginfo_t *sip, ucontext_t *uap)
{
  signal ( sig, SIG_DFL );

  const char *stack_trace = ContextStackTrace((void *)uap, true);
  fputs(stack_trace, stderr);
  catch_signal(sig, (sip != NULL ? sip->si_errno: 0));
}
#endif
...
setup_signal_handler (int s)
{
  ...
  if (signal (s, SIG_IGN) != SIG_IGN) {
	...
	sigaction(s,NULL,&act); // Get signal state
	// Set up SIGINFO handler
	act.sa_sigaction = (void (*)(int, siginfo_t*, void*))catch_signal_context;
	act.sa_flags |= SA_SIGINFO;
	sigaction(s,&act,NULL); // Update signal state
      } break;
#endif
    default:
...
#endif
#if !defined(linux) && defined(SIGEMT)
    setup_signal_handler (SIGEMT); /* emulation trap */
#endif
#ifdef SIGFPE
...
BE_EXPORTED void Abort_Compiler_Location (
  char * file_name,
  INT    line_number )
{
...
#if defined(TARG_ST) && defined(STACK_TRACE) && defined(BACK_END)
  const char *stack_trace = StackTrace(true);
  fputs(stack_trace, stderr);
#endif
...
#if defined(TARG_ST) && defined(BACK_END) && defined(STACK_TRACE) && defined(STACK_TRACE_IN_DEVWARN)
  const char *stack_trace = StackTrace(true,false,0,0,false,3);
#endif
