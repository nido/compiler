	       ...
	       Fb_Hdr& fb_hdr, Pu_Hdr *pu_hdr_table, char *str_table) 
{
  ...
  for (ULONG i = 0; i < fb_hdr.fb_pu_hdr_num; i++) {
...
{
  fb_reader_Fb_Hdr_Read(fb_hdr, fp, fname);
}
...
Get_Pu_Hdr_Table(FILE *fp, char *fname, Fb_Hdr& fb_hdr, 
		 Pu_Hdr *pu_hdr_table)
{
  fb_reader_Pu_Hdr_list_Read(pu_hdr_table, &fb_hdr, fp, fname);
}
...
{
  fb_reader_Str_Table(fp, fname, &fb_hdr, str_table);
}
...
#ifdef TARG_ST
  Inv_Table.reserve(pu_hdr_entry.pu_num_inv_entries);
  LIBFB_Info_Invoke *fb_info;
  size_t size = pu_hdr_entry.pu_num_inv_entries;
  fb_info = (LIBFB_Info_Invoke *)malloc(sizeof(LIBFB_Info_Invoke) * size);;
  fb_reader_Info_Invoke_list_Read(fb_info, &pu_hdr_entry, pu_ofst, fp, fname);
  for (size_t i = 0; i < size; i++) {
    Inv_Table.push_back (FB_Info_Invoke (FB_FREQ (fb_info[i].freq_invoke)));
  }
  free (fb_info);
#else
  Inv_Table.resize(pu_hdr_entry.pu_num_inv_entries);
  fb_reader_Info_Invoke_list_Read(&(Inv_Table.front ()), &pu_hdr_entry, pu_ofst, fp, fname);
#endif
...
#ifdef TARG_ST
  Br_Table.reserve(pu_hdr_entry.pu_num_br_entries);
  LIBFB_Info_Branch *fb_info;
  size_t size = pu_hdr_entry.pu_num_br_entries, i;
  fb_info = (LIBFB_Info_Branch *)malloc(sizeof(LIBFB_Info_Branch) * size);;
  fb_reader_Info_Branch_list_Read(fb_info, &pu_hdr_entry, pu_ofst, fp, fname);
  for (i = 0; i < size; i++) {
    Br_Table.push_back (FB_Info_Branch (FB_FREQ (fb_info[i].freq_taken),
				    FB_FREQ (fb_info[i].freq_not_taken)));
  }
  free (fb_info);
#else
  Br_Table.resize(pu_hdr_entry.pu_num_br_entries);
  fb_reader_Info_Branch_list_Read(&(Br_Table.front ()), &pu_hdr_entry, pu_ofst, fp, fname);
#endif  
}
                    ...
                    long pu_ofst, FILE *fp, char *fname)
{
  ...
  fb_reader_Get_Sizes_Of_Switches(&(targets_vector.front ()), &pu_hdr_entry, pu_ofst, fp, fname);
  ...
  fb_reader_Info_Switch_begin(pu_hdr_entry, pu_ofst, fp, fname);
  ...
  for (FB_Switch_Vector::iterator first (Switch_Table.begin ());
    ...
    int t;
    first->freq_targets.reserve(*target);
    INT64 *freq_targets = (INT64 *)malloc(*target * sizeof(INT64)); 
    fb_reader_INT64_list_Read(freq_targets, *target, fp, fname);
    for (t = 0; t < *target; t++ )
      first->freq_targets.push_back(FB_FREQ (freq_targets[t]));
    ++target;
  }
} 
                    ...
                    long pu_ofst, FILE *fp, char *fname)
{
  ...
  // Get sizes of CGOTOs
  fb_reader_Get_Sizes_Of_CGotos(&(target_vectors.front ()), &pu_hdr_entry, pu_ofst, fp, fname);
  ...
  // Set the right position in file
  fb_reader_Info_CGoto_begin(pu_hdr_entry, pu_ofst, fp, fname);
                    ...
                    long pu_ofst, FILE *fp, char *fname)
{
...
#ifdef TARG_ST
  Loop_Table.reserve(pu_hdr_entry.pu_num_loop_entries);
  LIBFB_Info_Loop *fb_info;
  size_t size = pu_hdr_entry.pu_num_loop_entries, i;
  fb_info = (LIBFB_Info_Loop *)malloc(sizeof(LIBFB_Info_Loop) * size);;
  fb_reader_Info_Loop_list_Read(fb_info, &pu_hdr_entry, pu_ofst, fp, fname);
  for (i = 0; i < size; i++) {
    // Did the very last pass through the loop have zero trip count?
    INT64 zero_trips = fb_info[i].num_zero_trips;
    if ( fb_info[i].invocation_count > 0 && fb_info[i].last_trip_count == 0 ) {
      zero_trips++;
    }

    INT64 tmp = fb_info[i].invocation_count - zero_trips;
    INT64 tmp1 = fb_info[i].total_trip_count - fb_info[i].invocation_count + zero_trips;
    Loop_Table.push_back(FB_Info_Loop(FB_FREQ (zero_trips),
				    FB_FREQ ( tmp),
  ...
  fb_reader_Info_Loop_list_Read(&(Loop_Table.front ()), &pu_hdr_entry, pu_ofst, fp, fname);
#endif
}
...
#ifdef TARG_ST
  Scircuit_Table.reserve(pu_hdr_entry.pu_num_scircuit_entries);
  LIBFB_Info_Circuit *fb_info;
  size_t size = pu_hdr_entry.pu_num_scircuit_entries, i;
  fb_info = (LIBFB_Info_Circuit *)malloc(sizeof(LIBFB_Info_Circuit) * size);;
  fb_reader_Info_Circuit_list_Read(fb_info, &pu_hdr_entry, pu_ofst, fp, fname);
  for (i = 0; i < size; i++) {
    //TB: Fix: freq left is never computed bu feedback info, so put it
    //to unknown and leave the freq propagation compte it
    Scircuit_Table.push_back(FB_Info_Circuit (FB_FREQ_UNKNOWN,
					      FB_FREQ (fb_info[i].freq_right),
					      FB_FREQ (fb_info[i].freq_neither)));
  }
  free (fb_info);
#else
  Scircuit_Table.resize(pu_hdr_entry.pu_num_scircuit_entries);
  fb_reader_Info_Circuit_list_Read(&(Scircuit_Table.front ()), &pu_hdr_entry, pu_ofst, fp, fname);
#endif
}
...
#ifdef TARG_ST
  Call_Table.reserve(pu_hdr_entry.pu_num_call_entries);
  LIBFB_Info_Call *fb_info;
  size_t size = pu_hdr_entry.pu_num_call_entries, i;
  fb_info = (LIBFB_Info_Call *)malloc(sizeof(LIBFB_Info_Call) * size);;
  fb_reader_Info_Call_list_Read(fb_info, &pu_hdr_entry, pu_ofst, fp, fname);

  for (i = 0; i < size; i++) {
    Call_Table.push_back(FB_Info_Call (FB_FREQ (fb_info[i].freq_entry),
				       FB_FREQ (fb_info[i].freq_exit),
				       fb_info[i].freq_entry == fb_info[i].freq_exit));
  }
  free (fb_info);
#else
  Call_Table.resize(pu_hdr_entry.pu_num_call_entries);
  fb_reader_Info_Call_list_Read(&(Call_Table.front ()), &pu_hdr_entry, pu_ofst, fp, fname);
#endif
}
...
#ifdef TARG_ST
  Icall_Table.reserve(pu_hdr_entry.pu_num_icall_entries);
  LIBFB_Info_Icall *fb_info;
  size_t size = pu_hdr_entry.pu_num_icall_entries, i;
  fb_info = (LIBFB_Info_Icall *)malloc(sizeof(LIBFB_Info_Icall) * size);;
  fb_reader_Info_Icall_list_Read(fb_info, &pu_hdr_entry, pu_ofst, fp, fname);

  for (i = 0; i < size; i++) {
    Icall_Table.push_back(FB_Info_Icall (fb_info[i].tnv));
  }
  free (fb_info);
#else
...
#ifdef TARG_ST
  Value_FP_Bin_Table.reserve(pu_hdr_entry.pu_num_value_fp_bin_entries);
  LIBFB_Info_Value_FP_Bin *fb_info;
  size_t size = pu_hdr_entry.pu_num_value_fp_bin_entries, i;
  fb_info = (LIBFB_Info_Value_FP_Bin *)malloc(sizeof(LIBFB_Info_Value_FP_Bin) * size);;
  fb_reader_Info_Value_FP_Bin_list_Read(fb_info, &pu_hdr_entry, pu_ofst, fp, fname);

  for (i = 0; i < size; i++) {
    Value_FP_Bin_Table.push_back( FB_Info_Value_FP_Bin(fb_info[i].exe_counter, fb_info[i].zopnd0, 
						       fb_info[i].zopnd1, fb_info[i].uopnd0, 
						       fb_info[i].uopnd1));
  }
  free (fb_info);
...
#ifdef TARG_ST
  Value_Table.reserve(pu_hdr_entry.pu_num_value_entries);
  LIBFB_Info_Value *fb_info;
  size_t size = pu_hdr_entry.pu_num_value_entries, i;
  fb_info = (LIBFB_Info_Value *)malloc(sizeof(LIBFB_Info_Value) * size);;
  fb_reader_Info_Value_list_Read(fb_info, &pu_hdr_entry, pu_ofst, fp, fname);

  for (i = 0; i < size; i++) {
    Value_Table.push_back( FB_Info_Value(fb_info[i].num_values, fb_info[i].exe_counter, 
					 fb_info[i].value, fb_info[i].freq));
  }
  free (fb_info);
#else
...
#ifdef TARG_ST
  Edge_Table.reserve(pu_hdr_entry.pu_num_edge_entries);
  LIBFB_Info_Edge *fb_info;
  size_t size = pu_hdr_entry.pu_num_edge_entries, i;
  fb_info = (LIBFB_Info_Edge *)malloc(sizeof(LIBFB_Info_Edge) * size);;
  fb_reader_Info_Edge_list_Read(fb_info, &pu_hdr_entry, pu_ofst, fp, fname);

  for (i = 0; i < size; i++) {
    Edge_Table.push_back( FB_Info_Edge(FB_FREQ(fb_info[i].freq_edge)));
  }
  free (fb_info);
#else
...
#ifdef TARG_ST
  FmtAssert( FALSE,
	     ("Intrumentation reader: stride reader not yet implemented")); 
