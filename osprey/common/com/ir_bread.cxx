...
#include "W_errno.h"		    /* for error code */
...
check_elf_header (char* baseaddr, Elf64_Word size, const ELF& tag)
{
	...
	return ERROR_RETURN;
// [HK]      if (ehdr->e_type != ET_IR || ehdr->e_shentsize != sizeof(ELF::Elf_Shdr))
...
// [HK] 	ehdr->e_shoff + ehdr->e_shnum * sizeof(ELF::Elf_Shdr) > size)
    ...
    if ((long) shdr & (
#ifndef __GNUC__
//  [HK]		       __builtin_alignof(ELF::Elf_Shdr)
		       __builtin_alignof(typename ELF::Elf_Shdr)
#else
// [HK] 		       __alignof__(ELF::Elf_Shdr)
