...
#define SIMPNODE_label_number WN_label_number	    // label number of LDA_LABEL
...
// [HK] interferences with double log2(double) of /usr/include/bits/mathcalls.h
//  static UINT64 log2(UINT64 x)
static UINT64 loga2(UINT64 x)
{
   UINT64 l;
...
#ifdef TARG_ST
BOOL SIMP_is_volatile_mem (simpnode x)
{
  OPCODE opc = SIMPNODE_opcode(x);
  if (OPCODE_has_1ty(opc)) {
	...
	OPCODE_operator(opc) == OPR_ISTBITS) {
      TY_IDX pointed = TY_pointed (SIMPNODE_ty (x));
      DevAssert(pointed, ("TY_pointed of %s type is NULL", OPCODE_name(opc)));
      return TY_is_volatile(pointed);
    } else {
      return TY_is_volatile(SIMPNODE_ty(x));
    }
  } else if (OPCODE_has_2ty(opc)) {
    TY_IDX pointed = TY_pointed (SIMPNODE_base_ty(x));
...
BOOL SIMP_has_side_effects (simpnode x)
{
  switch (SIMPNODE_operator (x)) {

    case OPR_ARRAY:
 
      if (SIMP_has_side_effects (SIMPNODE_kid0(x)))
        return TRUE;

      else {

        INT32 n = (SIMPNODE_kid_count (x)) >> 1;

        for (INT32 i = n + 1; i <= 2 * n; i++) {

          if (SIMP_has_side_effects (SIMPNODE_kid (x, i)))
            return TRUE;
        }
  ...
  case OPR_LROTATE:
    ...
    default:
      ...
      Fail_FmtAssertion ("SIMP_has_side_effects not implemented for %s",
                         OPERATOR_name (SIMPNODE_operator (x)));
      return FALSE;
  }

  {

    INT32 n = SIMPNODE_kid_count (x);
...
INT32 SIMPNODE_Simp_Compare_Trees(simpnode t1, simpnode t2)
{
   ...
   case OPR_LDA_LABEL:
     return SIMPNODE_label_number(t1) - SIMPNODE_label_number(t2);
#endif
    case OPR_ARRAY:
...
/* ====================================================================
 *
 * BOOL SIMPNODE_Simp_Trees_Have_Same_Value(simpnode t1, simpnode t2)
 *
 * Returns TRUE if two WHIRL trees are identical and have no side
 * effects.
 * ====================================================================
 */

static BOOL SIMPNODE_Simp_Trees_Have_Same_Value(simpnode t1, simpnode t2)
{
  return (SIMPNODE_Simp_Compare_Trees(t1,t2) == 0
...
#ifdef TARG_ST
   } else if (SIMPNODE_Simp_Trees_Have_Same_Value(k0,k1)) {
#else
...
#ifdef TARG_ST
		 SIMPNODE_Simp_Trees_Have_Same_Value(SIMPNODE_kid1(k0),SIMPNODE_kid1(k1))) {
#else
...
#ifdef TARG_ST
		 SIMPNODE_Simp_Trees_Have_Same_Value(SIMPNODE_kid1(k0),SIMPNODE_kid0(k1))) {
#else
...
#ifdef TARG_ST
		 SIMPNODE_Simp_Trees_Have_Same_Value(SIMPNODE_kid0(k0),SIMPNODE_kid1(k1))) {
#else
			  ...
			  BOOL k0const, BOOL k1const)
{
   ...
   if (op == OPR_RECIP) {
...
#ifdef TARG_ST
	 if (Reassoc_Level >= REASSOC_SIMPLE) {
#else
	 ...
	 if (Rsqrt_Allowed && !(Only_32_Bit_Ops && ty == MTYPE_F8)) {
	 ...
	 break;
	      ...
	      Rsqrt_Allowed && !(Only_32_Bit_Ops && ty == MTYPE_F8)) {
			  ...
			  BOOL k0const, BOOL k1const)
{
   ...
   if (op == OPR_CVT &&
	  ...
	  // Arthur: what it really wants is to make sure MTYPE_CLASSES
	  // are integer and the same signedness.
	  // I can replace a LOAD I8I1, etc. but not I4A4 or I4B !
	  MTYPE_is_class_integer(source_type) &&
	  MTYPE_is_class_integer(dest_type) &&
...
#ifdef TARG_ST
	 /*
	  * Arthur: also suppress the I8/U8 vs others if only 32-bit
	  *         operations allowed on the target
	  */
	 if (Only_32_Bit_Ops && 
	     (MTYPE_bit_size(source_type) > 32 || MTYPE_bit_size(dest_type) > 32)) {
	   newopc = OPCODE_UNKNOWN;
	 }
#endif
...
#ifdef TARG_ST
   /* 
    * Arthur: because we may be passing floating-point
    * data in integer registers, we may and up having 
    * these strange cases
    */
   if (op == OPR_TAS) {

     if (OPCODE_rtype(opc) == OPCODE_rtype(k0opc)) {
       r = k0;
       return (r);
     }
     ...
     if (k0op == OPR_TAS) {
       k0k0 = SIMPNODE_kid0(k0);
       source_type = SIMPNODE_rtype(k0k0);
       /*source_value_type = get_value_type(k0k0);*/
       inter_type = OPCODE_rtype(k0opc);
       dest_type = OPCODE_rtype(opc);
       if (MTYPE_is_class_integer(dest_type) &&
	   MTYPE_is_class_integer(source_type)) {
	 if (Is_Valid_Opcode_Parts(OPR_CVT,dest_type,source_type)) {
	   newopc = OPCODE_make_op(OPR_CVT,dest_type,source_type);
	   SHOW_RULE("t1TAS(t2TAS(a)) -> t1CVT(a)");
	   r = SIMPNODE_SimpCreateExp1(newopc,k0k0);
	   SIMP_DELETE(k0);
	 }
	 else if (source_type == dest_type) {
	    SHOW_RULE("t1TAS(t2TAS(a)) -> a");
	    r = k0k0;
	    SIMP_DELETE(k0);
	 } 
	 else {
	   FmtAssert(FALSE,("TAS->TAS: should be able to simplify"));
	 }
       }
     }
   }
		       ...
		       simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   if (first_op==OPR_NEG && second_op==OPR_NEG) {
      ...
      SHOW_RULE(" -x * c ");
#ifdef TARG_ST
      // (cbr) If we end up negating a constant that was unsigned,
      // make it signed.
      if (MTYPE_is_unsigned(ty) && SIMP_IS_TYPE_UNSIGNED(SIMP_TYPE(k1)))
          ty = MTYPE_complement(ty);
#endif
...
#ifdef TARG_ST
   if (Enable_Cfold_Aggressive && Reassoc_Level >= REASSOC_SIMPLE) {
#else
   if (Enable_Cfold_Aggressive && Roundoff_Level >= ROUNDOFF_SIMPLE) {
    ...
    if (first_op==OPR_SQRT && second_op==OPR_RECIP
	 ...
	 SIMP_DELETE_TREE(k0);
#ifdef TARG_ST
      } else if (first_op == OPR_RSQRT && SIMPNODE_Simp_Trees_Have_Same_Value(k1,SIMPNODE_kid0(k0))) {
#else
	 ...
	 SIMP_DELETE_TREE(k0);
#ifdef TARG_ST
      } else if (second_op == OPR_RSQRT && SIMPNODE_Simp_Trees_Have_Same_Value(k0,SIMPNODE_kid0(k1))) {
#else
		     ...
		     simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   if (k0const && SIMP_IS_TYPE_FLOATING(ty) && !SIMP_IS_TYPE_COMPLEX(ty)) {
	  ...
	  (Recip_Allowed && !(Only_32_Bit_Ops && ty == MTYPE_F8))) {
	 SHOW_RULE("+-1.0 / a");
	 ...
	 SIMP_DELETE(k0);
	      ...
	      && Div_Split_Allowed && (Recip_Allowed && !(Only_32_Bit_Ops && ty == MTYPE_F8))) {
...
#ifdef TARG_ST
	  && SIMPNODE_Simp_Trees_Have_Same_Value (SIMPNODE_kid0(k0),k1)) {
#else
...
#ifdef TARG_ST
		 && SIMPNODE_Simp_Trees_Have_Same_Value(SIMPNODE_kid0(k1),k0)) {
#else
...
#ifdef TARG_ST
   if (SIMPNODE_Simp_Trees_Have_Same_Value(k0,k1)) {
#else
		      ...
		      simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   if (k1const) {
      c1 = SIMP_Int_ConstVal(k1); 
#ifdef TARG_ST
      if (c1 == 0 && ! SIMP_has_side_effects (k0)) {
#else
      if (c1 == 0) {
#endif
	 SHOW_RULE("j&0");
...
#ifdef TARG_ST
   if ((SIMPNODE_operator(k0)==OPR_BNOT && SIMPNODE_Simp_Trees_Have_Same_Value(SIMPNODE_kid0(k0),k1)) ||
       (SIMPNODE_operator(k1)==OPR_BNOT && SIMPNODE_Simp_Trees_Have_Same_Value(SIMPNODE_kid0(k1),k0))) {
#else
		      ...
		      simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   if (k1const) {
	 ...
	 SIMP_DELETE(k1);
#ifdef TARG_ST
      } else if (c1 == -1 && ! SIMP_has_side_effects (k0)) {
#else
      } else if (c1 == -1) {
#endif
	 SHOW_RULE("j|-1");
	 ...
	 SIMP_DELETE(k1);
		 ...
		 && (get_inverse_relop(SIMPNODE_opcode(k0))!=0)) {
#else
      } else if ((c1 == 1) && (get_inverse_relop(SIMPNODE_opcode(k0))!=0)) {
#endif
	 SHOW_RULE("<comp> | 1");
...
#ifdef TARG_ST
   if ((SIMPNODE_operator(k0)==OPR_LNOT && SIMPNODE_Simp_Trees_Have_Same_Value(SIMPNODE_kid0(k0),k1)) ||
       (SIMPNODE_operator(k1)==OPR_LNOT && SIMPNODE_Simp_Trees_Have_Same_Value(SIMPNODE_kid0(k1),k0))) {
#else
		      ...
		      simpnode k0, simpnode k1, BOOL k0const, BOOL k1const)
{
   ...
   if (k1const) {
	 ...
	 SIMP_DELETE(k1);
#ifdef TARG_ST
      } else if (! SIMP_has_side_effects (k0)) {
#else
      } else {
#endif
	 SHOW_RULE("j||1");
...
#ifdef TARG_ST
   if ((SIMPNODE_operator(k0)==OPR_LNOT && SIMPNODE_Simp_Trees_Have_Same_Value(SIMPNODE_kid0(k0),k1)==0) ||
       (SIMPNODE_operator(k1)==OPR_LNOT && SIMPNODE_Simp_Trees_Have_Same_Value(SIMPNODE_kid0(k1),k0)==0)) {
#else
   if ((SIMPNODE_operator(k0)==OPR_LNOT && SIMPNODE_Simp_Compare_Trees(SIMPNODE_kid0(k0),k1)==0) ||
       (SIMPNODE_operator(k1)==OPR_LNOT && SIMPNODE_Simp_Compare_Trees(SIMPNODE_kid0(k1),k0)==0)) {
#endif
...
#ifdef TARG_ST
	|| Reassoc_Level < REASSOC_ANY)) return(r);
#else
...
simpnode simp_cvtl(OPCODE opc, INT16 cvtl_bits, simpnode k0)
{
   ...
   if (SIMP_Is_Constant(k0)) {
	...
	r = SIMPNODE_SimpCreateExp3(OPC_FROM_OPR(OPR_SELECT,OPCODE_rtype(opc)),
	 ...
	 SHOW_RULE("CVTL(SELECT(x,c1,c2))");
	      ...
	      SIMPNODE_operator(k0) == OPR_ILOAD) {
     /*
      * [CG]: Perform CVTL Tx #x_bits (LOAD Ty Tz) optimizations
      * The CVTL can be avoided when Ty and Tz are integer types
      * and bits(Tx) == bits(Ty)
      * and x_bits == bits(Tz)
      * and sign(Tx) == sign(Tz)
      */
     TYPE_ID load_src_ty = OPCODE_desc(SIMPNODE_opcode(k0));
     TYPE_ID load_dst_ty = OPCODE_rtype(SIMPNODE_opcode(k0));
     if (MTYPE_is_class_integer(load_dst_ty) && MTYPE_is_class_integer(load_src_ty) &&
	 MTYPE_bit_size(dest_ty) == MTYPE_bit_size(load_dst_ty)  &&
	 cvtl_bits == MTYPE_bit_size(load_src_ty) &&
	 MTYPE_is_unsigned(dest_ty) == MTYPE_is_unsigned(load_src_ty)) {
       r = k0;
     }
   } else if (SIMPNODE_operator(k0) == OPR_CVTL) {
     /*
      * Arthur: seems like a CVTL (on a 32 bit machine at least)
      *         is always of type I4 or U4. Thus, the opcodes may
      *         be different, i.e. I4CVTL vs U4CVTL. It would seem
      *         reasonable to just remove the first CVTL (kid) ?
      *  TODO: is it true also for 64 bit architectures ?
      *        why didn't they do it for IA64 ?
      *  [CG]: Yes it is also true.
      *  [CG]: The exact rules (for 32 and 64 bits) are:
      *  CVTL Tx #x_bits (CVTL Ty #y_bits (A))
      *  x_bits <= y_bits: CVTL Tx #x_bits (A)
      *  x_bits > y_bits && unsigned(Ty): CVTL Ty #y_bits (A)
      *  x_bits > y_bits && signed(Ty): CVTL Tx #x_bits (CVTL Ty #y_bits (A))
      */

     /* CVTL (CVTL) */
     if (SIMP_TYPE_SIZE(source_ty) == SIMP_TYPE_SIZE(dest_ty)) {
       k0_bits = SIMPNODE_cvtl_bits(k0);
       if (cvtl_bits <= k0_bits) {
	 SHOW_RULE("CVTL(n1,CVTL(n2,k0)), n1 <= n2");
	 r = SIMPNODE_SimpCreateCvtl(opc,cvtl_bits,SIMPNODE_kid0(k0));
       } else {
	 if (MTYPE_is_unsigned(source_ty)) {
	   SHOW_RULE("CVTL(n1,CVTL(n2,k0)), n1 > n2 (unsigned)");
	   r = k0;
	 }
       }
     }
   ...
   switch (opr0) {
    ...
    case OPR_SHL:
      ...
      k2 = SIMPNODE_kid1(k0);
#ifdef TARG_ST
      // FdF 24/05/2004 DDTS 18034: On MPY operators, it is useful to
      // keep some CVTLs to generate 16x16 or 16x32 MUL operators.
      if (opr0 == OPR_MPY &&
	  ((SIMPNODE_opcode(k1) == opc &&
	    SIMPNODE_cvtl_bits(k1) <= 16) ||
	   (SIMPNODE_opcode(k2) == opc &&
	    SIMPNODE_cvtl_bits(k2) <= 16)))
	return (r);
#endif
      if (SIMPNODE_opcode(k1) == opc &&
	  SIMPNODE_cvtl_bits(k1) >= cvtl_bits && 
...
#ifdef TARG_ST
	/* [CG 2005/03/29] We also have to handle integer conversions. */
	if (OPCODE_rtype(k0op) != opc_rtype)
	  r = SIMPNODE_SimpCreateExp1(
		OPCODE_make_op(OPR_CVT, opc_rtype, OPCODE_rtype(k0op)), r);
#else
...
#ifdef TARG_ST
	 // [CG]: must set volatile type if load addr type pointed is volatile
	 if (load_addr_ty != 0 && TY_is_volatile(TY_pointed(load_addr_ty))) Set_TY_is_volatile(ty);
#endif
