...
#include "W_signal.h"		    /* for signal()/kill() */
#include "W_errno.h"		    /* for system error code */
...
#ifdef TARG_ST // [CL] Generate intermediate files with host's endianness
    if (Host_Byte_Sex == BIG_ENDIAN) {
...
// [HK]     memcpy (shdr, &strtab_sec, sizeof(ELF::Elf_Shdr));
...
#if defined(TARG_ST) && defined(__MINGW32__)
#include "SYS.h"
#endif
...
WN_close_output (Output_File *fl)
{
    ...
    /* must unmap before truncate... (required for Win32 file systems) */
...
#ifdef TARG_ST
    // [CG]: Use new interface to get continuous buffer
    STR_TAB_BUFFER *stbuffer = Strtab_new_str_tab_buffer();
    WN_write_strtab(stbuffer->buffer(), stbuffer->size (), ir_output);
