...
typedef struct MHD_LEVEL {
  ...
  void      Merge_Options(const MHD_LEVEL& o);
  BE_EXPORTED BOOL      Valid() const;
  BE_EXPORTED BOOL      TLB_Valid() const;
...
#if defined(_LANGUAGE_C_PLUS_PLUS)
  BE_EXPORTED INT       First();
  BE_EXPORTED INT       Next(INT);
...
BE_EXPORTED extern MHD Mhd;
BE_EXPORTED extern MHD Mhd_Options;
