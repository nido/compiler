...
ST_Verify_Flags (const ST &s)
{
  ...
  if (ST_is_thread_private (s)) {
    Is_True (ST_sym_class(s) == CLASS_VAR,
	     (msg, "Class", Class_Name(ST_sym_class(s)), "ST_IS_THREAD_PRIVATE"));
    Is_True (ST_sclass(s) == SCLASS_PSTATIC ||
	     ST_sclass(s) == SCLASS_FSTATIC ||
	     ST_sclass(s) == SCLASS_EXTERN  ||
	     ST_sclass(s) == SCLASS_UGLOBAL ||
...
ST_Verify_Fields(const ST &s) 
{
  ...
  if ( ST_base_idx(s) != ST_st_idx(s)) {
    ...
    if ( ST_storage_class (*sb) != SCLASS_UNKNOWN) {
      ...
      if ( !ST_is_weak_symbol (s) &&
	   !(
	     // [CG]: In addition to weak symbols, aliased synmbols may also
	     // have different storage class (see wfe_decl.cxx).
	     // It seems that identifying an alias is done with the 
	     // following sequence:
	     Has_Base_Block(&s) && ST_class(ST_base(&s)) != CLASS_BLOCK
	     && ST_emit_symbol(&s)))
