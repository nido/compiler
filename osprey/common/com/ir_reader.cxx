...
static void ir_put_wn(WN * wn, INT indent)
{
        ...
        && opcode != OPC_PRAGMA)
	{
		...
		if (Preg_Offset_Is_Int(WN_offset(wn))) {
	    		fprintf(ir_ofile, " # $r%d", 
				WN_offset(wn) - Int_Preg_Min_Offset);
		}
		else if (Preg_Offset_Is_Ptr(WN_offset(wn))) {
	    		fprintf(ir_ofile, " # $p%d", 
				WN_offset(wn) - Ptr_Preg_Min_Offset);
		}
...
BE_EXPORTED extern void Check_for_IR_Dump(INT phase, WN *pu, char *phase_name)
{
    BOOL dump_ir;
    BOOL dump_symtab;
