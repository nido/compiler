...
BE_EXPORTED extern TCON Targ_WhirlOp ( OPCODE op, TCON left_opnd, TCON right_opnd,
		     BOOL *folded /* Was operator folded? */ );
#endif

BE_EXPORTED extern TCON Targ_IntrinsicOp ( UINT32 intrinsic, TCON c[],
		     BOOL *folded /* Was operator folded? */ );

/* Perform target exponentiation: */
BE_EXPORTED extern TCON Targ_Pow ( TCON mantissa, TCON exponent );
 ...
 * to target constant:
 */
BE_EXPORTED extern TCON Targ_Hexfptoc ( const TYPE_ID ty, const char * const str );
 ...
 */
BE_EXPORTED extern INT64 Targ_To_Host ( TCON cvalue );

/* Convert to host signed integer.  The TCON must be of a type reasonably
 * convertible to an integer type, e.g. float types are not allowed:
 */
BE_EXPORTED extern INT64 Targ_To_Signed_Host ( TCON cvalue );

/* Convert host integer value to a TCON of given type: */
BE_EXPORTED extern TCON Host_To_Targ ( TYPE_ID ctype, INT64 ivalue );

/* Convert host floating point value to a TCON of given type: */
BE_EXPORTED extern TCON Host_To_Targ_Float ( TYPE_ID ctype, double fvalue );
BE_EXPORTED extern TCON Host_To_Targ_Float_4 ( TYPE_ID ctype, float fvalue );
BE_EXPORTED extern TCON Host_To_Targ_Quad  ( QUAD_TYPE fvalue );

/* Convert a TCON to a double or a quad: */
BE_EXPORTED extern double Targ_To_Host_Float ( TCON fvalue );
BE_EXPORTED extern QUAD_TYPE Targ_To_Host_Quad ( TCON fvalue );

/* Convert host complex value to a TCON of given type: */
BE_EXPORTED extern TCON Host_To_Targ_Complex_4 ( TYPE_ID ctype, float real, float imag );
BE_EXPORTED extern TCON Host_To_Targ_Complex ( TYPE_ID ctype, double real, double imag );
BE_EXPORTED extern TCON Host_To_Targ_Complex_Quad ( QUAD_TYPE real, QUAD_TYPE imag );

/* Make complex TCON from two TCONs representing real and imaginary parts. */
BE_EXPORTED extern TCON Make_Complex ( TYPE_ID ctype, TCON real, TCON imag );

/* Create Tcon for unitialized variable */
BE_EXPORTED extern TCON Host_To_Targ_UV(TYPE_ID ctype);

/* Extract complex real/imag TCON to a TCON: */
BE_EXPORTED extern TCON Extract_Complex_Real( TCON complex);
BE_EXPORTED extern TCON Extract_Complex_Imag( TCON complex);

/* Extract hi/lo of quad TCON to a TCON: */
BE_EXPORTED extern TCON Extract_Quad_Hi( TCON q);
BE_EXPORTED extern TCON Extract_Quad_Lo( TCON q);

#ifdef TARG_ST
/* Extract hi/lo of double TCON to a TCON: */
BE_EXPORTED extern TCON Extract_Double_Hi(TCON d);
BE_EXPORTED extern TCON Extract_Double_Lo(TCON d);

/* Extract hi/lo of longlong TCON to a TCON: */
BE_EXPORTED extern TCON Extract_LongLong_Hi(TCON d);
BE_EXPORTED extern TCON Extract_LongLong_Lo(TCON d);
#endif
 ...
 */
BE_EXPORTED extern TCON Host_To_Targ_String ( TYPE_ID ctype, char *cp, UINT32 len );
BE_EXPORTED extern char *Targ_String_Address ( TCON cvalue );
BE_EXPORTED extern mUINT32 Targ_String_Length ( TCON cvalue );
 ...
 * a comment.
 */
BE_EXPORTED extern void Targ_Emit_String ( FILE *fl, char *str, INT32 len, INTSC loc );
 ...
 * to it:
 */
BE_EXPORTED extern char *Tcon_To_Str ( char *buf, TCON cval );

/* Given a constant in target format in the given buffer, of the given
 * type, convert it to a TCON:
 */
BE_EXPORTED extern TCON Str_To_Tcon ( TYPE_ID ctype, char *buf );
BE_EXPORTED extern TCON Bit_Str_To_Tcon ( TYPE_ID ctype, char *buf );

/* A target constant with value zero: */
BE_EXPORTED BE_EXPORTED extern TCON Zero_I4_Tcon;
BE_EXPORTED BE_EXPORTED extern TCON Zero_I8_Tcon;
BE_EXPORTED BE_EXPORTED extern TCON Quad_Zero_Tcon;

/* Is the given constant a zero? */
BE_EXPORTED extern BOOL Targ_Is_Zero ( TCON t );

/* Determine whether a TCON represents an integral value, and if so
 * return its value in *iv:
 */
BE_EXPORTED extern BOOL Targ_Is_Integral ( TCON t, INT64 *iv );

/* Determine whether a TCON represents a power of two: */
BE_EXPORTED extern BOOL Targ_Is_Power_Of_Two ( TCON t );

/* Determin whether a TCON contains only one bit set to 1
 * If onebit is not NULL then return the bit number which is on
 */
BE_EXPORTED extern BOOL Targ_Contains_One_Bit_On ( TCON t, INT32 *onebit);
 ...
 */
BE_EXPORTED extern BOOL Targ_Determine_High_Bit ( TCON t, INT32 *highbit );

/* Hash a TCON into a 32-bit integer modulo another integer: */
BE_EXPORTED extern UINT32 Hash_TCON ( TCON * t, UINT32 modulus );

BE_EXPORTED extern INT32 Targ_fp_class(TCON fvalue);

#ifdef Is_True_On
/* Check the TCON for validity. */
BE_EXPORTED extern void Check_TCON (TCON *tc);
#endif /* Is_True_On */
