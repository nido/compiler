...
#ifdef TARG_ST
// [SC] TLS models.  See Dreppers paper for details.
// These must be ordered by desirability, most desirable last.
// Beware, only two bits are allocated to represent this in a symbol.
// If you add more values here, the field will need to be widened.
enum ST_TLS_MODEL
{
  ST_TLS_MODEL_GLOBAL_DYNAMIC = 0,
  ST_TLS_MODEL_LOCAL_DYNAMIC  = 1,
...
enum ST_MEMORY_SPACE
{
  ...
  ST_MEMORY_SDA       = 3,
  ST_MEMORY_TDA       = 4,
  ST_MEMORY_GOTSMALL  = 5,
  ST_MEMORY_GOTSTD    = 6,
  ST_MEMORY_GOTLARGE  = 7
}; // ST_MEMORY_SPACE
#endif
#endif
...
/* (cbr) vague linkage support */
    ST_IS_COMDAT = 0x02,        //  C++ PU need will be generated by any
                                      // translation unit than needs it
    ST_HAS_ALIAS = 0x04,        // symbol is base of aliases
    ST_ALIAS_USED= 0x08,        // symbol is an alias and is referenced
    ST_IS_USED =   0x10,        // symbol has used attribute
    ST_IS_NOT_SIZED =   0x20       // symbol has not its size for the moment
}; // ST_FLAGS_EXT
...
public:
    ...
    BE_EXPORTED void Print(FILE *f, BOOL verbose = TRUE) const;

#ifdef TARG_ST
    // [CL] used for consistency checks in ipc_symtab_merge.cxx
    // be sure to update this function if you add fields
    BOOL operator== (const ST& st) const {
	return u1.name_idx == st.u1.name_idx
	  && u1.tcon == st.u1.tcon
	  && flags == st.flags
	  && flags_ext == st.flags_ext
	  && sym_class == st.sym_class
	  && storage_class == st.storage_class
	  && export_class == st.export_class
	  && tls_model == st.tls_model
	  && u2.type == st.u2.type
	  && u2.pu == st.u2.pu
	  && u2.blk == st.u2.blk
	  && offset == st.offset
...
#ifdef TARG_ST
    TY_IS_NOTHROW	= 0x0800	// (cbr) type doesn't throws */
#endif
};
...
BE_EXPORTED extern SCOPE		* Scope_tab;
...
BE_EXPORTED extern FILE_INFO	        File_info;
BE_EXPORTED extern PU_TAB		Pu_Table;
BE_EXPORTED extern SYMBOL_TABLE	St_Table;
BE_EXPORTED extern TY_TAB		Ty_tab;
BE_EXPORTED extern TYPE_TABLE	        Ty_Table;
BE_EXPORTED extern FLD_TAB		Fld_Table;
BE_EXPORTED extern TYLIST_TAB	        Tylist_Table;
BE_EXPORTED extern ARB_TAB		Arb_Table;
BE_EXPORTED extern STRING_TABLE        Str_Table;
BE_EXPORTED extern TCON_TAB		Tcon_Table;
BE_EXPORTED extern INITV_TAB	        Initv_Table;
BE_EXPORTED extern INITO_TABLE	        Inito_Table;
BE_EXPORTED extern PREG_TABLE	        Preg_Table;
BE_EXPORTED extern ST_ATTR_TABLE	St_Attr_Table;
BE_EXPORTED extern LABEL_TABLE	        Label_Table;
// some BLK_TAB items are really local, but make global to ease management
BE_EXPORTED extern BLK_TAB	        Blk_Table;

// global variables

BE_EXPORTED extern SYMTAB_IDX          Current_scope;
BE_EXPORTED extern PU*                 Current_pu;
