...
BE_EXPORTED extern BOOL WN_Simplifier_Enable(BOOL enable);

BE_EXPORTED extern WN *WN_SimplifyCvtl(OPCODE opc, INT16 cvtl_bits, WN *k0);

BE_EXPORTED extern WN *WN_SimplifyExp1(OPCODE opc, WN *k0);

BE_EXPORTED extern WN *WN_SimplifyExp2(OPCODE opc, WN *k0, WN *k1);

BE_EXPORTED extern WN *WN_SimplifyExp3(OPCODE opc, WN *k0, WN *k1, WN *k2);

BE_EXPORTED extern WN *WN_SimplifyIntrinsic(OPCODE opc, UINT32 intrinsic, INT32 n, WN *k[]);

BE_EXPORTED extern WN *WN_SimplifyIload(OPCODE opc, WN_OFFSET offset, 
			    TY_IDX ty, UINT field_id, TY_IDX load_addr_ty,
			    WN *addr);


BE_EXPORTED extern WN *WN_SimplifyIstore(OPCODE opc, WN_OFFSET offset, 
			    TY_IDX ty, UINT field_id, WN *value, WN *addr);

BE_EXPORTED extern INT32 WN_Simp_Compare_Trees(WN *t1, WN *t2);

struct ALIAS_MANAGER;  /* Needed as a forward reference */

BE_EXPORTED extern WN *WN_Simplify_Tree(WN *tree, ALIAS_MANAGER *am=NULL);

BE_EXPORTED extern WN *WN_Simplify_Rebuild_Expr_Tree(WN *tree, ALIAS_MANAGER *am=NULL);

BE_EXPORTED extern WN_MAP WN_SimpParentMap;

BE_EXPORTED extern BOOL WN_Simp_Fold_ILOAD;

BE_EXPORTED extern BOOL WN_Simp_Fold_LDA;
