...
extern WN *WN_LOOP_TripCount(const WN *loop)
{
  ...
  trip_cnt = WN_CreateExp2( OPCODE_make_op(OPR_DIV,trip_mtype,MTYPE_V),
			    trip_cnt, WN_COPY_Tree(incr) );
#ifdef TARG_ST
  WN *cmp_exp = WN_CreateExp2(ub_compare, WN_COPY_Tree(lb), WN_COPY_Tree(ub));

  if ((cmp_exp != NULL) && (WN_operator(cmp_exp) == OPR_INTCONST)) {
    if (!WN_const_val(cmp_exp)) {
      /* Loop is never executed. Raise a warning if not detected. */
      if ( trip_cnt != NULL ) {
	if ( WN_operator(trip_cnt) == OPR_INTCONST ) {
	  INT64 trip_count_val = WN_const_val(trip_cnt);
	  if ( trip_count_val > 0 ) {
	    //	    fprintf(stderr, "WARNING: Incorrect trip count computation. Fixed.\n");
	    trip_cnt = WN_CreateIntconst(OPR_INTCONST,trip_mtype,MTYPE_V,0);
	  }
	}
      }
    }
 ...
 *   It is the WN_ty() for STID|LDID
 *   It is the WN_ty() for direct loads/stores
 *   It is the type pointed to by WN_ty() for IST[ORE|BITS]/MSTORE/MLOAD.
 *   It is the type pointed to by WN_load_addr_ty() for IL[OAD|DBITS]
...
WN_object_ty (const WN *wn)
{
  ...
  if (!OPERATOR_is_load(opr) &&
      !OPERATOR_is_store(opr)) return (TY_IDX)0;
  ...
  if (OPERATOR_has_1ty(opr)) {
    ...
    pointed = TY_pointed (WN_load_addr_ty(wn));
  ...
  if (WN_field_id(wn) != 0 && TY_kind(pointed) == KIND_STRUCT)
    return field_type(pointed, WN_field_id(wn));
  return pointed;
