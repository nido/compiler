...
***
#ifdef TARG_ST
***	WN *WN_UVConstPointer(
***		TYPE_ID rtype,
***	)
***		Return the correct bit pattern for an uninitialized pointer
***
#endif
***	WN * WN_Zerocon (
***		TYPE_ID ty
...
BE_EXPORTED extern BOOL Types_Are_Compatible(TYPE_ID ltype, WN *wn);
BE_EXPORTED extern  BOOL IPO_Types_Are_Compatible(TYPE_ID ltype, TYPE_ID rtype);
BE_EXPORTED extern BOOL Is_Const_Parm (WN *, INT );     /* for a call call_wn, 
...
BE_EXPORTED extern void Dont_Use_WN_Free_List (void);
BE_EXPORTED extern MEM_POOL *WN_mem_pool_ptr;

BE_EXPORTED extern WN *WN_Create(OPERATOR opr, TYPE_ID rtype, TYPE_ID desc, mINT16 kid_count);
inline WN *
...
BE_EXPORTED extern void IPA_WN_Delete(WN_MAP_TAB *maptab, WN *wn);

#define WN_Delete(wn) \
  IPA_WN_Delete(Current_Map_Tab, (wn))

BE_EXPORTED extern void IPA_WN_DELETE_Tree(WN_MAP_TAB *maptab, WN *tree);

BE_EXPORTED extern void WN_Register_Delete_Cleanup_Function(void (*cleanup_fn)(WN *wn));

BE_EXPORTED extern void WN_Remove_Delete_Cleanup_Function(void (*cleanup_fn)(WN *wn));

BE_EXPORTED extern  BOOL WN_Equiv(WN *wn1, WN *wn2);

BE_EXPORTED extern WN *WN_CreateBlock(void);

BE_EXPORTED extern WN *WN_CreateDO(WN *index,
		       WN *start,
		       WN *end,
		       WN *step,
		       WN *body,
...
BE_EXPORTED extern WN *WN_CreateDoWhile(WN *test,
			  WN *body);

BE_EXPORTED extern WN *WN_CreateWhileDo(WN *test,
			  WN *body);

BE_EXPORTED extern WN *WN_CreateIf(WN *test,
		       WN *if_then,
		       WN *if_else);

BE_EXPORTED extern WN *WN_CreateEntry (INT16 nkids, ST_IDX name, WN *body, WN *pragmas,
			   WN *varrefs);

BE_EXPORTED extern WN *WN_CreateRegion(REGION_KIND kind,
			   WN *body, 
		    	   WN *pragmas,
			   WN *exits, 
			   INT region_id,
			   INITO_IDX ereg_supp);
BE_EXPORTED extern WN *WN_CreateRegionExit (INT32 label_number);

/* set maximum region id seen so far;
 * special case 0 to mean reset region ids to 0 (done at each PU) */
BE_EXPORTED extern void Set_Max_Region_Id (INT id);

BE_EXPORTED extern WN *WN_CreateGoto(INT32 label_number);

BE_EXPORTED extern WN *WN_CreateGotoOuterBlock (INT32 label_number, SYMTAB_IDX label_level);

BE_EXPORTED extern WN *WN_CreateAgoto(WN *addr);

BE_EXPORTED extern WN *WN_CreateAltentry(ST_IDX entry);

#define WN_CreateCondbr WN_CreateTruebr
BE_EXPORTED extern WN *WN_CreateTruebr(INT32 label_number, WN *exp);
BE_EXPORTED extern WN *WN_CreateFalsebr(INT32 label_number, WN *exp);

BE_EXPORTED extern WN *WN_CreateReturn(void);
BE_EXPORTED extern WN *WN_CreateReturn_Val (OPERATOR opr, TYPE_ID rtype, TYPE_ID desc, WN * val);
inline WN *
...
BE_EXPORTED extern WN *WN_CreateLabel(INT32 label_number, UINT32 label_flag, WN *loop_info);

BE_EXPORTED extern WN *WN_CreateCompgoto(INT32 num_entries,
		     	   WN *value, WN *block, WN *deflt, INT32 last_label);

BE_EXPORTED extern WN *WN_CreateSwitch(INT32 num_entries,
			   WN *value, WN *block, WN *deflt, INT32 last_label);

BE_EXPORTED extern WN *WN_CreateCasegoto(INT64 case_value, INT32 case_label_number);

BE_EXPORTED extern WN *WN_CreateXgoto (INT32 num_entries, WN *value, WN *block, ST_IDX st);

BE_EXPORTED extern WN *WN_CreateIstore(OPERATOR opr,
			  TYPE_ID rtype, 
			  TYPE_ID desc, 
			  WN_OFFSET offset, 
			  TY_IDX ty,
...
BE_EXPORTED extern WN *WN_CreateIstorex(OPERATOR opr, 
			  TYPE_ID rtype, 
			  TYPE_ID desc, 
		          TY_IDX ty,
			  WN *value,
...
BE_EXPORTED extern WN *WN_CreateMstore(WN_OFFSET offset,
			   TY_IDX ty,
			   WN *value,
			   WN *addr,
			   WN *num_bytes);

BE_EXPORTED extern WN *WN_CreateStid(OPERATOR opr,
			 TYPE_ID rtype, 
			 TYPE_ID desc, 
			 WN_OFFSET offset, 
			 ST *st, 
...
BE_EXPORTED extern WN *WN_CreatePrefetch( WN_OFFSET offset,  UINT32 flag, WN *addr);
BE_EXPORTED extern WN *WN_CreatePrefetchx(UINT32 flag, WN *addr1, WN *addr2);

BE_EXPORTED extern WN *WN_CreateIo(IOSTATEMENT iostatement, mINT16 kid_count);
BE_EXPORTED extern WN *WN_CreateIoItem0(IOITEM ioitem, TY_IDX ty);
BE_EXPORTED extern WN *WN_CreateIoItem1(IOITEM ioitem, WN *kid0, TY_IDX ty);
BE_EXPORTED extern WN *WN_CreateIoItem2(IOITEM ioitem, WN *kid0, WN *kid1, TY_IDX ty);
BE_EXPORTED extern WN *WN_CreateIoItem3(IOITEM ioitem, WN *kid0, WN *kid1, WN *kid2, TY_IDX ty);
BE_EXPORTED extern WN *WN_CreateIoItemN(IOITEM ioitem, mINT16 kid_count, TY_IDX ty);

BE_EXPORTED extern WN *WN_CreateEval(WN *exp);

BE_EXPORTED extern WN *WN_CreatePragma(WN_PRAGMA_ID pragma_name, ST_IDX st, 
			   INT32 arg1, INT32 arg2);
BE_EXPORTED extern WN *WN_CreatePragma(WN_PRAGMA_ID pragma_name, ST_IDX st, 
			   INT32 arg1, PREG_NUM asm_copyout_preg,
			   UINT32 asm_opnd_num);
BE_EXPORTED extern WN *WN_CreateXpragma(WN_PRAGMA_ID pragma_name, ST_IDX st, 
			    INT16 kid_count);

BE_EXPORTED extern WN *WN_CreateExp0(OPERATOR opr, TYPE_ID rtype, TYPE_ID desc);
inline WN *
...
BE_EXPORTED extern WN *WN_CreateExp1(OPERATOR opr,
			 TYPE_ID rtype,
			 TYPE_ID desc,
			 WN *kid0);
inline WN *
...
BE_EXPORTED extern WN *WN_CreateExp2(OPERATOR opr,
			 TYPE_ID rtype,
			 TYPE_ID desc,
			 WN *kid0,
			 WN *kid1);
...
BE_EXPORTED extern WN *WN_CreateExp3(OPERATOR opr,
			 TYPE_ID rtype,
			 TYPE_ID desc,
			 WN *kid0,
			 WN *kid1,
...
BE_EXPORTED extern WN *WN_CreateIload(OPERATOR opr,
			 TYPE_ID rtype,
			 TYPE_ID desc,
			 WN_OFFSET offset, 
			 TY_IDX ty,
...
BE_EXPORTED extern WN *WN_CreateIloadx(OPERATOR opr, 
			 TYPE_ID rtype,
			 TYPE_ID desc,
			 TY_IDX ty,
			 TY_IDX load_addr_ty,
...
BE_EXPORTED extern WN *WN_CreateMload(WN_OFFSET offset, 
			  TY_IDX ty,
			  WN *addr,
			  WN *num_bytes);

BE_EXPORTED extern WN *WN_CreateLdid(OPERATOR opr,
			 TYPE_ID rtype,
			 TYPE_ID desc,
			 WN_OFFSET offset,
			 ST_IDX st,
...
BE_EXPORTED extern WN *WN_CreateLda(OPERATOR opr, 
			TYPE_ID rtype,
			TYPE_ID desc,
			WN_OFFSET offset,
			TY_IDX ty,
...
BE_EXPORTED extern WN *WN_CreateIlda(OPERATOR opr, 
			 TYPE_ID rtype,
			 TYPE_ID desc,
			 WN_OFFSET offset,
			 TY_IDX ty);

BE_EXPORTED extern WN *WN_CreateLdaLabel(OPERATOR opr,
			     TYPE_ID rtype,
			     TYPE_ID desc,
			     mUINT32 label_number);

BE_EXPORTED extern WN *WN_CreateIdname(WN_OFFSET offset,
			   ST_IDX st);

BE_EXPORTED extern WN *WN_CreateConst (OPERATOR opr, TYPE_ID rtype, TYPE_ID desc, ST_IDX st);
inline WN*
...
BE_EXPORTED extern WN *WN_CreateIntconst(OPERATOR opr, 
			     TYPE_ID rtype,
			     TYPE_ID desc,
			     INT64 const_val);
inline WN *
...
BE_EXPORTED extern WN *WN_CreateCvtl(OPERATOR opr,
			 TYPE_ID rtype,
			 TYPE_ID desc,
			 INT16 cvtl_bits,
			 WN *kid0);
...
BE_EXPORTED extern WN *WN_Create_Intrinsic(OPERATOR opr,
			       TYPE_ID rtype,
			       TYPE_ID desc,
			       INTRINSIC intrinsic,
			       INT32 n,
...
BE_EXPORTED extern WN *WN_CreateParm(TYPE_ID rtype,
			 WN *parm_node,
			 TY_IDX ty, 
			 UINT32 flag);
...
BE_EXPORTED extern WN *WN_CreateRcomma(OPERATOR opr, TYPE_ID rtype, TYPE_ID desc, WN *value, WN *block);
inline WN *
...
BE_EXPORTED extern WN *WN_CreateComment (char *s);	/* create comment node */
BE_EXPORTED extern STR_IDX WN_GetComment (const WN *wn);  /* get string idx from comment node */

BE_EXPORTED extern WN *WN_CreateAsm_Stmt (INT16 kid_count, char *asm_string);

BE_EXPORTED extern WN *WN_CreateAsm_Input (char *constraint, UINT32 opnd_num, WN *opnd_expr);

BE_EXPORTED extern WN *WN_CopyNode(const WN* src_wn);


#if 0
#define WN_Move_Maps(dst,src) \
  IPA_WN_Move_Maps(Current_Map_Tab, (dst), (src))
BE_EXPORTED extern void IPA_WN_Move_Maps(WN_MAP_TAB *maptab, WN *dst, WN *src);
#endif

BE_EXPORTED extern void IPA_WN_Move_Maps_PU(WN_MAP_TAB *src, WN_MAP_TAB *dst, WN *wn);


BE_EXPORTED extern void WN_Mem_Push(void);

BE_EXPORTED extern void WN_Mem_Pop(void);

BE_EXPORTED extern WN *WN_Ldid(TYPE_ID desc,
		   WN_OFFSET offset,
		   ST_IDX sym,
		   TY_IDX align,
		   UINT field_id = 0);

BE_EXPORTED extern WN *WN_RLdid(TYPE_ID rtype,
		    TYPE_ID desc,
		    WN_OFFSET offset,
		    ST_IDX sym,
		    TY_IDX align);

BE_EXPORTED extern WN *WN_LdidPreg(TYPE_ID desc,
		       WN_OFFSET pregno);

BE_EXPORTED extern WN *WN_Iload(TYPE_ID desc,
		    WN_OFFSET offset,
		    TY_IDX align,
		    WN *addr,
		    UINT field_id = 0);

BE_EXPORTED extern WN *WN_RIload(TYPE_ID rtype,
		     TYPE_ID desc,
		     WN_OFFSET offset,
		     TY_IDX align,
		     WN *addr);

BE_EXPORTED extern WN *WN_IloadLdid(TYPE_ID desc,
			WN_OFFSET offset,
			TY_IDX align,
			ST *sym,
			WN_OFFSET symOffset);

BE_EXPORTED extern WN *WN_Istore(TYPE_ID desc,
		     WN_OFFSET offset,
		     TY_IDX align,
		     WN *addr,
		     WN *value,
...
BE_EXPORTED extern WN *WN_Unary(OPERATOR opr,
		    TYPE_ID rtype,
		    WN *l);

BE_EXPORTED extern WN *WN_Binary(OPERATOR opr,
		     TYPE_ID rtype,
		     WN *l,
		     WN *r);

BE_EXPORTED extern WN *WN_Ternary(OPERATOR opr,
		      TYPE_ID rtype,
		      WN *kid0,
		      WN *kid1,
		      WN *kid2);

BE_EXPORTED extern WN *WN_Stid(TYPE_ID desc,
		   WN_OFFSET offset,
		   ST *sym,
		   TY_IDX align,
		   WN *value,
...
BE_EXPORTED extern WN *WN_StidIntoPreg(TYPE_ID desc,
			   WN_OFFSET offset,
			   ST *sym,
			   WN *value);
...
BE_EXPORTED extern WN *WN_Intconst(TYPE_ID rtype,
		       INT64 value);

BE_EXPORTED extern WN *WN_RotateIntconst(WN *tree,
			     INT32 rotate);

BE_EXPORTED extern WN *WN_Inverse(TYPE_ID type,
		      WN *tree);

BE_EXPORTED extern WN *WN_Floatconst(TYPE_ID type,
			 double value);
...
#endif
BE_EXPORTED extern WN *WN_UVConst(TYPE_ID type);

BE_EXPORTED extern WN * WN_Zerocon (TYPE_ID ty);

BE_EXPORTED extern WN *WN_Cvt(TYPE_ID desc,
		  TYPE_ID rtype,
		  WN *kid0);

BE_EXPORTED extern WN *WN_Trunc(TYPE_ID desc,
		    TYPE_ID rtype,
		    WN *kid0);

BE_EXPORTED extern WN *WN_Rnd(TYPE_ID desc,
		  TYPE_ID rtype,
		  WN *kid0);

BE_EXPORTED extern WN *WN_Ceil(TYPE_ID desc,
		   TYPE_ID rtype,
		   WN *kid0);

BE_EXPORTED extern WN *WN_Floor(TYPE_ID desc,
		    TYPE_ID rtype,
		    WN *kid0);

BE_EXPORTED extern WN *WN_Relational(OPERATOR opr,
			 TYPE_ID desc,
			 WN *kid0,
			 WN *kid1);

BE_EXPORTED extern WN *WN_ConstPowerOf2(TYPE_ID rtype,
			    INT32 n);

BE_EXPORTED extern WN *WN_Lda(TYPE_ID rtype,
		  WN_OFFSET ldaOffset,
		  ST *sym,
		  UINT field_id = 0);


BE_EXPORTED extern WN *WN_LdaString(char *str,
			WN_OFFSET ldaOffset,
			INT32 len);

BE_EXPORTED extern WN *WN_LdaLabel(TYPE_ID rtype,
                       INT32   label_number);

BE_EXPORTED extern WN *WN_Icall(TYPE_ID rtype,
		    TYPE_ID desc,
		    INT32 n,
		    TY_IDX ty);

BE_EXPORTED extern WN *WN_generic_call(OPERATOR opr,
			   TYPE_ID rtype,
			   TYPE_ID desc,
			   INT32 n,
			   ST_IDX sym);

BE_EXPORTED extern WN *WN_generic_intrinsic(OPERATOR opr,
				TYPE_ID rtype,
				TYPE_ID desc,
				INT32 n,
				INTRINSIC intrinsic);
...
BE_EXPORTED extern WN *WN_CreateLoopInfo (WN *induction, WN *trip, UINT16 trip_est, UINT16 depth, INT32 flags);

BE_EXPORTED extern WN *WN_CreateExcScopeBegin(INT32 id, INT16 nkids, struct inito* ereg_supp);

BE_EXPORTED extern WN *WN_CreateExcScopeEnd(INT32 id);

BE_EXPORTED extern WN *WN_CreateBarrier (BOOL forward, INT16 nkids);

BE_EXPORTED extern WN *WN_CreateTrap (INT32 value);

BE_EXPORTED extern WN *WN_CreateAssert (INT32 value, WN *condition);

BE_EXPORTED extern void WN_CopyMap( WN	    *dst,
			WN_MAP	    map,
			const WN    *src);

BE_EXPORTED extern WN *WN_Tas(TYPE_ID rtype,
		  TY_IDX ty,
		  WN *l);

BE_EXPORTED extern WN *WN_Iloadx(TYPE_ID rtype,
		     TY_IDX ty,
		     TY_IDX addr_ty,
		     WN *base,
		     WN *index);

BE_EXPORTED extern WN *WN_Istorex(TYPE_ID desc,
		      TY_IDX ty,
		      WN *value,
		      WN *base,
		      WN *index);
...
BE_EXPORTED extern WN * WN_Int_Type_Conversion( WN *wn, TYPE_ID to_type );
BE_EXPORTED extern WN * WN_Float_Type_Conversion( WN *wn, TYPE_ID to_type );
BE_EXPORTED extern WN * WN_Type_Conversion( WN *wn, TYPE_ID to_type );

BE_EXPORTED extern INT32 WN_Size_and_StartAddress (WN *wn, void **StartAddress);

BE_EXPORTED extern BOOL WN_verifier(WN *);
...
BE_EXPORTED extern BOOL WN_Tree_Has_Duplicate_Labels(
       WN *pu_wn,
       MEM_POOL *tmp_pool
);
...
BE_EXPORTED extern BOOL WN_Rename_Duplicate_Labels (
       WN *orig_wn,
       WN *copied_wn,
       WN *pu_wn,
       MEM_POOL *tmp_pool
...
BE_EXPORTED extern WN* WN_CreateAffirm (WN* condition);
BE_EXPORTED extern WN* WN_CreateAlloca (WN* size);
BE_EXPORTED extern WN* WN_CreateDealloca (INT32 n);
BE_EXPORTED extern WN* WN_CreateLdma (TYPE_ID rtype, WN_OFFSET offset, TY_IDX ty, ST_IDX st);

BE_EXPORTED extern void WN_set_st_addr_saved (WN *);
