...
BE_EXPORTED char *Whirl_Revision = WHIRL_REVISION;
...
#if !defined(MAP_AUTOGROW)
#ifdef TARG_ST
    /* [CM] (MBTst16964, MBTst16965) 
       Under Win32, the to-be-mmaped file must be resized before being mmaped.
       Doing it the original way (eg mmap before ftruncate) looks harmless also 
       under other OSes (Solaris and Linux).
       However, this solution requires no distinction between platforms, so I
       think is preferable.
    */
    ...
    if (getenv("MMAP_DEBUG"))
      fprintf(stderr, " ** ir_b_grow_map: size:%ld\n", (long)fl->mapped_size);
#endif
...
#endif
#else 
    /* using IRIX MAP_AUTOGROW feature */
    fl->map_addr = (char *) mmap (0, fl->mapped_size, PROT_READ|PROT_WRITE,
...
#if !defined(MAP_AUTOGROW)
    ftruncate(fl->output_fd, fl->mapped_size);
#ifdef Is_True_On
    if (getenv("MMAP_DEBUG"))
      fprintf(stderr, " ** ir_b_create_map: size:%ld\n", (long)fl->mapped_size);
#endif
    fl->map_addr = (char *) mmap (0, fl->mapped_size, PROT_READ|PROT_WRITE,
				  MAP_SHARED, fd, 0); 
#else
