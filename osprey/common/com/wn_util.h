...
BE_EXPORTED extern WN_ITER* WN_WALK_TreeIter(
       WN*
);

BE_EXPORTED extern WN_ITER* WN_WALK_TreeNext(
       WN_ITER*
);

BE_EXPORTED extern WN_ITER* WN_WALK_StmtIter(
       WN*
);

BE_EXPORTED extern WN_ITER* WN_WALK_StmtNext(
       WN_ITER*
);

BE_EXPORTED extern WN_ITER* WN_WALK_SCFIter(
       WN *wn
);

BE_EXPORTED extern WN_ITER* WN_WALK_SCFNext(
       WN_ITER*
);

BE_EXPORTED extern void WN_WALK_Abort(
       WN_ITER*
);

BE_EXPORTED extern void WN_INSERT_BlockBefore(
       WN* b, 
       WN* wn,
       WN* in
);
...
BE_EXPORTED extern void WN_INSERT_BlockAfter(
       WN* b, 
       WN* wn, 
       WN* in
);
...
BE_EXPORTED extern WN* WN_COPY_Tree(
       WN *tree_node
);

BE_EXPORTED extern WN* WN_COPY_Tree_With_Map(
       WN *tree_node
);

BE_EXPORTED extern void WN_COPY_All_Maps(
       WN *dst,
       WN *src
);

BE_EXPORTED extern void WN_DELETE_Tree(
       WN* tree
);

BE_EXPORTED extern void WN_DELETE_FromBlock(
       WN *blck,
       WN *wn
);

BE_EXPORTED WN *WN_EXTRACT_FromBlock(
       WN *parent,
       WN *item
);

BE_EXPORTED WN *WN_EXTRACT_ItemsFromBlock(
	WN *parent, 
	WN *first_item, 
	WN *last_item
);

BE_EXPORTED WN *WN_LOOP_LowerBound(
	const WN *loop
);

BE_EXPORTED WN *WN_LOOP_UpperBound(
	const WN *loop,
	OPCODE *compare
);

BE_EXPORTED WN *WN_LOOP_Increment(
	const WN *loop,
	BOOL *is_incr
);

BE_EXPORTED WN *WN_LOOP_TripCount(
	const WN *loop
);

BE_EXPORTED WN *WN_LOOP_InductionVariable(
	const WN *loop
);

BE_EXPORTED UINT32 OPR_INTCONST_get_align(INT64 val);
BE_EXPORTED UINT32 OPR_ADD_get_align(UINT32 align1, UINT32 align2);
BE_EXPORTED UINT32 OPR_MPY_get_align(UINT32 align1, UINT32 align2);
BE_EXPORTED UINT32 WN_get_align (WN *wn);

/*  Other WN utilities */

/* Obtain the high-level type of the item accessed */
BE_EXPORTED extern TY_IDX WN_object_ty(const WN *);

/* Obtain the size of object being accessed */
BE_EXPORTED extern INT32 WN_object_size(const WN *);
...
extern "C" BE_EXPORTED void Add_Pragma_To_MP_Regions (WN_VECTOR *wnv, 
                                          WN_PRAGMA_ID pragma_id,
                                          ST *st, WN_OFFSET ofst,
                                          WN_MAP parent_map,
                                          BOOL make_compiler_generated);
