...
/*       fprintf(stdout,"gcov_open: switch to unbuffered writing\n"); */
  ...
  /*  setbuf (gcov_var.file, (char *)0); */
/*   fprintf(stdout,"gcov_open mode %d\n",gcov_var.mode); */
...
gcov_allocate (unsigned length)
{
  ...
  gcov_var.buffer = (gcov_unsigned_t *)realloc (gcov_var.buffer, new_size << 2);
}
#endif
...
{
/*   fprintf(stdout,"gcov_write_block: fwrite\n"); */
...
gcov_write_words (unsigned words)
{
  ...
  GCOV_CHECK_WRITING ();
#if IN_LIBGCOV
/*   fprintf(stdout,"gcov_write_words start offset %d\n",gcov_var.offset); */
...
/*   fprintf(stdout,"gcov_write_words end offset %d\n",gcov_var.offset); */
  return result;
}
...
{
/*   fprintf(stdout,"gcov_write_unsigned pos %d, value:%d\n",gcov_var.offset,value); */
...
{
/*   fprintf(stdout,"gcov_write_counter pos %d, value:%lld\n",gcov_var.offset,value); */
...
{
/*   fprintf(stdout,"gcov_write_tag_length pos %d, value:%x length:%d \n",gcov_var.offset,tag, length); */
  gcov_unsigned_t *buffer = gcov_write_words (2);
...
#endif
/*       fprintf(stdout,"gcov_read_words: fread\n"); */
...
{
  GCOV_CHECK_WRITING ();
/*   fprintf(stdout,"gcov_seek: base:%d current offst %d, current start %d\n",base, gcov_var.offset, gcov_var.start); */
  /*  if (gcov_var.offset)
    gcov_write_block (gcov_var.offset);
  fseek (gcov_var.file, base << 2, base ? SEEK_SET : SEEK_END);
  gcov_var.start = ftell (gcov_var.file) >> 2;
  */
/*   fprintf(stdout,"gcov_seek end: base:%d current offst %d, current start %d\n",base, gcov_var.offset, gcov_var.start); */
}
#endif
