...
// [HK]
typedef __GNUEXT::hash_map<TY_POINTEE_KEY, TY_IDX, hash_pointee_key>
    TY_IDX_POINTER_MAP;
...
// [HK]
typedef __GNUEXT::hash_map<TY_IDX, TY_IDX, TY_hash, TY_EQUIV> HASH_TY_TABLE;
...
Setup_Preg_Pointers ()
{
  ...
  // Arthur:
  // This all should be defined in the target dependent part.
  // For now just use the #ifdef:
  if (ST_type(Int64_Preg) == Spill_Int_Type)
    Int_Preg = Int64_Preg;
  else
#ifdef ARTHUR /*TARG_ST100 */
    Int_Preg = Int40_Preg;
#else
    Int_Preg = Int32_Preg;
#endif
 ...
 *
 *   for phases other then the front end, we read in the global symtab 
 *   from file, so we don't create the preg sybmols.  But we still need 
 *   to intialize the MTYPE_TO_PREG_array.
 * ====================================================================
 */
static void
Set_up_all_preg_symbols ()
...
Create_Special_Global_Symbols ()
{
    ...
    TYPE_ID set_pointed = MTYPE_UNKNOWN;
  ...
  if (set_pointed == MTYPE_A4) {
    ty_idx = MTYPE_To_TY (MTYPE_A4);
    Set_TY_pointed (Ty_Table[ty_idx], MTYPE_To_TY (MTYPE_V));
  }
  else if (set_pointed == MTYPE_A8) {
    ty_idx = MTYPE_To_TY (MTYPE_A8);
    Set_TY_pointed (Ty_Table[ty_idx], MTYPE_To_TY (MTYPE_V));
  }
 ...
 *
 *   In phases other then the frontends, we read in the global symtab 
 *   from the file, and that includes all the predefined global symbols 
 *   and types.  We don't need to create them again, but we still need 
 *   to initialize the MTYPE_TO_TY array.
 * ====================================================================
 */
void
Initialize_Special_Global_Symbols ()
