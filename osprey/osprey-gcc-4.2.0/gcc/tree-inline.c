...
/* Data passed to visit functions in duplidate_tree. */

typedef struct dup_data
{
  splay_tree dup_map;
...
static void
add_translation (dup_data *id, tree orig_t, tree new_t)
{
  splay_tree_insert (id->dup_map, (splay_tree_key) orig_t,
		     (splay_tree_value) new_t);
}

static tree
get_translation (dup_data *id, tree orig_t)
{
  splay_tree_node n = splay_tree_lookup (id->dup_map, (splay_tree_key) orig_t);
  return n ? (tree)n->value : 0;
}

/* Visit function for first pass of duplicate_tree */

static tree
make_duplicates_r (tree *tp, int *walk_subtrees ATTRIBUTE_UNUSED,
		   void *data)
{
  dup_data *id = (dup_data *)data;
  int do_not_walk_subtrees = 0;
  tree orig_t = *tp;
  tree new_t = orig_t;
  /*   printf ("make_duplicates_r visiting %s (%p)\n",
       tree_code_name[(int) TREE_CODE (orig_t)], orig_t); */
  /* copy_tree_r decides whether it is necessary to make a copy
     of this node: many nodes do not need to be copied, e.g.
     types, decls, constants. */
  /* For the interesting cases of TARGET_EXPR/SAVE_EXPR which must
     be shared by multiple parents, note that we copy such nodes
     exactly once here.  Later we will redirect all pointers to the
     original to that one copy, so the shared-ness is correct
     in the duplicate tree, but it is not shared at all with the
     original tree. */
  switch (TREE_CODE (orig_t))
    {
    case STATEMENT_LIST:
      copy_statement_list (&new_t);
      break;
    case BIND_EXPR:
      copy_tree_r (&new_t, & do_not_walk_subtrees, data);
      /* copy_tree_r does not set the BIND_EXPR_BLOCK, so do it
	 here */
      BIND_EXPR_BLOCK (new_t) = BIND_EXPR_BLOCK (orig_t);
      break;
    default:
      copy_tree_r (&new_t, & do_not_walk_subtrees, data);
      break;
  ...
  if (new_t != orig_t)
    {
      gcc_assert (! get_translation (id, orig_t));
      add_translation (id, orig_t, new_t);
    }
  /* We _do_ need to go into subtrees, even if we do not duplicate
     this node, since we may still find subtrees that need to be
     duplicated.  The canonical unpleasant example of this is
     a type tree for a VLA: the dimension can be a SAVE_EXPR which
     must be copied, even though the parent type node is not copied.
     Furthermore, we must force a walk into type trees, since
     walk_tree does not do it by default.
  */
  switch (TREE_CODE (orig_t))
    {
    case VAR_DECL:
    case TYPE_DECL:
      walk_tree (&TREE_TYPE(orig_t), make_duplicates_r, data, id->pset);
      break;
    case ARRAY_TYPE:
      walk_tree (&TYPE_SIZE(orig_t), make_duplicates_r, data, id->pset);
      walk_tree (&TYPE_SIZE_UNIT(orig_t), make_duplicates_r, data, id->pset);
      break;
    default:
      break;
    }
...
/* Visit function for second pass of duplicate_tree */

static tree
map_to_duplicates_r (tree *tp, int *walk_subtrees ATTRIBUTE_UNUSED,
		     void *data)
{
  dup_data *id = (dup_data *) data;
  tree orig_t = *tp;
  /* printf ("map_to_duplicates_r visiting %s (%p)\n",
     tree_code_name[(int) TREE_CODE (orig_t)], orig_t); */
  tree new_t = get_translation (id, orig_t);
  if (new_t)
    *tp = new_t;
  switch (TREE_CODE (*tp))
    {
    case VAR_DECL:
    case TYPE_DECL:
      walk_tree_without_duplicates (&TREE_TYPE(*tp),
				    map_to_duplicates_r, data);
      break;
    case ARRAY_TYPE:
      walk_tree_without_duplicates (&TYPE_SIZE(*tp),
				    map_to_duplicates_r, data);
      walk_tree_without_duplicates (&TYPE_SIZE_UNIT(*tp),
				    map_to_duplicates_r, data);
      break;
    default:
      break;
...
/* Produce a new unique copy of a tree, suitable for applying
   tree transformations, without disturbing the original in
   any way. */

tree
duplicate_tree (tree t, void *dup_map)
{
  tree root = t;
  dup_data id;

  id.dup_map = (splay_tree) dup_map;
  id.pset = pointer_set_create ();

  /* First pass: we just create the duplicate nodes, without
     redirecting any existing nodes to point to them.
     Note that we use pset to avoid duplicates, so we will
     copy each node exactly once. */
  walk_tree (&root, make_duplicates_r, &id, id.pset);

  pointer_set_destroy (id.pset);
  id.pset = NULL;

  /* Second pass: redirect all pointers in the new nodes to new nodes. */
  /* No real need for a tree-walk here, we could just iterate over
     the child links in all the newly created nodes; however, there is no
     convenient way to do that. */
  /* Cannot use walk_tree_without_duplicates here, since we want to
     visit pointers to nodes, and there can be multiple pointers to the
     same node, and walk_tree_without_duplicates will only visit the
     first of these multiple pointers. */
  walk_tree (&root, map_to_duplicates_r, &id, NULL);

  return root;
}

/* Visit function for inverting a dup_map. */

static int
invert_map_r (splay_tree_node n, void *data)
{
  splay_tree *inverse_dup_map = (splay_tree *) data;

  splay_tree_key new_key = (splay_tree_key) n->value;
  splay_tree_value new_value = (splay_tree_value) n->key;
  splay_tree_insert (*inverse_dup_map, new_key, new_value);
  return 0;
}

/* repair the side-effects that duplicate_tree had on the original
   tree. */

tree
repair_duplicate_side_effects (tree t, void *dup_map)
{
  tree root = t;
  dup_data id;
  splay_tree orig_dup_map = (splay_tree) dup_map;
  splay_tree inv_dup_map = splay_tree_new (splay_tree_compare_pointers,
					   NULL, NULL);
  splay_tree_foreach (orig_dup_map, invert_map_r, &inv_dup_map);

  id.dup_map = inv_dup_map;
  walk_tree (&root, map_to_duplicates_r, &id, NULL);

  splay_tree_delete (inv_dup_map);
  return root;
}
#endif
