...
/* WFE pragmas are defined in wgen_pragmas.def, refer to this file
   for documentation.
*/
...
/* Pragma parsing state. */
static enum cpp_ttype pragma_last_lex;
static tree pragma_last_tree;
static enum cpp_ttype
get_pragma_lex(void)
{
  if (pragma_last_lex == 0)
    pragma_last_lex = pragma_lex(&pragma_last_tree);
  return pragma_last_lex;
}

static int
match_pragma_lex(enum cpp_ttype lex)
{
  return get_pragma_lex() == lex;
}

static int
parse_pragma_lex(enum cpp_ttype lex)
{
  int res = match_pragma_lex(lex);
...
static int
match_pragma_arg(const char * ARG_UNUSED (name), char arg)
{
  switch (arg)
    {
    case 'I':
      if (!match_pragma_lex(CPP_NUMBER))
	return 0;
      break;
    case 'A':
      if (!match_pragma_lex(CPP_NAME))
	return 0;
      break;
    case 'S':
      if (!match_pragma_lex(CPP_STRING))
	return 0;
      break;
    }
  return 1;
...
static int
parse_pragma_arg(tree *list, const char *name, char arg)
{
  if (match_pragma_arg(name, arg))
    {
      if (*list == NULL)
	*list = build_tree_list(NULL_TREE, pragma_last_tree);
      else
	*list = chainon(*list,build_tree_list(NULL_TREE, pragma_last_tree));
      pragma_last_lex = 0;
      pragma_last_tree = 0;
      return 1;
    }
...
static int
parse_pragma_args(tree *args, const char *name, const char *args_spec)
{
  const char *p = args_spec;
  tree list = NULL;

  pragma_last_lex = 0;
  pragma_last_tree = NULL;
  while (*p)
    {
      switch (*p) {
      case '(':
	if (!parse_pragma_lex(CPP_OPEN_PAREN))
	  return 0;
	break;
      case ')':
	if (!parse_pragma_lex(CPP_CLOSE_PAREN))
	  return 0;
	break;
      case ',':
	if (!parse_pragma_lex(CPP_COMMA))
	  return 0;
	break;
      case 'I':
      case 'A':
	...
	if (*(p+1) == '?')
	  {
	    if (match_pragma_arg(name, *p))
	      {
		if (!parse_pragma_arg(&list, name, *p))
		  return 0;
	      }
	    p++;
	  }
	else if (*(p+1) == '*')
	  {
		...
		if (!parse_pragma_arg(&list, name, *p))
		  return 0;
		while (match_pragma_lex(CPP_COMMA))
		  {
		    if (!parse_pragma_lex(CPP_COMMA))
		      return 0;
		    if (!parse_pragma_arg(&list, name, *p))
	...
	else if (!parse_pragma_arg(&list, name, *p))
	  return 0;
	break;
      default:
	internal_error ("unexpected token %c in pragma %s specification: %s", 
			*p, name, args_spec);
      }
      p++;
    }
  
  if (!parse_pragma_lex (CPP_EOF))
    return 0;
...
static void
handle_WGEN_PRAGMA_IDENT (cpp_reader * ARG_UNUSED (dummy))
{
  tree prefix, x;
  enum cpp_ttype t;

  if (pragma_lex (&prefix) != CPP_STRING)
    GCC_BAD ("malformed #pragma ident, ignored");
  t = pragma_lex (&x);
  if (t != CPP_EOF)
    warning (OPT_Wpragmas, "junk at end of %<#pragma ident%>");
  
  if (flag_spin_file)
    gspin_emit_ident (TREE_STRING_POINTER (prefix), 1);
}

static GTY(()) tree wfe_pragma_list;

tree
wfe_pragma_context (void)
{
  return wfe_pragma_list;
}

static void
add_file_wfe_pragma (tree pragma_stmt)
{
  SET_EXPR_LOCATION (pragma_stmt, input_location);
  wfe_pragma_list = tree_cons (NULL, pragma_stmt, wfe_pragma_list);
}

/*
 * Handle expansion of special handling function for each pragma.
 */
#define DECL_WGEN_PRAGMA(id,name,in_code,wn_id,nargs,args_spec,usage)	\
  static void								\
  handle_##id (cpp_reader * ARG_UNUSED (dummy))				\
  {									\
    tree pragma_args;							\
    if (in_code && current_function_decl == NULL)			\
      {									\
	warning (OPT_Wpragmas, "pragma not expected out of function scope at '#pragma %s' - ignored", name); \
	return;								\
      }									\
    if (!in_code && current_function_decl != NULL)			\
      {									\
	warning (OPT_Wpragmas, "pragma not expected in function scope at '#pragma %s' - ignored", name); \
	return;								\
      }									\
    if (parse_pragma_args (&pragma_args, name, args_spec))		\
      {									\
	tree pragma_stmt;						\
	pragma_stmt = build_nt (PRAGMA_STMT,				\
				build_int_cst (0, (in_code << 16) | id),\
                                pragma_args);				\
	if (in_code)							\
	  add_stmt (pragma_stmt);					\
	else  								\
	  add_file_wfe_pragma (pragma_stmt);				\
      }									\
    else								\
      warning (OPT_Wpragmas, "malformed pragma - ignored. Usage is: %s", usage); \
  }
#include "wgen_pragmas.def"
#undef DECL_WGEN_PRAGMA
...
#ifdef TARG_ST /* HANDLE_WFE_PRAGMAS */
  c_register_pragma (0, "ident", handle_WGEN_PRAGMA_IDENT);

#define DECL_WGEN_PRAGMA(id,name,wn_scope,wn_id,nargs,args_spec,usage) \
  if (*name != '\0') \
    c_register_pragma (0, name, handle_##id);
