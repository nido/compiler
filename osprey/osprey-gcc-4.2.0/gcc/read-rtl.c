...
find_mode (const char *name, FILE *infile)
{
  ...
  for (i = 0; i < STATIC_COUNT_MACHINE_MODE; i++)
...
mode_attr_index (struct map_value **mode_maps, const char *string)
{
  ...
  *mode_maps = mv;
  ...
  /* We return a code which we can map back into this string: the
     ...
     we just used.  */
  return STATIC_COUNT_MACHINE_MODE + htab_elements (modes.macros) + mv->number;
}
		 ...
		 int value, FILE *infile, const char **unknown)
{
  ...
  offset = STATIC_COUNT_MACHINE_MODE + htab_elements (modes.macros);
...
initialize_macros (void)
{
  ...
  modes.num_builtins = STATIC_COUNT_MACHINE_MODE;
