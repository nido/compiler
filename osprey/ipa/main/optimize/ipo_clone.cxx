...
IPO_Clone (IPA_NODE* orig_node, IPA_NODE* clone_node)
{
  ...
  //TB: fix up whirl feedback in IPO_Clone + IPA frequencies for the
  //node and the clone
#ifdef KEY
  if (orig_node->Has_Feedback () && clone_node->Has_Feedback ())
  {
    // Create whirl feedback for the clome
...
#endif
#ifdef TARG_ST
      //TB: Update SUMMARY_FEEDBACK info for clone and orig
      SUMMARY_FEEDBACK* sfb = clone_node->Get_feedback ();
      sfb->Set_frequency_count (clone_freq);
      sfb = orig_node->Get_feedback ();
      sfb->Set_frequency_count (orig_freq - clone_freq);
#endif
#ifdef KEY
      // Update whirl frequencies for node and clone
