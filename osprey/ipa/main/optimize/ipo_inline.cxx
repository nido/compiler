...
/* Was static but needed as an exportation */
#if !defined(TARG_ST)
static
#endif
...
Create_array(WN* base)
{
  ...
  for (i=0; i< ndims; ++i) {  
	...
	((TY_AR_lbnd_var(ty, i) == (INT)0) ||
	 (TY_AR_ubnd_var(ty, i) == (INT)0))
	  {
...
#ifdef TARG_ST
	    // [CG] The return type is available by the symbol
	    // It is unsafe to get the type from the child of
	    // the RETURN_VAL node. Indeed we are not guaranteed that
	    // the whirl node has a type information (for instance CSELECT, COMMA,...).
	    TY_IDX stid_ty = ST_type(tmp_st);
	    FmtAssert(stid_ty != 0, ("Undefined return type for inlined function"));
#else
	    TY_IDX stid_ty = WN_ty(WN_kid0(wn));
#endif
#if (defined(_STANDALONE_INLINER) || defined(_LIGHTWEIGHT_INLINER))
	    if (stid_ty == (INT)0) { // This is potentially a COMMA node
...
#ifdef TARG_ST
	  // Arthur: shouldn't we rather make a STID of the same
	  //         type as RETURN_VAL ?
	  iter.Insert (WN_StidIntoPreg (WN_rtype(wn),
...
IPO_INLINE::Process_Formals (IPO_INLINE_AUX& aux)
{
	...
	ste = Symtab()->Get_ST(ste);	// Get the ST version in the caller side
	std::pair<ST*, WN_OFFSET> sym = Create_Copy_In_Symbol (ste); 
...

{
...
#ifdef TARG_ST
	// [CL] record the callee's DST in the pragma, this is useful
	// when merging DSTs
	DST_TYPE dst = IP_FILE_HDR_dst(Callee_node()->File_Header());
	WN* pragma_node_begin =
	  WN_CreatePragma (WN_PRAGMA_INLINE_BODY_START, cp, lang, (INT32)dst);
#else
	WN* pragma_node_begin =
	...
	// when merging DSTs
	WN* pragma_node_end =
	  WN_CreatePragma (WN_PRAGMA_INLINE_BODY_END, cp, lang, (INT32)dst);

	// [CL] also record the appropriate line number for the 'end'
	// pragma
