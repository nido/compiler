...
#ifdef TARG_ST
		  // [CL] we have to take into account the sign of the field,
		  // so get it directly here through WN_rtype(wn) rather
		  // than derive it from WN_desc(wn)
		    iter.Replace (WN_CreateLdid (OPR_LDID, WN_rtype(wn), WN_desc (wn),
						 WN_offset (wn), actual,
...
{
    INT last = std::min (kid, cprop_annot.Lastidx () + 1);
