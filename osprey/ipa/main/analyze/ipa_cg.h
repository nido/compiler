...
#ifdef TARG_ST
  // [CL] command line option must= overrides pragmas
  void Set_Must_Inline_Attrib ()        {
    Set_PU_must_inline (Get_PU ());
    if (Has_Noinline_Attrib ()) {
      Clear_Noinline_Attrib ();
    }
  }
#else
...
#ifdef TARG_ST
  // [CL] command line option never= overrides pragmas
  void Set_Noinline_Attrib ()		{
    Set_PU_no_inline (Get_PU ());
    if (Has_Must_Inline_Attrib ()) {
      Clear_Must_Inline_Attrib ();
    }
  }
#else
