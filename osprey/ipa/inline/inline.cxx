...
#ifdef TARG_ST
  // [CL] Handle pragma noinline (just as in ipa_inline.cxx:do_inline() )
  if (edge.Summary_Callsite()->Is_no_inline()) {
#if (defined(_STANDALONE_INLINER) || defined(_LIGHTWEIGHT_INLINER))
        // check for pragmas and command line options before setting this
  	// call to no inline
	if ( !edge.Has_Must_Inline_Attrib() && !callee.Has_Must_Inline_Attrib()) {
...
#else
    // [CL] disable inlining of may-inline functions at -O0
	 callee.Has_Must_Inline_Attrib() ||
	   (callee.Has_Inline_Attrib() && !INLINE_None) ) ) {
#endif
#endif // _LIGHTWEIGHT_INLINER
...
#ifdef TARG_ST
        if (!(INLINE_Keep_PU_Order || INLINE_Sort_PU_Order)
	    || callee->Is_Deletable()) 
#else
...
#ifdef TARG_ST
//----------------------------------------------------------------
// Sort a list of PUs into a preferred output ordering.
//
// The perferred ordering is depth-first ordering of the call graph,
// so that callees are seen by the code generator before their callers.
// This enables the register allocator to calculate the registers
// that are affected by the callee at a call site.
//----------------------------------------------------------------
...
Inliner_Sort_PUs (PU_Info *pu_tree)
{
  ...
  for (iter = pu_sort_map.begin(); iter != pu_sort_map.end(); iter++) {
    PU_Info *pu = iter->second;
    PU_Info_next (pu) = current_pu;
    current_pu = pu;
  }
  return current_pu;
}
...
#ifdef TARG_ST
    if (INLINE_Sort_PU_Order && ! INLINE_Keep_PU_Order) {
      pu_tree = Inliner_Sort_PUs (pu_tree);
    }
#endif
    for (current_pu = pu_tree; current_pu != NULL; current_pu = PU_Info_next (current_pu)) {
...
#ifdef TARG_ST
        /* (cbr) never emit must_delete functions */
        if ((IPA_Enable_DFE && deletable) || PU_must_delete (node->Get_PU ())) {
#else
...
#  ifdef TARG_ST
                if ( ( INLINE_Keep_PU_Order || INLINE_Sort_PU_Order ) && node ) {
#  else
...
// [CL] initialize fake function pointers to avoid linking with
// wopt.so
void Inliner_INIT()
{
