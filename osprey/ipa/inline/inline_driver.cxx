    ...
    for ( i = 1; i < argc; i++ ) {
	  ...
	  continue;
		...
		switch (argv[i][2]) 
			...
			break;
#ifdef TARG_ST
			// [CL] Remember we are at -O0, so that later we can
			// disable DFE, auto inlining and even may-inline
			// functions inlining. We'll revert to -O1 later on.
		    case '0':
			Opt_Level = 0;
			INLINE_Enable_Split_Common = FALSE;
			break;
...
#ifdef TARG_ST
    // [CL] at -O0, disable DFE if not forced by user,
    // same for auto inlining, and ignore may-inline functions
    // as a result, only must-inline functions will be inlined.
    if (Opt_Level == 0) {
      if (!IPA_Enable_DFE_Set)
	IPA_Enable_DFE = FALSE;
      if (!INLINE_O0) {
...
#ifdef TARG_ST
    // [CL] enable configuration of these variables even
    // without IPA, so perform the same consistency checks
    // as with IPA
