...
struct PIC_OPT
{
	...
	if(f_call_shared) {
	    ...
	    // [CL] if we export all global symbols, don't
	    // 'promote' to internal or hidden
	    else if (!ld_ipa_opt[LD_IPA_EXPORTS].flag) {
	      if (address_taken) {
		// [CL] if symbol is already internal (by user attribute)
		// don't demote it to hidden
		if (export_class != EXPORT_INTERNAL)
		  Set_ST_export(st, EXPORT_HIDDEN);
	      } else  {
		Set_ST_export(st, EXPORT_INTERNAL);
	      }
...
struct fix_static_func
{
	...
	BOOL address_taken = ST_addr_saved (st) || ST_addr_passed (st);
        ...
        if (eclass != EXPORT_LOCAL && eclass != EXPORT_LOCAL_INTERNAL) {
	      ...
	      {
		// [CL] if we export all global symbols, don't
		// 'promote' to internal or hidden, unless st
		// is already hidden, in which case it may
		// be promoted to internal
	      if (ld_ipa_opt[LD_IPA_EXPORTS].flag && eclass != EXPORT_HIDDEN)
		return;
...
struct Opt_global_var
{
	...
	case SCLASS_UGLOBAL:
	    ...
	    // [CL] handle aliases
	case SCLASS_EXTERN:
	  if (Has_Base_Block(st) && ST_emit_symbol(st)) {
	    break;
	  }
#endif
	...
	if (TY_is_volatile (ty) ||
	    ...
	    if (Has_Base_Block(st) && ST_emit_symbol(st)) {
	      ...
	      AUX_ST& base_aux_st = Aux_St_Table[base_st_idx];
	      Set_AUX_ST_flags (base_aux_st, IGNORE_REFCOUNTS);
...
{
#ifdef TARG_ST
    UINT32 my_max_gpa_size = IP_get_max_gpa_size ();
    return std::min (my_max_gpa_size, IPA_Gspace);
#else
    UINT32 max_gpa_size = IP_get_max_gpa_size ();
    return std::min (max_gpa_size, IPA_Gspace);
#endif
}
