...
#ifdef TARG_ST
// [CL] for Find_Section_Name_For_ST()
...
typedef __GNUEXT::hash_map<UINT32,UINT32> ST_TO_INITO_MAP;
...
typedef __GNUEXT::hash_map<const TCON*, TCON_IDX, tcon_hash, eq_tcon,
...
typedef __GNUEXT::hash_map<const ST*, ST_IDX, const_st_hash, eq_const_st,
...
Initialize_Auxiliary_Tables ()
{
    ...
    Common_Block_Elements_Map = 
      CXX_NEW (COMMON_BLOCK_ELEMENTS_MAP (100, __GNUEXT::hash<ST_IDX> (),
                                          __GNUEXT::equal_to<ST_IDX> (),
                                          &Common_map_pool), 
               &Common_map_pool);
...
Synch_ST_flags (ST& merged_st, const ST& original_st)
{
    ...
    // [CL] handle flags_ext
    // attributes that can be set iff both ST entries set them
    const UINT8 st_flags_ext_and_mask =
      0;

    // attributes that must be set if any one of the ST entries sets them
    const UINT8 st_flags_ext_or_mask =
      ST_IS_COMDAT | ST_HAS_ALIAS | ST_ALIAS_USED;

    UINT8 original_flags_ext = original_st.flags_ext;
    UINT8 merged_flags_ext = merged_st.flags_ext;

    merged_flags_ext = (merged_flags_ext & ~st_flags_ext_and_mask) |
	(merged_flags_ext & original_flags_ext & st_flags_ext_and_mask);

    merged_flags_ext |= original_flags_ext & st_flags_ext_or_mask;

    merged_st.flags_ext = merged_flags_ext;
#endif
} // Synch_ST_flags
...
// [SC] TLS support
void
Synch_ST_tls_model (ST& merged_st, const ST& original_st)
{
  // Make the merged TLS model the most efficient one used.
  // If that model works in some of the modules, then it must work in all
  // of them.
  ST_TLS_MODEL original = ST_tls_model (original_st);
  if (original > ST_tls_model (merged_st)) {
    Set_ST_tls_model (merged_st, original);
  }
}
#endif
...
#ifdef TARG_ST
    // [CL] Handle aliases: base of merged_st is the merged base of original_st
    if (ST_raw_base_idx (original_st) != ST_st_idx (original_st)) {
      ST_IDX base_idx =
	Merge_Global_St (ST_IDX_index (ST_raw_base_idx (original_st)),
			 original_tabs);
      Set_ST_raw_base_idx(merged_st, base_idx);
    } 
    // [SC] TLS support
    Synch_ST_tls_model (merged_st, original_st);
#endif
...
#ifdef TARG_ST
    // [CL] bcmp fails if there is padding in the data structure,
    // so use a dedicated comparison operator
...
#ifdef TARG_ST
      // [CL] also accept aliases
      if ((ST_raw_base_idx (original_st) == ST_st_idx (original_st)) ||
	  ST_emit_symbol(original_st))
#else
	if (ST_raw_base_idx (original_st) == ST_st_idx (original_st))
#endif
	    return Enter_Original_St (original_tabs, original_st);
...
//
#ifdef TARG_ST
// [CG] Reimplemented it without recursion.
// This avoids system crash for large INITV chains were the 
// recursion stack can overflow the system stack.
// This is mainly an INITV problem as INITV chains can be very large
// for instance when a very large array of char is initialized.
// For initialization of more than 100000 elements for instance
// we have a recursion stack of 100000*80 bytes ~= 8Mbytes. This
// causes crashes on linux and cygwin systems.
static INITV_IDX
		   ...
		   INITV_IDX_MAP	 &initv_map)
{
  ...
  // Iteratively allocate INITV chains.
  while(1) {
...
/* (cbr) DDTSst24451. add support for label diffs initializers */
    ...
    if (INITV_next(original_initv) == 0)  break;
			...
			BOOL is_cmod)
{
	...
	&& !ST_is_used(st) &&
	// [CL] don't delete global vars if we export all global
	// symbols or if we are generating a shared lib (global
	// symbols may be accessed later)
	( ST_is_export_local(st) ||
	  ST_export(st) == EXPORT_HIDDEN ||
	  ST_export(st) == EXPORT_INTERNAL ||
	  !ld_ipa_opt[LD_IPA_EXPORTS].flag &&
...
#ifdef TARG_ST
// [CL] support comdat attribute
BOOL symbol_comdat (ST_IDX st_idx)
{
    ST& st = St_Table[st_idx];

    return ST_is_comdat(st);
}

// [CL] support comdat attribute
BOOL symbol_linkonce (ST_IDX st_idx)
{
    ST& st = St_Table[st_idx];

    if (ST_has_named_section(&st)) {
      STR_IDX name = Find_Section_Name_For_ST (&st);
      if (!strncmp(Index_To_Str(name), ".gnu.linkonce", sizeof(".gnu.linkonce") - 1)) {
	return TRUE;
      }
    }
    return FALSE;
