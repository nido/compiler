...
// [HK] added missing declarations
extern INT Trace_IPA;
...
#ifdef TARG_ST
	// [CG]: We must also handle ASM_INPUT that acts as a 
	// local store of a reference to an address taken symbol
	// if the input is a LDA.
...
#ifdef TARG_ST
  // [CG] Don't use ST_type for functions
  if (ST_class(st) != CLASS_FUNC) {
#endif
