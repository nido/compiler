...
// these entry points, so that we do not need to link with wopt

// from be/opt/opt_du.h
BOOL (DU_MANAGER::CD_is_br_taken) (IDTYPE) { return TRUE;}
BOOL (DU_MANAGER::CD_is_fall_thru) (IDTYPE) {return TRUE;}
BOOL (DU_MANAGER::Dominate) (IDTYPE, IDTYPE) {return TRUE;}
IDTYPE (DU_MANAGER::Get_cd) (IDTYPE) {return 0;}
WN* (DU_MANAGER::Get_last_stmt) (IDTYPE) {return NULL;}

// from be/opt/opt_htable.h
BOOL (CODEREP::Def_at_entry) () const {return TRUE;}
BB_NODE* (CODEREP::Defbb) () const {return NULL;}
...
// from be/opt/opt_du.h
BE_EXPORTED extern BOOL (DU_MANAGER::*CD_is_br_taken_p) (IDTYPE);
BE_EXPORTED extern BOOL (DU_MANAGER::*CD_is_fall_thru_p) (IDTYPE);
BE_EXPORTED extern BOOL (DU_MANAGER::*Dominate_p) (IDTYPE, IDTYPE);
BE_EXPORTED extern IDTYPE (DU_MANAGER::*Get_cd_p) (IDTYPE);
BE_EXPORTED extern WN* (DU_MANAGER::*Get_last_stmt_p) (IDTYPE);

// from be/opt/opt_htable.h
BE_EXPORTED extern BOOL (CODEREP::*Def_at_entry_p) () const;
BE_EXPORTED extern BB_NODE* (CODEREP::*Defbb_p) () const;
...
#if defined(TARG_ST)
    if ((cr->*Def_at_entry_p) () || cr->Is_flag_set (CF_DEF_BY_PHI) ||
#else
...
SUMMARIZE<program>::Process_phi_jump_function (WN *orig_wn, PHI_NODE *phi)
{
...
#if defined(TARG_ST)
    IDTYPE cd0_bb_idx = (cr0->*Def_at_entry_p) () ?
	du->Get_entry_bb () : (du->*Get_cd_p) ((cr0->*Defbb_p)()->Id());
    IDTYPE cd1_bb_idx = (cr1->*Def_at_entry_p) () ?
	du->Get_entry_bb () : (du->*Get_cd_p) ((cr1->*Defbb_p)()->Id());
#else
...
#if defined(TARG_ST)
	if ((du->*CD_is_fall_thru_p) ((cr0->*Defbb_p)()->Id()) ==
	    (du->*CD_is_fall_thru_p) ((cr1->*Defbb_p)()->Id()))
#else
...
#if defined(TARG_ST)
	if ((du->*Dominate_p) (cd0_bb_idx, cd1_bb_idx)) {
	    if (!(du->*Dominate_p) (cd1_bb_idx, phi->Bb()->Id()))
		return -1;
	    merged_cd = cd1_bb_idx;
	} else if ((du->*Dominate_p) (cd1_bb_idx, cd0_bb_idx)) {
	    if (!(du->*Dominate_p) (cd0_bb_idx, phi->Bb()->Id()))
		return -1;
	    merged_cd = cd0_bb_idx;
	}
#else
...
#if defined(TARG_ST)
	switch (WN_opcode ((du->*Get_last_stmt_p) (merged_cd))) {
#else
	...
	case OPC_IF:
		...
		phi_node->Set_branch (0,
				      (du->*CD_is_fall_thru_p)((cr0->*Defbb_p)()->Id()));
	...
	case OPC_TRUEBR:
		...
		phi_node->Set_branch (0,
#if defined(TARG_ST)
				      (du->*CD_is_br_taken_p)((cr0->*Defbb_p)()->Id()));
#else
		...
		phi_node->Set_branch (1,
#if defined(TARG_ST)
				      (du->*CD_is_br_taken_p)((cr1->*Defbb_p)()->Id()));
#else
...
SUMMARIZE<program>:: Record_ref_formal ( WN* w )
{
    ...
    if ( Trace_Modref ) {
	if (WN_operator(w) == OPR_ILOAD) // [CL] include fix from 0.15
...
#if defined(TARG_ST)
    WN *cond_stmt = (du->*Get_last_stmt_p) (cd_bb_idx);
#else
...
SUMMARIZE<program>::Process_control_dependence (WN *w, INT node_index)
{
...
#if defined(TARG_ST)
    IDTYPE cd_bb_idx = (du->*Get_cd_p)(bb_idx);
#else
	...
	return FALSE;
#if defined(TARG_ST)
      cond_stmt = (du->*Get_last_stmt_p)(cd_bb_idx);
#else
    ...
    case OPC_IF:
#if defined(TARG_ST)
      branch = (du->*CD_is_fall_thru_p)(bb_idx);
#else
    ...
    case OPC_TRUEBR:
#if defined(TARG_ST)
      branch = (du->*CD_is_br_taken_p)(bb_idx);
#else
