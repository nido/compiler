...
#ifdef TARG_ST // [CL] we can have more than 2 return regs
    for (int i=0; i<RETURN_INFO_count(return_info); i++) {
      if (WN_store_offset(w) == RETURN_INFO_preg (return_info, i)) {
	return TRUE;
      }
    }
    return FALSE;
...
count_stats(WN *w, INT32& bbs, INT32& stmts, FB_FREQ& cycles, FB_FREQ freq)
{
...
#ifdef TARG_ST
    // [CL] merge CG's modification of
    // commom/com/xstats.cxx Count_WN_Operator()
    if (OPERATOR_is_endsbb(opr) ||
	opr == OPR_DO_LOOP ||
	opr == OPR_DO_WHILE ||
	opr == OPR_IF ||
	opr == OPR_WHILE_DO) {
	...
	cycles += freq;
	  ...
	  opr == OPR_IO) {
	/* nothing (is ++calls in xstats.cxx) */
...
// blocks and statements, i.e., those whose frequence is non-zero.
void
