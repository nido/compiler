...
)
{
  switch (mtype) {
    case MTYPE_I4:
    case MTYPE_U4:
      return (imm) ? TOP_GP32_EQW_GT_BR_DR_U8 : TOP_GP32_EQW_GT_BR_DR_DR;
      break;
    default:
...
)
{
  ...
  if (const_divisor) {
    if (divisor_val == 0) {
      if (div_zero_check) {
	Build_OP (TOP_GP32_TRAP_GT_U4, True_TN, 
...
)
{
  switch (mtype) {
    case MTYPE_I4:
      Build_OP (TOP_IFR_DIVW_GT_DR_DR_DR, result, True_TN,
	                                         src1, src2, ops);
      return;

    case MTYPE_U4:
      Build_OP (TOP_IFR_DIVUW_GT_DR_DR_DR, result, True_TN,
	                                         src1, src2, ops);
      return;

    default:
      FmtAssert (FALSE,("Expand_Intrinsic_Divide: unhandled mtype"));
  }
