...
/////////////////////////////////////
{
  ...
  for (int code = 0; code < PROCESSOR_count; code++) {
    ...
    fprintf (cfile, "  " PRINTF_LONGLONG_FORMAT( "0x", "0*", "x" ) "%s, /* %s:", int_size / 4,
					    flag_value, 
					    int_suffix,
  ...
  fprintf (cfile, "  " PRINTF_LONGLONG_FORMAT( "0x", "0*", "x" ) "%s  /* UNDEFINED */\n"
		  "};\n",
		  int_size / 4,
		  0ULL,
		  int_suffix);
  ...
  for ( isi = properties.begin(); isi != properties.end(); ++isi ) {
    ...
    if (property->bit_position >= 0) {
      fprintf (hfile, "#define PROP_%-16s " PRINTF_LONGLONG_FORMAT( "0x", "0", "x" ) "%s\n", 
	       property->name, 
