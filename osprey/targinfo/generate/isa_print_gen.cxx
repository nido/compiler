...
#define FNAME "targ_isa_print"
  ...
  fprintf(hfile, "\nTARGINFO_EXPORTED extern const ISA_PRINT_INFO ISA_PRINT_info[%d];\n",
						list_index + 1);
  ...
  fprintf(hfile, "\nTARGINFO_EXPORTED extern const unsigned char ISA_PRINT_info_index[%d];\n", TOP_count);
  ...
  if (asmname) {
    fprintf(cfile, "\nTARGINFO_EXPORTED const char * const ISA_PRINT_asmname[] = {\n");
    for (top = 0; top < TOP_count; ++top) {
      fprintf(cfile, "  \"%s\",\n", asmname((TOP)top));
    }
    ...
    fprintf(hfile, "\ninline const char *ISA_PRINT_AsmName(TOP topcode)\n"
		   "{\n"
		   "  TARGINFO_EXPORTED extern const char * const ISA_PRINT_asmname[];\n"
  ...
  //
  fprintf(hfile, "\nTARGINFO_EXPORTED extern BOOL ISA_PRINT_Operand_Is_Part_Of_Name(TOP topcode, INT opindex);\n");
