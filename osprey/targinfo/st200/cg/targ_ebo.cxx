...
/* Define a macro to tell whether a load is unsigned */
#define LOAD_IS_UNSIGNED(o)      \
          (OP_code(o) == TOP_ldhu_i ||    \
	   OP_code(o) == TOP_ldhu_ii ||   \
	   OP_code(o) == TOP_ldhu_d_i ||  \
	   OP_code(o) == TOP_ldhu_d_ii || \
	   OP_code(o) == TOP_ldbu_i ||    \
	   OP_code(o) == TOP_ldbu_ii ||   \
	   OP_code(o) == TOP_ldbu_d_i ||  \
	   OP_code(o) == TOP_ldbu_d_ii)
...
/* =====================================================================
 *   TOP_Is_Unconditional_Compare
 * =====================================================================
 */
static BOOL
TOP_Is_Unconditional_Compare (TOP opcode)
{
  return (TOP_is_icmp(opcode));
}
...
)
{
  ...
  size_pred = TOP_Mem_Bytes(OP_code(pred_op));
  size_succ = TOP_Mem_Bytes(opcode);
...
{
#if 1
  // don't know whetherv this makes a whole lot of sence.
  return FALSE;
#else
  OP *pred_op = opinfo->in_op;
  BB *bb = OP_bb(op);
  ...
  size_pred = TOP_Mem_Bytes(OP_code(pred_op));
  size_succ = TOP_Mem_Bytes(OP_code(op));
  if (size_pred < size_succ) return FALSE;
  ...
  TOP extr_op = LOAD_IS_UNSIGNED(op) ? TOP_shru_i : TOP_shr_i;
  Build_OP(extr_op, 
	   succ_result, 
	   tmp, 
  ...
  if (EBO_in_loop) 
    // but why ??
    EBO_Set_OP_omega (OPS_first(&ops), NULL, NULL, NULL, NULL);
...
)
{
 ...
 /* Be sure we have a "Store .. Store .. Load" pattern. */
  ...
  size_succ = TOP_Mem_Bytes(opcode);
  size_pred = TOP_Mem_Bytes(pred_opcode);
  size_intervening = TOP_Mem_Bytes(intervening_opcode);
  ...
  if ((OP_store(pred_op) && (OP_results(pred_op) > 1)) ||
      (OP_store(intervening_op) && (OP_results(intervening_op) > 1)))
    return FALSE;

  if ((OP_load(pred_op) && (OP_results(pred_op) > 1)) ||
      (OP_load(intervening_op) && (OP_results(intervening_op) > 1)))
    return FALSE;
  ...
  if (TNs_Are_Equivalent(pred_result, intervening_result)) {
     ...
     * off sign bit.
     */
    if ((size_succ != 4) && LOAD_IS_UNSIGNED(op)) {
      /* 
     ...
     /* Use extract instruction to simulate store-load sequence.          */
      ...
      /* Use full word copy. */
      Exp_COPY(OP_result(op, 0), pred_result, &ops);
      OP_srcpos(OPS_last(&ops)) = OP_srcpos(op);
    }
  ...
  INT pred_offset_idx = TOP_Find_Operand_Use(pred_opcode, OU_offset);
  INT intervening_offset_idx =  TOP_Find_Operand_Use(intervening_opcode, OU_offset);
  ...
  TN *pred_index = OP_opnd(pred_op, pred_base_idx);
  TN *intervening_index = OP_opnd(intervening_op, intervening_base_idx);
   ...
   * and reuse the predecesor value if they are not the same.
   */

  /* Arthur: If load/stores are of the form ldw/stw (sym, base)
   *         Compare sym should suffice.
   */
  if ((TN_is_symbol(pred_offset) && !TN_is_symbol(intervening_offset)) ||
    ...
    Build_OP (TOP_cmpeq_r_b, predicate, pred_index, intervening_index, &ops);
    OP_srcpos(OPS_last(&ops)) = OP_srcpos(op);
...
)
{
  ...
  size_pred = TOP_Mem_Bytes(OP_code(opinfo->in_op));
  size_succ = TOP_Mem_Bytes(opcode);
  ...
  if (OP_load(op) && OP_load(opinfo->in_op)) {
    ...
    */
    ...
    Exp_COPY(OP_result(op, 0), OP_result(opinfo->in_op, 0), &ops);
#if 0
    if (OP_results(op) == 2) {
      EBO_Exp_COPY(predicate_tn, 
		   OP_result(op, 1), OP_result(opinfo->in_op, 1), &ops);
    }
#endif
	...
	*/
	...
	return FALSE;
      ...
      Exp_COPY(OP_result(op, 0), storeval_tn, &ops);
...
)
{
  ...
  l0_opnd1_idx = 0;
  l0_opnd2_idx = (OP_opnds(op) > 1) ? 1 : -1;
  ...
  if ((const_val == 0) &&
    ...
    EBO_Exp_COPY(NULL, tnr, Zero_TN, &ops);
  ...
  if (opcode == TOP_add_r) {
    ...
    OP_srcpos(new_op) = OP_srcpos(op);
    if (EBO_in_loop) 
      EBO_Set_OP_omega (new_op, NULL, opnd_tninfo[l0_opnd2_idx],NULL);
  ...
  if (opcode == TOP_sub_r) {
    ...
    if (EBO_in_loop) 
      EBO_Set_OP_omega (new_op, NULL, NULL, opnd_tninfo[l0_opnd2_idx] );
  ...
  if (opcode == TOP_and_r) {
    OP *new_op;
    if (ISA_LC_Value_In_Class (const_val, LC_s9)) {
      new_op = Mk_OP(TOP_and_i, 
		     tnr,
		     tn1,
                     Gen_Literal_TN(const_val, TN_size(tn1)));
    }
    else {
      new_op = Mk_OP(TOP_and_ii, 
		     tnr,
		     tn1,
                     Gen_Literal_TN(const_val, TN_size(tn1)));
    }
    ...
    if (EBO_Trace_Optimization) 
      fprintf(TFile,"replace and with and_i\n");
  ...
  if (opcode == TOP_or_r) {
    OP *new_op;
    if (ISA_LC_Value_In_Class (const_val, LC_s9)) {
      new_op = Mk_OP(TOP_or_i, 
		     tnr, 
		     tn1,
		     Gen_Literal_TN(const_val, TN_size(tn1)));
    }
    else {
      new_op = Mk_OP(TOP_or_ii, 
		     tnr, 
		     tn1,
		     Gen_Literal_TN(const_val, TN_size(tn1)));
    }
    ...
    if (EBO_Trace_Optimization) 
      fprintf(TFile,"replace or with or_i\n");

    OP_srcpos(new_op) = OP_srcpos(op);
    if (EBO_in_loop) 
      EBO_Set_OP_omega (new_op, opnd_tninfo[l0_opnd2_idx], NULL);
  ...
  if (opcode == TOP_xor_r) {
    OP *new_op;
    if (ISA_LC_Value_In_Class (const_val, LC_s9)) {
      new_op = Mk_OP(TOP_xor_i, 
		     tnr, 
		     tn1,
		     Gen_Literal_TN(const_val, TN_size(tn1)));
    }
    else {
      new_op = Mk_OP(TOP_xor_ii, 
		     tnr, 
		     tn1,
		     Gen_Literal_TN(const_val, TN_size(tn1)));
    }
    ...
    if (EBO_Trace_Optimization) 
      fprintf(TFile,"replace xor with xor_i\n");
  ...
  if (OP_icmp(op)) {
    ...
    TOP new_opcode = TOP_Immediate_Compare(opcode, new_const_val);
	...
	TOP_Can_Have_Immediate(new_const_val, new_opcode)) {
      ...
      if (EBO_Trace_Optimization) 
	fprintf(TFile,"replace cmp with cmp_i\n");

      Build_OP (new_opcode, 
		r1, 
		tn1,
                Gen_Literal_TN(new_const_val, TN_size(tn1)), &ops);

      if (EBO_in_loop) 
	EBO_Set_OP_omega (OPS_first(&ops), opnd_tninfo[l0_opnd2_idx], NULL);
...
)
{
  ...
  if (const_val == 0) {
	...
	fprintf(TFile,"replace shift/subtract of zero\n");
      ...
      EBO_Exp_COPY(NULL, tnr, tn0, &ops);
      if (EBO_in_loop) 
	EBO_Set_OP_omega (OPS_first(&ops), opnd_tninfo[l0_opnd1_idx]);
    ...
    if ((opcode == TOP_and_r) || (opcode == TOP_and_i)) {
      OPS ops = OPS_EMPTY;

      if (EBO_Trace_Optimization) 
	fprintf(TFile,"replace and with zero\n");
  ...
  if (const_val > 31) {
	...
	fprintf(TFile,"replace shift with zero\n");
      ...
      EBO_Exp_COPY(NULL, tnr, Zero_TN, &ops);
      if (EBO_in_loop) 
	EBO_Set_OP_omega (OPS_first(&ops), NULL);
      OP_srcpos(OPS_first(&ops)) = OP_srcpos(op);
      BB_Insert_Ops(OP_bb(op), op, &ops, FALSE);
      return TRUE;
    }
  ...
  if (opcode == TOP_add_r) {
    TOP new_opcode = ISA_LC_Value_In_Class (const_val, LC_s9) ? TOP_add_i : TOP_add_ii;
    OP *new_op = Mk_OP(new_opcode, 
		       tnr, 
		       tn0,
    ...
    OP_srcpos(new_op) = OP_srcpos(op);
    if (EBO_in_loop) 
      EBO_Set_OP_omega (new_op, opnd_tninfo[l0_opnd1_idx], NULL);
    BB_Insert_Op_After(OP_bb(op), op, new_op);

    if (EBO_Trace_Optimization) 
      fprintf(TFile,"Replace add with add_i\n");
    return TRUE;
  }
