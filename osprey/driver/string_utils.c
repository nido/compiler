...
#ifdef TARG_ST
	/* If no suffix add .<suffix> */
	if (p == NULL) {
	  string tmp = (string) malloc(strlen(new) + strlen(suffix) + 1 + 1);
	  strcpy(tmp, new);
	  strcat(tmp, ".");
	  strcat(tmp, suffix);
	  return tmp;
...
/* add string that has already been allocated */
static void
prepend_existing_string (string_list_t *list, string s)
{
	string_item_t *p;
	p = (string_item_t *) malloc(sizeof(string_item_t));
	p->name = s;
...
extern void
prepend_string (string_list_t *list, string s)
{
  /* don't worry about blanks in this version of add_string */
  prepend_existing_string (list, string_copy(s));
}
...
#ifdef TARG_ST
				/* (cbr) keep args together after name DDTSst21437 */
				break;
#endif
			}
		}
