...
#ifdef sun /* this symbol is already used for signals */
#ifdef S_NONE
#undef S_NONE
#endif
#endif
...
typedef enum {
	...
	P_ldsimple,	/* Last Link phase for cache optims: simple ld (to resolve relocation) */
	P_binopt, /* Icache optimization module */
...
#ifdef TARG_ST
/* TB: For ld -r relocatable link need for binopt phase, add -Wz,*
   options */
#define phasekey_for_ld_r 'y'
#define phasekey_for_simple_ld 'z'
#define phasekey_for_ld 'l'
#endif
...
#ifdef TARG_ST
extern mask_t CMPLRS_MASK;	/* mask for all cmplrs phases */
extern mask_t TOOLCHAIN_MASK;	/* mask for all toolchain components */
#endif
...
#ifdef TARG_ST
extern char get_phase_key (mask_t phase_mask);
#endif
