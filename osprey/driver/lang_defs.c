...
#ifdef TARG_ST200
	{'c',	0x00000002,	{"cc","st200cc"}},	/* cc */
	{'C',	0x00000004,	{"CC","st200c++"}},	/* c++ */
	{'f',	0x00000008,	{}}, /* f77 */
	{'F',	0x00000010,	{}},	/* f90 */
...
static phase_info_t phase_info[] = {
   ...
   {'p',  0x0000000000000020LL,	"gfec", PHASEPATH, TRUE}, /* gcpp */
   {'p',  0x0000000000000040LL,	"gfecc", PHASEPATH, TRUE}, /* gcppp */
#else
   {'p',  0x0000000000000020LL,	"cpp", PHASEPATH, FALSE}, /* gcpp */
   {'p',  0x0000000000000040LL,	"cpp", PHASEPATH, FALSE}, /* gcppp */
#endif
   ...
   {'p',  0x0000000000000040LL,	NAMEPREFIX "g++", BINPATH, FALSE}, /* gcppp */
   {'p',  0x0000000000000080LL,	"fec",	 PHASEPATH,	FALSE},	/* c_cpp */
   {'p',  0x0000000000000100LL, "cpp",   PHASEPATH,     FALSE}, /* cplus_cpp */
#endif /* mips */
#endif /* TARG_ST */
...
#ifdef TARG_ST
   {'a',  0x0000002000000000LL,	NAMEPREFIX "as", BINPATH,	FALSE},	/* gas */
#else
...
/* #else /\* BCO_Enabled Thierry *\/ */
/*           0x030fffffffffffffLL; */
/* #endif /\* BCO_Enabled Thierry *\/ */
mask_t LIB_MASK =
/* #ifndef BCO_ENABLED /\* Thierry *\/ */
          0xfff0000000000000LL;
/* #else /\* BCO_Enabled Thierry *\/ */
/*           0xfcf0000000000000LL; */
/* #endif /\* BCO_Enabled Thierry *\/ */
...
/* #endif /\* BCO_Enabled Thierry *\/ */
mask_t TOOLCHAIN_MASK =
0x00000C2000000000LL; /* as , ld, ldplus */ 
#endif
...
#ifdef BCO_ENABLED
	/* TB: phase key to access ld-r phase when doing binopt optims */
	if (key == phasekey_for_ld_r || key == phasekey_for_simple_ld) {
	  return P_any_ld;
	}
#endif
...
// [CL]
extern char 
get_phase_key (mask_t phase_mask)
{
        int i;
	/* iterate in reverse, so find generic fe for 'f' */
	for (i = P_LAST-1; i >= (int) P_NONE; i--) {
		if (( phase_info[i].mask != 0) &&
		    (phase_info[i].mask == phase_mask)) {
			return phase_info[i].key;
		}
	}
	return '\0';
}
...
#ifdef DRIVER
/* [CG] The following is only compiled if we effectively compile the driver
 * not if we compile the tools for the driver (table for instance).
 */
...
prefix_all_phase_dirs (mask_t mask, string path)
{
  ...
  for (i = P_NONE; i < P_LAST; i++) {
    if ((phase_info[i].mask & mask) != 0) {
      /* Check that we really have a phase_info dir. */
      if (*phase_info[i].dir != '\0') {
	phase_info[i].dir = 
	  concat_path(path, phase_info[i].dir);
      }
    }
  }
}
...
append_all_phase_dirs (mask_t mask, string path)
{
	...
	for (i = P_NONE; i < P_LAST; i++) {
		if ((phase_info[i].mask & mask) != 0) {
			...
			{
				phase_info[i].dir = 
					concat_path(phase_info[i].dir, path);
			}
		}
	}
}
...
{
	phase_info[index].dir = concat_path (phase_info[index].dir, path);
}
...
{
  return concat_strings(concat_path(get_phase_dir(index), get_phase_name(index)), EXE_SUFFIX);
}
...
get_named_language (string name)
{
	...
	for (i = L_NONE; i < L_LAST; i++) {
		...
		p = name+strlen(name);
                {
		  /* take possible exe suffix into account" */
		  string t = p - strlen(EXE_SUFFIX);
		  if (t >= name && same_string(EXE_SUFFIX, t))
		    p = t;
                }
		p -= strlen(language_info[i].name[j]);
		if (p >= name && same_string_prefix(p, language_info[i].name[j])) {
