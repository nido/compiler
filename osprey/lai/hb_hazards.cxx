...
#if 1
	Is_True(estart <= OP_scycle(op), ("BB %d, Clock %d, op %s: illegal bundle generated by the scheduler", BB_id(bb), Clock, TOP_Name(OP_code(op))));
#else
	if (estart > OP_scycle(op))
	  DevWarn("BB %d, Clock %d, op %s: illegal bundle generated by the scheduler", BB_id(bb), Clock, TOP_Name(OP_code(op)));
#endif
	if (estart < OP_scycle(op))
	  estart = OP_scycle(op);
      }
