...
#ifdef TARG_ST200
  BOOL  LoadStore_Check_Packing( CG_LOOP &cg_loop, BB *body );
  BOOL  LoadStore_Packing( CG_LOOP &cg_loop, BB *body );
#endif
};
...
Pack32_Get_Alignment(OP *memop, INT64 offset) {
  ...
  // the variable on which the alignment on known. Use loophead to
  ...
  */
  ...
  // Look for a pragma STREAM_ALIGN
  INT pragma_align = -1;
  ...
  if (align_ant) {
    ...
    pragma_align = WN_pragma_arg1(wn);
  }

  if (pragma_align == -1) {
    if ((aligned_memop == -1) && (CG_LOOP_stream_align == 8))
      aligned_memop = 0;
  }
  else if (pragma_align == 8) { /* stream alignment is 0%8 */
    if (aligned_memop == 4) {
      DevWarn("Pack32_Get_Alignment found an alignment 4%%8, in loop %d with pragma STREAM_ALIGNMENT(8). Pragma ignored.", BB_id(loophead));
    }
  }
  else if (pragma_align == 4) { /* stream alignment is 0%4 */
    if (aligned_memop != -1) {
      DevWarn("Pack32_Get_Alignment found an alignment %d%%8, in loop %d with pragma STREAM_ALIGNMENT(4). Pragma ignored.", aligned_memop, BB_id(loophead));
    }
  }
...
static BOOL
Combine_Adjacent_Loads( Pack32_entry *pack32_table, Candidate_Memory_t *memop_table)
{
  ...
  if (Get_Trace(TP_CGLOOP, 0x10))
      fprintf(stdout, "<packing> Combine adjacent %d Loads\n", memop_count);
  ...
  INT64 IV_step = Get_IV_step(pack32_table, IV_index);
  ...
  */
  ...
  for (int candidate_index = 0; candidate_index < memop_count; candidate_index += 2) {
    ...
    OP *load1_op = pack32_table[memop_table[candidate_index].index].op;
    OP *load2_op = pack32_table[memop_table[candidate_index+1].index].op;
	...
	if (OP_load(new_op)) {
	  ...
	  if (Get_Loopdep_Kind(body) != 0) {
	    if (Get_Trace(TP_CGLOOP, 0x10) && candidate_index == 0)
	      fprintf(stdout, "<packing> Optimistics dependences with LOOPDEP.\n");
...
static BOOL
Combine_Adjacent_Stores( Pack32_entry *pack32_table, Candidate_Memory_t *memop_table )
{
  ...
  OP *store_op = pack32_table[store_index].op;

  int IV_index = Get_IV_index(pack32_table, store_index);
  OP *IV_op = pack32_table[IV_index].op;
  TN *IV_addr_tn = OP_result(IV_op, 0);
  ...
  int IV_offset = Get_IV_offset(pack32_table, store_index);
  ...
  */
  ...
  if (Get_Trace(TP_CGLOOP, 0x10))
    fprintf(stdout, "<packing> Combine adjacent %d Stores\n", memop_count);
  ...
  for (int candidate_index = 0; candidate_index < memop_count; candidate_index += 2) {
    ...
    OP *store1_op = pack32_table[memop_table[candidate_index].index].op;
    OP *store2_op = pack32_table[memop_table[candidate_index+1].index].op;
...
static void
Init_Pack32_Table(Pack32_entry *pack32_table, INT pack32_count, OP *first_op, hTN_MAP32 tn_last_op) {

  INT index;
  OP *op = first_op;
  for ( index = 1; index <= pack32_count; ++index ) {
  ...
  for ( index = 1; index <= pack32_count; index++ ) {
	...
	while ( arcs ) {
	  ...
	  INT index_pred = Pack32_Lookup_Op( pack32_table,
					     pack32_count, ARC_pred( arc ) );
	  if ( index_pred && !Packing_Load_Candidate_Arc( arc ) ) {
	...
	ARC_LIST *arcs = OP_succs( entry.op );
	while ( arcs ) {
	  ARC *arc = ARC_LIST_first( arcs );
	  arcs = ARC_LIST_rest( arcs );

	  INT index_succ = Pack32_Lookup_Op( pack32_table,
					     pack32_count, ARC_succ( arc ) );
	  if ( index_succ && !Packing_Store_Candidate_Arc( arc ) ) {
	    Reset_OP_flag1(entry.op);
	    Reset_OP_flag1(pack32_table[index_succ].op);
	  }
	}
      }
    }
...
static INT
Compute_Pack32_IVs(Pack32_entry *pack32_table, INT pack32_count) {
...
static INT
Find_Consecutive_Memops(Pack32_entry *pack32_table, INT pack32_count, Candidate_Memory_t *memop_table) {
  ...
  for ( index = 1; index <= pack32_count; ++index ) {
    ...
    int IV_offset = Get_IV_offset(pack32_table, index);
    INT64 step = Get_IV_step(pack32_table, IV_index);
    ...
    INT memop_idx = 0;
    memop_table[memop_idx].index = index;
    ...
    for ( int index2 = index+1; index2 <= pack32_count; ++index2 ) {
	...
	continue;
      ...
      int IV_offset2 = Get_IV_offset(pack32_table, index2);
	...
	memop_table[memop_idx].index = index2;
    ...
    int alignment = Pack32_Get_Alignment(pack32_table[memop_table[0].index].op, memop_table[0].offset);
...
BOOL
CIO_RWTRAN::LoadStore_Packing( CG_LOOP &cg_loop, BB *body )
{
  ...
  Pack32_entry *pack32_table
    = (Pack32_entry *) CXX_NEW_ARRAY( Pack32_entry, pack32_count + 1,
                                     _loc_mem_pool );
  ...
  Init_Pack32_Table(pack32_table, pack32_count, BB_first_op( body ), tn_last_op);

  // Make a pass to initialize the IV variables for each candidate
  // load/store, and see if there are opportunities for packing.

  INT Candidate_count = Compute_Pack32_IVs(pack32_table, pack32_count);
  ...
  while ((memop_count = Find_Consecutive_Memops(pack32_table, pack32_count, candidate_stream)) != 0) {
  ...
  for ( candidate_stream = memop_table;
	...
	cg_loop.Peel_stream()) {
      OP *base_op = pack32_table[candidate_stream->index].op;
	...
	if (Get_Trace(TP_CGLOOP, 0x10)) {
	  fprintf(stdout, "<packing> Using static alignment %d after loop peeling.\n", candidate_stream->alignment);
	}
      }
    }
	...
	if ((OP_map_idx(cg_loop.Special_stream_op(idx)) == orig_idx) &&
	  ...
	  if (Get_Trace(TP_CGLOOP, 0x10)) {
	    fprintf(stdout, "<packing> Using static alignment %d after loop specialization.\n", candidate_stream->alignment);
	  }
	  break;
	}
      }
    ...
    if (OP_load(pack32_table[candidate_stream->index].op) && ((CG_LOOP_load_store_packing&0x4) != 0)) {
      // Check also that alignment is statically known
      if (candidate_stream->alignment == -1)
	continue;
    }
    else if (OP_store(pack32_table[candidate_stream->index].op)) {
      if (candidate_stream->aligned != 1)
	continue;
    }

    // Generate the code to pair load/store together.
    if (OP_load(pack32_table[candidate_stream->index].op)) {
      if (Get_Trace(TP_CGLOOP, 0x10)) {
	const char *align_kind = (candidate_stream->alignment == -1) ? "select" : "static";
	fprintf(stdout, "<packing> Load stream, %d memops, alignment %s(%d)\n", candidate_stream->count, align_kind, candidate_stream->alignment);
      }
      if (Combine_Adjacent_Loads(pack32_table, candidate_stream)) {
	load_stream_count ++;
	packing_done = TRUE;
      }
    }
      ...
      if (Get_Trace(TP_CGLOOP, 0x10)) {
	fprintf(stdout, "<packing> Store stream, %d memops, alignment static(%d)\n", candidate_stream->count, candidate_stream->alignment);
      }
      if (Combine_Adjacent_Stores(pack32_table, candidate_stream)) {
	store_stream_count ++;
	packing_done = TRUE;
      }
    }
  ...
  if (Get_Trace(TP_CGLOOP, 0x10)) {
    ...
    fprintf(stdout, "<packing> new_latency_II %d, new_resource_II %d %s(memop=%d)\n", latency_II, resource_II, (mem_cnt == resource_II) ? "(memory bounded)" : "", mem_cnt);
  }
...
BOOL
CIO_RWTRAN::LoadStore_Check_Packing( CG_LOOP &cg_loop, BB *body )
{
  // Count the number of OPs in the loop body.
  OP *op;
  INT pack32_count = 0;
  ...
  // Initialize the table with the streams
  for (candidate_stream = memop_table;
       (memop_count = Find_Consecutive_Memops(pack32_table, pack32_count, candidate_stream)) != 0;
       candidate_stream += memop_count) {
	...
	fprintf(stdout, "<packing> Store stream, %d memops, alignment %s(%d)\n", candidate_stream->count, align_kind, candidate_stream->alignment);
      }
  ...
  if ((CG_LOOP_load_store_packing&0x80) && !doPeeling &&
    ...
    if (dynamic_streams > 4) {
      if (Get_Trace(TP_CGLOOP, 0x10))
	fprintf(stdout, "<packing> Specialization cannot be applied on loops with more than 4(%d) dynamic streams.\n", dynamic_streams);
    }
  ...
  if ((doPeeling || doSpecialize) && (loop_iter <= 16)) {
    if (Get_Trace(TP_CGLOOP, 0x10)) {
      ...
      fprintf(stdout, "<packing> %s cannot be applied on loops with less or equal to 16 iterations.\n", text);
    }
  }
  else if (doPeeling) {
    // Mark the loop for peeling
    int memop_index = candidate_stream->index;
    OP *base_op = pack32_table[memop_index].op;
    int IV_index = Get_IV_index(pack32_table, memop_index);
    INT64 IV_step = Get_IV_step(pack32_table, IV_index);

    cg_loop.Set_peel_op(base_op);
    // TBD: check peel_tn is available at loop entry
    Is_True(OP_opnd(base_op, TOP_Find_Operand_Use(OP_code(base_op), OU_base)) == OP_result(pack32_table[IV_index].op, 0),
  ...
  if (cg_loop.Special_streams() == 0) {
	...
	continue;
      ...
      // be packed.
      if (OP_store(pack32_table[candidate_stream->index].op) &&
  ...
  if (Get_Trace(TP_CGLOOP, 0x10)) {
    if (cg_loop.Even_factor()) {
      fprintf(stdout, "<packing> Loop requires even unroll factor\n");
    }
    fprintf(stdout, "<packing> before unrolling: Load stream %d, Store stream %d, Peeling %d, Special %d\n", load_stream_count, store_stream_count, cg_loop.Peel_stream(), (cg_loop.Special_streams() > 0));
  }
...
BOOL Perform_Load_Store_Packing( CG_LOOP &cg_loop )
{
  ...
  BOOL changed_loop = FALSE;
  {
    ...
    changed_loop = cio_rwtran.LoadStore_Packing( cg_loop, body );
...
BOOL Analyze_Load_Store_Packing( CG_LOOP &cg_loop )
{
...

  {
    ...
    can_be_packed = cio_rwtran.LoadStore_Check_Packing( cg_loop, body );
